...
> **历史归档**：本文保存 2026 年 7—8 月设备调试过程中的日期化记录，内容没有继续更新。
> 其中的“当前基线”“下一步”和候选目录均可能过期。通用排错流程见
> [`../TROUBLESHOOTING.md`](../TROUBLESHOOTING.md)，当前状态见
...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。当前 next160 已在 `screen=640x480 dpi=192` 通过至 TEST122；next157-159 的代理对失败/诊断包均已由 next160 替代。next161 首轮运行在 TEST13 第三段 TLS 握手 EOF 时停止，TEST123 未执行；next162 仅加入一次主文档 GET 握手 EOF 重试和 retry 遥测。自动探针不创建系统 IME context，不能替代真实 SIP 输入。TEST79/float 候选已撤回。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
...
- 2026-08-09：`C:\WMShare\Positron-next161\test_host.log` 的 example.com 和 IANA Example Domains 均成功，Reserved Domains 在 HTTP 状态前报 `ssl_handshake -0x7280`、`recv n=0 (peer closed mid-handshake)`，TEST13 `2/3` 后终止，故日志没有任何 TEST123 结果。next162 只在主文档幂等 GET 同时满足 status 0、空 body 和该握手 EOF 分类时重试一次；第二次仍失败就原样报错。若日志出现 `retry=1`，应继续核对最终导航结果；不得把它解读成 POST、DNS 或子资源重试。
