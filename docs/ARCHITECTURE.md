...
- `sessionStorage`/`localStorage` 的独立 Storage facade；条目、键和值受头文件固定预算约束，新增/超长写入在 mutation 前抛出 `QuotaExceededError`，替换和删除后的容量回收保持可用；宿主不提供持久磁盘或跨 session 存储。
- 有界图像 `decode()` Promise 与 host 驱动的 `load`/`error` 终态；source 或支持的
  ...
  缺失、过旧或不匹配 generation，以及无法解析到带 id `<img>` 的 source 关系，均
  fail closed；
...
Script session 的 native function 数量、listener、collection、Fragment、selector、字符串、FormData/URLSearchParams pairs、Storage entries 和任务队列都受固定预算约束。超限、参数错误、回调缺失、stale handle 和不支持的 Web API 均按头文件约定返回安全失败或 no-op，不伪造完整浏览器行为。
