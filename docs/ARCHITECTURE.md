...
WM6 application / test_host
        ...
        +-- positron_browser
              ...
              +-- script bootstrap + DOM read/write/attribute/value/checked/event bridge
              +-- remaining defaultValue/form/input/location/navigation bridge (in migration)
...
bootstrap、按 id 查询元素、读取/写入 textContent、attribute、input value、checked 以及 event 的 DOM JSON 分发
已由此 DLL 持有并执行；defaultValue/selectedIndex、其余 form/input/location/navigation 适配和
页面生命周期会逐步迁入此 DLL。窗口、
...
2. browser layer 通过稳定 ABI 注册宿主提供的 typed DOM 读写/attribute/value/checked/event 适配，并逐步承接表单和导航适配；
3. browser layer 持有并执行产品 bootstrap；后续把 form/input/location/navigation callback 实现从 `test_host` 迁入 browser layer；
4. 宿主继续提供资源、窗口和控件回调，browser layer 在页面提交、失败或关闭时释放 context 和 bridge。

因此浏览器绑定不是第二个引擎，也不应把 Duktape 或 libdom 类型暴露成公共 ABI。当前
history/session、脚本 context 所有权、bootstrap 和 DOM 读写/attribute/value/checked/event callback 分发已进入
