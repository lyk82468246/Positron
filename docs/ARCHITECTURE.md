...
WM6 application / test_host
        ...
        +-- positron_browser
              ...
              +-- script bootstrap + DOM read/write/attribute/value/checked/disabled/required/readOnly/multiple/noValidate/formNoValidate/min/max/step/form-property/navigation/location/event/native-input/key/focus/edit-input/click/programmatic-click/form-event/invalid/file-input/checkbox-radio-change/select bridge
...
bootstrap、按 id 查询元素、读取/写入 textContent、attribute、input value、checked、`HTMLElement.disabled`、约束相关
...
programmatic `HTMLElement.click()`（包括 file input 的 typed click 边界）/submit/reset/invalid/file-input/checkbox/radio input/change/SELECT-input/change
...
2. browser layer 通过稳定 ABI 注册宿主提供的 typed DOM 读写/attribute/value/checked/`HTMLElement.disabled`/约束相关 reflected properties/form-property/navigation 适配，承接同文档 location/history 事件分发和 native input/composition/keyboard/focus/EDIT-change/post-change-input/click/programmatic-click（file input 只承接 typed click，系统 picker 仍由宿主触发）/submit-reset/invalid/file-input/checkbox/radio input/change/SELECT-input/change dispatch contract，并逐步承接其余表单适配；
...
history/session、脚本 context 所有权、bootstrap 和 DOM 读写/attribute/value/checked/disabled/constraint-reflection/form-property/
