...
WM6 application / test_host
        ...
        +-- positron_browser
              ...
              +-- script bootstrap + DOM read/write/attribute/value/checked/form-property/navigation/location/event/native-input/key/focus/edit-input/click/form-event/invalid/select bridge
...
DOM JSON 分发以及 native input/composition/keyboard/focus-family/EDIT-change/post-change-input/click/submit-reset/invalid/SELECT-input/change typed dispatch entry 已由此 DLL 持有并执行；
...
2. browser layer 通过稳定 ABI 注册宿主提供的 typed DOM 读写/attribute/value/checked/form-property/navigation 适配，承接同文档 location/history 事件分发和 native input/composition/keyboard/focus/EDIT-change/post-change-input/click/submit-reset/invalid/SELECT-input/change dispatch contract，并逐步承接其余表单适配；
...
navigation/location-event/native-input/keyboard/focus/EDIT-change/post-change-input/click/submit-reset/invalid/SELECT-input/change dispatch entry 已进入 `positron_browser.dll`，其余 DOM bridge 仍在迁移中且默认关闭；
