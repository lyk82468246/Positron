...
Script session 的 native function 数量、listener、collection、Fragment、selector、字符串、FormData/URLSearchParams pairs 和任务队列都受固定预算约束。超限、参数错误、回调缺失、stale handle 和不支持的 Web API 均按头文件约定返回安全失败或 no-op，不伪造完整浏览器行为。
