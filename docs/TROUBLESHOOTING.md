...
Windows CE 可能在系统范围继续复用已加载 DLL。只替换一个文件、从多个 stage 目录拼包或保留旧进程，都可能造成“新 EXE + 旧 DLL”的假回归。设备门会为每次远端启动使用带时间戳的唯一宿主 basename 来降低旧路径/名称复用风险，但它不能代替关闭超时进程。最可靠的做法是关闭进程、使用新的隔离 stage 目录，必要时重启模拟器。
...
宿主写日志时会一直保持 `test_host.log` 的写句柄。WM6/RAPI 1 可能在文件已经成功打开后，让某一次 `CeReadFile` 因并发写入暂时失败；设备门把这次复制当作“快照尚未就绪”，从文件开头重新尝试，而不是把它判成测试失败。部署文件先写入门自己生成的唯一 `.part-*` 文件，完整关闭后再用同卷 `CeMoveFile` 改成最终名称；已知的连接重置/设备访问暂态会释放并重新打开当前 RAPI 会话后重试当前文件一次。等待窗口结束时，设备门还会释放并重开当前会话，再做一次两次稳定快照的完整日志回收；只有这次恢复也失败才返回超时。只有读到最终 `TESTBENCH PASS`/`TESTBENCH FAIL`，并在两次完整复制之间保持不变，才会进入结果校验和远端目录清理。若在宿主启动前两次复制都失败，或最终日志恢复仍没有终态，应保留本地不完整证据并按连接、宿主崩溃或设备空间继续取证，不能把它写成产品测试失败。

`CeCreateProcess` 在 Windows CE 上不支持 `lpCurrentDirectory`；设备门使用绝对远端
可执行路径并传入 `NULL`，不会依赖工作目录。新目录或新二进制第一次启动时，设备可能
显示“未知程序”安全警示；应在设备 GUI 中确认该程序后等待 gate 继续。若复制全部成功却
仍返回设备错误 `30123`，先确认安全警示已处理、旧宿主进程已关闭且 EXE/DLL 来自同一
stage，再保留完整证据重试；不要把该设备端加载错误误写成 Core 测试断言失败。
...
`tests=` 范围内的每个编号都必须可运行；例如 `1-45` 包含已撤回的 TEST23，应写成 `1-22,24-45`。TEST999 单独列出。设备门会根据当前源码的 dispatcher 检查选择，并要求唯一的 `auto=1`；无效编号、倒序范围或手动模式在部署前报错。运行 `pwsh -File scripts/test_device_gate_config.ps1` 可验证这层检查。

设备上的配置文件存在但为空、不可读或格式错误时，宿主记录 `TESTBENCH FAIL` 并退出，不再弹出 Ignore 后继续分组选择。只有移走 INI 才进入分组选择；要保留指定测试的手动验收，应使用有效 INI 和 `auto=0`。旧宿主仍可能显示 `test_host.ini ignored`，此时先核对 EXE 的构建身份。

### 系统错误报告与 PASS 冲突

先用报告中的 AppStamp、ModStamp 和模块偏移匹配保留的 PE 二进制，再使用匹配的 PDB/反汇编定位；截图保存时间不等于崩溃发生时间。即使日志已有 PASS，同一进程的系统异常仍必须排查。若故障位于函数入口下移栈指针后的首次写栈，应检查大型自动变量和栈探测，而不是只增加栈预算。Core 内部 libcss 的样式选择与 computed-style 初始化均使用每次调用独立的堆状态，失败和成功路径均释放，避免 ARMV4I 大栈帧跨越保护页。

设备门在启动前记录 `\Windows\System\DumpFiles` 的 `.kdmp` 清单，完整日志回收后等待短暂的转储落盘窗口，再比较路径、大小和修改时间。新增或变化的转储令 `crash_check=NEW_DUMPS`，检查失败为 `UNAVAILABLE`；两者都阻止 PASS、保留当前部署，并尽力回收新转储。结果中的 `new_crash_dump_count=0` 仅表示观察窗口内没有新增转储，不能保证未启用错误报告的设备无崩溃，也不能代替用户报告的真实异常。
