...
`auto=1`、`javascript=0`、`tests=13,20,27,43,44,56,58-77,80-222,999`；next262
采用定向门，不要求每批重复全量。设备 gate 通过 `-TestSelection` 只修改隔离 staging，
不改 tracked ini：

```bat
scripts\device_gate.bat -Candidate next262-programmatic-form-stage-final ^
  -TestSelection "229,999"
scripts\device_gate.bat -Candidate next262-programmatic-form-regression-retry ^
  -TestSelection "68-69,189-229,999"
```

next262 的两组最终定向门分别为 2/2 和 44/44；回归首尝 TEST193 的既有 JavaScript timeout
已用原配置重试通过。只有出现回归、设备环境变化或累计达到下一
个检查点时，才需要再次运行完整链。
...
2. 关闭设备上已有的 `test_host.exe`，在仓库根目录执行：
   ...
   ```bat
   scripts\stage.bat Debug C:\WMShare\Positron-manual-next262
   ```

3. 在设备 File Explorer 打开 `Storage Card\Positron-manual-next262`（或共享目录映射的
