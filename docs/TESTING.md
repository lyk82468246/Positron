...
（仅另有 TEST1201/1215 的两条有界 removal 路径）、
...
该门不实现通用节点插入、reparent、独立文本节点删除（TEST1214/1215 的窄路径除外）、MutationObserver、完整 live
...
与 detached 数据。自动断言还覆盖错误 parent、重复 detached 调用和 Comment 拒绝均不产生
部分 mutation；宿主只负责 callback 接线、fixture 与断言。通用 Node/DocumentFragment、
