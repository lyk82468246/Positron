...
`auto=1`、`javascript=0`、`tests=13,20,27,43,44,56,58-77,80-222,999`；next261
采用定向门，不要求每批重复全量。设备 gate 通过 `-TestSelection` 只修改隔离 staging，
不改 tracked ini：

```bat
scripts\device_gate.bat -Candidate next261-programmatic-stage ^
  -TestSelection "228,999"
scripts\device_gate.bat -Candidate next261-programmatic-regression ^
  -TestSelection "189-228,999"
```

next261 的两组定向门分别为 2/2 和 41/41；只有出现回归、设备环境变化或累计达到下一
个检查点时，才需要再次运行完整链。
...
TEST190-228 是自动 history/script-session/bootstrap/DOM-read/DOM-write/DOM-attribute/value/checked/form-property/navigation/location/event/input/key/focus/edit/select/click/form-event/invalid/file-input/checkbox-radio-input/change/label-click/toggle-key/programmatic-click 断言，不属于这次需要肉眼观察的包；TEST201
...
2. 关闭设备上已有的 `test_host.exe`，在仓库根目录执行：
   ...
   ```bat
   scripts\stage.bat Debug C:\WMShare\Positron-manual-next261
   ```

3. 在设备 File Explorer 打开 `Storage Card\Positron-manual-next261`（或共享目录映射的
