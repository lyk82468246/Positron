...
这是人工验收用的窄选择，不是完整自动回归基线。自动 next225 基线仍是
`auto=1`、`javascript=0`、`tests=13,20,27,43,44,56,58-77,80-192,999`；人工检查结束后，
下一次自动 gate 前必须恢复这三行。
...
TEST190-192 是自动 history-state 断言，不属于这次需要肉眼观察的包。
...
2. 关闭设备上已有的 `test_host.exe`，在仓库根目录执行：
   ...
   ```bat
   scripts\stage.bat Debug C:\WMShare\Positron-manual-next225
   ```

3. 在设备 File Explorer 打开 `Storage Card\Positron-manual-next225`（或共享目录映射的
