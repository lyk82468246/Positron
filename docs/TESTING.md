...
这是窄的自动 smoke 选择，不是完整自动回归基线。自动 next235 候选基线是
`auto=1`、`javascript=0`、`tests=13,20,27,43,44,56,58-77,80-202,999`，设备 gate
...
TEST190-202 是自动 history/script-session 断言，不属于这次需要肉眼观察的包；TEST201 直接调用
`positron_browser.dll` 公共 history API，TEST202 直接验证 product script session，避免只验证
test_host 私有适配。
...
2. 关闭设备上已有的 `test_host.exe`，在仓库根目录执行：
   ...
   ```bat
   scripts\stage.bat Debug C:\WMShare\Positron-manual-next235
   ```

3. 在设备 File Explorer 打开 `Storage Card\Positron-manual-next235`（或共享目录映射的
