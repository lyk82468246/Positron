...
| **5** | `positron_image.dll` — 可复用图片基础设施 | 🚧 retained 解码、SVG、独立消费及 PNG/JPEG 内存编码均已真机闭环；ABI 1.2 JPEG 质量控制待 next48 确认 |
...
最新设备反馈（2026-07-15）：next44 的 TEST13 全流程确认 next37 恢复点可作为 Browse 冻结基线；next45 又确认公共位图 ABI 的 TEST19 四格式、TEST20 缓存图片、TEST26/27 SVG 与 TEST13 全流程均正常。next46 的 ABI 1.0 独立 WM 示例也已在横竖屏确认，SVG 曲线缩放后的平滑观感略逊于先前大图但可接受。next47 已确认 ABI 1.1 的 PNG/JPEG 内存编码、DLL 配套释放及重新解码闭环；其中 PNG、源 BMP 与 retained SVG 正常，设备默认 JPEG 编码在 16x16 高饱和四象限样本上出现明显色度串扰。ABI 1.2 因此兼容追加 `PImage_EncodeBitmapEx`，允许用 WM Imaging 原生 `EncoderQuality` 指定 0..100，next48 示例使用 100 待真机复测；不重启 next38 之后的导航实验。
