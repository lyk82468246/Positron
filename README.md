...
最新设备反馈（2026-07-14）：TEST3 已确认 TLS 1.2 GET 的真实、单调正文进度，TEST43 已确认外链资源事务的显式 origin URL、去重、复制缓存命中和一次失败语义；TEST44-46 也已确认主文档失败回滚、CSS import tree、缓存跨断点重选和 stylesheet 元数据。随后 TEST13 暴露 `example.com` 的 Learn more 重定向后偶发长期不提交，加载期间高频拖动旧页滚动条还可能崩溃。根因批次已修正 HTTP 最终重定向 URL 未成为新文档 origin，并为 WinInet 每跳和宿主资源阶段加入有限等待；加载期间 thumb tracking 改为合并重绘。TEST48 离线覆盖 IANA 式 HTTPS→HTTP/www 最终 origin，待设备与 TEST13 一并验收。此前 TEST29-47 的基线仍保留，但不代表完整页面兼容性通过。
...
- **Engine**：TEST 6-11、15、16、18、21、22、24、25、38、40-48，HTML/CSS/DOM/select/style/layout/box tree/image resource cache、responsive media viewport、row-reverse flex padding、cached CSS restyle、SVG parse、受约束的 `:root` token、现代 CSS 值、grid 宽度隔离、overflow scrollbar、分阶段导航资源事务、主文档失败回滚、CSS import tree、stylesheet 元数据、文档 base URL 与重定向 origin，离线。TEST40-46 已真机通过，TEST47-48 待设备确认；TEST23 float 最小样例已因真实 Browse 回归撤回。
...
- **导航卡顿**：主文档、外链 CSS、CSS `@import`、`<img>` 与已计算 CSS 背景资源的 GET 已组成分阶段 worker 事务，旧页在网络等待时可滚动。`PHttp_GetEx/PostEx` 在请求线程报告已解码正文大小和可选 `Content-Length`；父窗口进度条对已知总长显示当前响应的真实百分比，对 chunked/无长度响应保持活动动画。HTTP 响应现保留最终重定向 URL，新文档以它作为 CSS/图片/后续链接 origin；明文 WinInet 每跳连接/接收等待为 15 秒，宿主可选资源阶段总预算 20 秒，超出后保留 fallback 并提交页面。加载期间滚动 thumb 不再逐消息同步强制 NetSurf/GDI 重绘，而由 WM 合并失效区。HTML parse、style、图片 cache copy、layout 仍在 UI 线程分四个 timer 阶段，单个调用仍可能短暂卡顿。TLS socket 尚无强制取消/完整 deadline，整页聚合进度、字体和脚本也未完成。`test_host` 暂存最多 64 个 URL、合计 2 MiB 原始字节；这是可替换的宿主预算，不是 `positron_core` ABI 限制。
