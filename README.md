...
**最新门禁（next138，2026-08-07）**：next134 在 `screen=240x320 dpi=96`
设备上已确认 TEST13/20/27/43/44/56/58-77/80-99 全部通过。next137 在
`screen=320x320 dpi=128` 下确认 TEST13/20/27/43/44/56/58/59 通过，TEST60
暴露出离线 CSS 几何夹具错误继承运行时 DPI；next138 只隔离 TEST60 的 CSS 96-DPI
参考上下文，并在结束时恢复真实设备视口。ARMV4I Debug 增量构建与 staging 已通过，
设备复测待进行。96 DPI 只是离线 CSS 参考，不是产品固定 DPI；仍需轮换分辨率/DPI，
并人工复查 TEST13。

> **当前构建候选（2026-08-07）**：next138 保留 next137 的非整数 DPI 设备像素换算，
> 并隔离 TEST60 的显式 CSS 几何上下文；没有放宽 TEST20/60 断言，也不固定分辨率或
> 产品 DPI。next138 需要在 `screen=320x320 dpi=128` 及另一组分辨率/DPI 上运行，
> 重点确认 TEST60-77、TEST100-104 与 TEST13；浏览器核心仍不执行脚本，float 候选
