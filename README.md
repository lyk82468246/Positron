...
**最新门禁（next134，2026-08-07）**：next132 在 `480x640 dpi=192` 设备上
TEST13/20/27/43/44/56 通过，但 TEST58 因离线几何段继承前一个设备视口待布局状态
而停止；next133 的固定视口隔离方案已撤销。next134 让 TEST58 直接使用运行时屏幕/DPI，
并按 CSS 96 DPI 规范基准换算物理几何断言；VS2008 ARMV4I Debug 增量构建与 staging
已通过，等待设备复测。不能用自动 `OK` 或固定设备 DPI 替代高 DPI 验收。
