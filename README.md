...
| **4** | `positron_core.dll` — NetSurf 内核移植（HTML/CSS 渲染层）；HTTP keep-alive / 重定向 / gzip 等 | 🚧 进行中：5 库编译 + 解析引擎（HTML→DOM + CSS）真机验证；`positron_core.dll` 边界已立；布局/绘制后续 |
