...
> **当前构建候选（2026-08-06）**：next122 在已验收的 next121/next114 Browse 基线上为独立 `positron_script.dll` 增加宿主模块源码 provider 与 TEST84；ARMV4I Debug/Release 构建和 staging 已通过，设备验收待用户运行。next121 仍是最后一个已验证脚本基线，配置的 TEST13/20/27/43/44/56/58-77/80-83 已为 `TESTBENCH PASS`。浏览器核心仍不执行脚本。`TrackMouseEvent` 等桌面 API 不属于 WM6 方案；宿主使用 `WM_MOUSEMOVE` 与定时器轮询离开窗口。next115 与 next116 的 float 候选均已撤回：设备 TEST79 失败，next116 的 TEST13 截图仍出现导航/正文排版回归；next117/next114 保持为当前可靠 Browse 基线。
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI、TEST80-83 安全边界与模块生命周期已由 ARMV4I 设备 testbench 验收；next122/TEST84 宿主源码 provider 待设备验收；浏览器 DOM/网络绑定保持关闭 |
...
- **Standalone script**：TEST 80-84，独立 positron_script.dll 的 ABI、持久上下文、错误恢复、DLL 内存遥测、执行超时/源码长度边界、硬内存配额、CommonJS 风格模块生命周期和宿主源码 provider；不初始化 positron_core，不连接 DOM/window/network。TEST80-83 已由 next121 设备 testbench 验收，TEST84 属于 next122 待验收项。
