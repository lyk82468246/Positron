...
状态更正：上段“动态状态伪类仍不完整”截至 next106 的描述已被 next109/TEST73 部分取代；目前 `:focus/:active/:checked/:enabled/:disabled` 已进入正式重样式路径。仍未完成的是 hover/visited/target/indeterminate、DOM 事件对象与传播/取消/default-action，以及无 CSS 尺寸空 text input 的默认 intrinsic size。
...
- TEST 73：验证 live checked/enabled/disabled、宿主 focus/active、cache-only 重样式、纵横屏保持与 reset；不代表 DOM 事件传播已经实现。
