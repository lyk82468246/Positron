...
| **5** | `positron_image.dll` — 可复用图片基础设施 | 🚧 retained 解码、SVG、独立消费及 PNG/JPEG 内存编码均已真机闭环；next49 已确认 libjpeg-turbo 4:4:4，ABI 1.3 原始像素入口待 next50 验收 |
...
最新设备反馈（2026-07-15）：next44 的 TEST13 全流程确认 next37 恢复点可作为 Browse 冻结基线；next45 又确认公共位图 ABI 的 TEST19 四格式、TEST20 缓存图片、TEST26/27 SVG 与 TEST13 全流程均正常。next46 的 ABI 1.0 独立 WM 示例也已在横竖屏确认，SVG 曲线缩放后的平滑观感略逊于先前大图但可接受。next47 已确认 ABI 1.1 的 PNG/JPEG 内存编码、DLL 配套释放及重新解码闭环；其中 PNG、源 BMP 与 retained SVG 正常，设备默认 JPEG 编码在 16x16 高饱和四象限样本上出现明显色度串扰。next48 又确认 WM `EncoderQuality=100` 只有有限改善，横纵色带仍明显，说明系统 codec 没有提供可用的采样控制。next49 已确认静态 libjpeg-turbo 1.5.3 的显式 quality JPEG 为正确 4:4:4，行方向、红绿蓝黄颜色与 PNG 一致，大面积色带消失；旧 WM 默认入口继续保留。Debug DLL 相比 next48 增加 243,712 字节（约 238 KiB），设备不需额外 JPEG DLL；额外 CPU 与约 `width*height*3` 的主要中间像素内存只发生在显式 JPEG 编码。当前 ABI 1.3 继续增加复制式 BGR24/BGRA32 原始像素入口，next50 集中验收。不重启 next38 之后的导航实验。
