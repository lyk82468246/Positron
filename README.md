...
**当前构建候选（next144，2026-08-08）**：新增默认关闭的 `javascript=0/1` 浏览器
...
ARMV4I 增量构建已通过，设备验收待补；默认 TEST13 不扫描、抓取或执行 JavaScript。
...
> 全部通过，下一批应继续轮换分辨率/DPI，并人工复查 TEST13；next144 候选只在显式
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI、模块/provider、global/JSON、native callback 与 structured JSON setter 已完成构建；next134 日志中的 TEST80-99 已通过。next144 候选增加默认关闭的最小浏览器 DOM text bridge，外部脚本/网络/事件绑定仍未开放 |
