...
**最新门禁（next139，2026-08-07）**：next134 在 `screen=240x320 dpi=96`
设备上已确认 TEST13/20/27/43/44/56/58-77/80-99 全部通过。next137 在
`screen=320x320 dpi=128` 下确认 TEST13/20/27/43/44/56/58/59/60 通过，TEST63
随后暴露出另一个离线 SVG 夹具错误继承运行时 DPI；next138 隔离 TEST60，next139
隔离 TEST63 并补充具体布局诊断。ARMV4I Debug 增量构建与 staging 已通过，设备复测
待进行。96 DPI 只是离线 CSS 参考，不是产品固定 DPI；仍需轮换分辨率/DPI，并人工
复查 TEST13。

> **当前构建候选（2026-08-07）**：next139 保留 next137 的非整数 DPI 设备像素换算，
> 并隔离 TEST60/63 的显式 CSS 几何上下文；没有放宽 TEST20/60/63 断言，也不固定
> 分辨率或产品 DPI。next139 需要在 `screen=320x320 dpi=128` 及另一组分辨率/DPI
> 上运行，重点确认 TEST63-77、TEST100-104 与 TEST13；浏览器核心仍不执行脚本，float 候选
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI、模块/provider、global/JSON、native callback 与 structured JSON setter 已完成构建；next134 日志中的 TEST80-99 已通过，浏览器 DOM/网络绑定仍保持关闭 |
...
独立脚本边界补充：`positron_script.dll` 已提供可供其他 WM 程序调用的 Duktape 2.7.0 UTF-8 求值服务，TEST80-99 已在 next134 设备日志通过；模块/provider、global/JSON、native callback 与 structured setter 均保持独立 ABI，JSON 结果超过 DLL 的 255 字节有效载荷会显式失败而不截断。这不改变浏览器 JavaScript 默认关闭的策略，也不提供 DOM/window/fetch/network binding。
