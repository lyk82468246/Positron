...
| `positron_browser.dll` | 浏览器 session、history、脚本 session/bootstrap 和 DOM 读写/attribute/value/checked/form-property/event 回调组合层 | 不拥有窗口、网络；其余 form/input、location/navigation bridge 仍在迁移 |
...
form property（defaultValue/defaultChecked/selectedIndex）和 event 回调分发已迁入；其余 form/input
以及 location/navigation 适配仍在迁移中。
它不是第二套引擎。
两者的关系和所有权见
