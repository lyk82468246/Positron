...
| `positron_browser.dll` | 浏览器 session、history、脚本 session/bootstrap 和 DOM 读写/attribute/value/checked/form-property/navigation/location/event/native-input/key/focus/form-event 回调组合层 | 不拥有窗口、网络；其余 form/input 适配、core 事件传播及控件副作用仍由宿主提供 |
...
EDIT change/post-change input、click、submit/reset 和 SELECT input/change typed dispatch entry 已迁入；其余 form/input 适配、core 事件传播以及窗口、网络、控件和
