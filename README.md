...
> **当前构建候选（2026-08-04）**：next119 在 next118/next114 Browse 基线上为独立 `positron_script.dll` 增加 TEST81 安全边界断言，复用仓库内 NetSurf Duktape 2.7.0 单文件源，覆盖执行超时、64 KiB 源码长度拒绝和超时/拒绝后的上下文恢复。VS2008 的 Windows Mobile 6 Professional SDK（ARMV4I）Debug 增量构建与 staging 已通过；next118 的设备日志已确认 TEST80 与此前默认门禁最终 `TESTBENCH PASS`，next119 的 TEST81 等待设备日志。默认 testbench 为 TEST13/20/27/43/44/56/58-77/80-81；浏览器核心仍不执行脚本。`TrackMouseEvent` 等桌面 API 不属于 WM6 方案；宿主使用 `WM_MOUSEMOVE` 与定时器轮询离开窗口。next115 与 next116 的 float 候选均已撤回：设备 TEST79 失败，next116 的 TEST13 截图仍出现导航/正文排版回归；next117/next114 保持为当前可靠 Browse 基线。
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI 与 TEST80 已由 ARMV4I 设备 testbench 验收；TEST81 的超时/源码边界待设备确认；浏览器 DOM/网络绑定保持关闭 |
...
2026-08-03 的 next114/TEST77 建立了独立的脚本资源 ABI：core 扫描外部 `<script src>`，可通过 `PCoreResolveUrlFn` 使用宿主 URL 策略，调用 transport-agnostic fetch/free 回调，把成功字节按 document 生命周期缓存，并提供只读计数/枚举接口。该批不执行 inline 或 external JavaScript，也尚未接入 TEST13 的网络事务；ARMV4I 增量构建通过，TEST77 与整批设备自动 testbench 已确认 PASS。2026-08-04 的 next118 又把仓库已有 Duktape 2.7.0 封装成独立 `positron_script.dll`，TEST80 覆盖 ABI、持久求值、throw 后恢复和 DLL 内存遥测；设备日志确认 TEST13 三段导航、TEST20/27、TEST43/44、TEST56、TEST58-77 与 TEST80 全部通过，最终为 `TESTBENCH PASS`。next119 新增 TEST81，覆盖执行超时、源码长度上限和上下文恢复；本地 ARMV4I 构建已通过，设备验收待运行日志。
...
独立脚本边界补充：`positron_script.dll` 已提供可供其他 WM 程序调用的 Duktape 2.7.0 UTF-8 求值服务，TEST80 已在设备通过；TEST81 的 timeout、64 KiB 源码上限和恢复断言已编译进 next119，尚待设备确认；这不改变浏览器 JavaScript 默认关闭的策略。
...
auto=1
tests=13,20,27,43,44,56,58-77,80-81
```
...
- **Standalone script**：TEST 80-81，独立 positron_script.dll 的 ABI、持久上下文、错误恢复、DLL 内存遥测，以及执行超时和源码长度边界；不初始化 positron_core，不连接 DOM/window/network。TEST80 已由设备 testbench 验收，TEST81 随 next119 待设备确认。
