...
| `positron_browser.dll` | 浏览器 session、history、脚本 session 和同源 state 组合层 | 不拥有窗口、网络；DOM/bootstrap bridge 仍在迁移 |
...
组合。目前 history/session 和浏览器脚本 context 的所有权、host JSON callback 注册已经迁入；
bootstrap 文本、DOM/Event/form/location 适配仍在迁移中。它不是第二套引擎。
两者的关系和所有权见
