...
> **当前构建候选（2026-08-05）**：next120 在已验收的 next119/next114 Browse 基线上为独立 `positron_script.dll` 增加 TEST82 硬内存配额断言，覆盖 512 KiB runtime heap 上限、峰值不越界和超限后的求值恢复。VS2008 的 Windows Mobile 6 Professional SDK（ARMV4I）Debug 增量构建、staging 与设备验收均已通过；next120 默认 testbench 为 TEST13/20/27/43/44/56/58-77/80-82，日志最终为 `TESTBENCH PASS`。浏览器核心仍不执行脚本。`TrackMouseEvent` 等桌面 API 不属于 WM6 方案；宿主使用 `WM_MOUSEMOVE` 与定时器轮询离开窗口。next115 与 next116 的 float 候选均已撤回：设备 TEST79 失败，next116 的 TEST13 截图仍出现导航/正文排版回归；next117/next114 保持为当前可靠 Browse 基线。
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI、TEST80/81 安全边界与 TEST82 硬内存配额均已由 ARMV4I 设备 testbench 验收；浏览器 DOM/网络绑定保持关闭 |
...
独立脚本边界补充：`positron_script.dll` 已提供可供其他 WM 程序调用的 Duktape 2.7.0 UTF-8 求值服务，TEST80-82 已在设备通过；TEST82 日志为 `rc=-6 used=70057 peak=496184 limit=524288 recovery=42 eval=2/2`；这不改变浏览器 JavaScript 默认关闭的策略。
...
- **Standalone script**：TEST 80-82，独立 positron_script.dll 的 ABI、持久上下文、错误恢复、DLL 内存遥测、执行超时/源码长度边界和硬内存配额；不初始化 positron_core，不连接 DOM/window/network，已由 next120 设备 testbench 验收。
