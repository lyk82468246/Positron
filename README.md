...
**当前自动化设备基线（next200，2026-08-12）**：同一个 Duktape 页面 context 已具备最小
...
TEST20/27/43/44/56/58-77/80-168 全部通过：配置所选 115 项全部 OK、零条 `[ERROR]`、零 FAIL、
最终 `TESTBENCH PASS`，日志为 `C:\WMShare\Positron-next200\test_host.log`；TEST13 使用
`OK (overview)`，其余 114 项使用标准数字 OK 行。最近一次定向人工门
仍是 next167：用户确认
...
auto=1
tests=13,20,27,43,44,56,58-77,80-168
```
