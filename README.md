...
> **当前构建候选（2026-08-05）**：next120 在已验收的 next119/next114 Browse 基线上为独立 `positron_script.dll` 增加 TEST82 硬内存配额断言，覆盖 512 KiB runtime heap 上限、峰值不越界和超限后的求值恢复。VS2008 的 Windows Mobile 6 Professional SDK（ARMV4I）Debug 增量构建与 staging 已通过；next119 默认 testbench 已由设备确认 `TESTBENCH PASS`，next120 的 TEST82 等待设备日志。默认 testbench 为 TEST13/20/27/43/44/56/58-77/80-82；浏览器核心仍不执行脚本。`TrackMouseEvent` 等桌面 API 不属于 WM6 方案；宿主使用 `WM_MOUSEMOVE` 与定时器轮询离开窗口。next115 与 next116 的 float 候选均已撤回：设备 TEST79 失败，next116 的 TEST13 截图仍出现导航/正文排版回归；next117/next114 保持为当前可靠 Browse 基线。
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI、TEST80/81 安全边界已由 ARMV4I 设备 testbench 验收；TEST82 硬内存配额待设备确认；浏览器 DOM/网络绑定保持关闭 |
...
独立脚本边界补充：`positron_script.dll` 已提供可供其他 WM 程序调用的 Duktape 2.7.0 UTF-8 求值服务，TEST80/81 已在设备通过；TEST82 增加 512 KiB runtime heap 配额和峰值遥测，已编译进 next120，尚待设备确认；这不改变浏览器 JavaScript 默认关闭的策略。
...
auto=1
tests=13,20,27,43,44,56,58-77,80-82
```
...
- **Standalone script**：TEST 80-82，独立 positron_script.dll 的 ABI、持久上下文、错误恢复、DLL 内存遥测、执行超时/源码长度边界和硬内存配额；不初始化 positron_core，不连接 DOM/window/network。TEST80/81 已由 next119 设备 testbench 验收，TEST82 随 next120 待设备确认。
