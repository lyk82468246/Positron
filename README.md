...
| `positron_browser.dll` | history/session 与每项 viewport snapshot、脚本 bootstrap/scroll 与有限布局几何同步、DOM/Event、候选导航生命周期与结果摘要、资源/候选提交及清理快照、平台回调协调 | `PBrowser_HistoryCreate`、`PBrowser_HistoryEntryScroll`、`PBrowser_HistorySetEntryScroll`、`PBrowser_ScriptSessionCreate`、`PBrowser_ScriptSessionNotifyScroll`、脚本 `Element.getBoundingClientRect()`、`PBrowser_NavigationCandidateCreate`、`PBrowser_NavigationCandidateGetResult`、`PBrowser_NavigationCommitGetInfo`、`PBrowser_NavigationCleanupGetInfo` |
