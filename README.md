...
让 18 年前的 WM6 设备能跑现代 TLS/HTTP/JSON，并逐步具备自带浏览器内核能力。当前主线已经进入 HTML/CSS 真实渲染：NetSurf 3.11 的解析、样式、layout/redraw、GDI 绘制和点击导航都在 `positron_core.dll` 后面跑通；JavaScript 是长期必须实现的目标，但尚不是当前可用能力。
...
| **4** | `positron_core.dll` — NetSurf 内核移植（HTML/CSS 渲染层） | 🚧 M5f/M7 后：正式 Browse 路径已走 NetSurf `layout.c/redraw.c`；flex、table、border 与 CSS attribute/sibling/static-pseudo selector 已由 TEST 9/17 真机验证；图片 fetch 骨架待验证 |
...
当前 Browse 正式路径已经从早期手写块流布局切到 **NetSurf 真实布局/重绘引擎**：`PCore_LayoutDocument` / `PCore_PaintDocument` / `PCore_LinkAt` 走 `pcore_box_construct` → NetSurf `layout_document` → `html_redraw` → GDI plotter。M7-flex 已接入 `layout_flex.c`，TEST 17 三色块横排；M7-table 已接入 `table.c` + `BOX_TABLE > ROW_GROUP > ROW > CELL` 构建，TEST 17 2×2 表格网格真机验证。M5f 已把 NetSurf `redraw_border.c` 加入工程并移除 border no-op stub；2026-07-08 根据真实 VS2008 编译日志补齐前置 include，2026-07-10 已成功复编且 TEST 17 边框真机通过。CSS attribute/sibling selector 与 `:link` / `:lang()` 已由 TEST 9 真机验证。`<img>` alt fallback 已由 TEST 17 验证；`PCore_FetchImageResources` 已接入 `<img src>` 资源发现/fetch 骨架并扩展 TEST 18。TEST 11 也已从旧手写布局器的 `body.y=8` / `p.y=24` 修正为 NetSurf margin collapse 的 `body.y=p.y=16`，待复编回归。真实位图/SVG 解码仍待后续。详见 [PHASE4.md](PHASE4.md) 和 [.agents/ROADMAP.md](.agents/ROADMAP.md)。

当前可用能力：TLS/HTTP/JSON 通信栈；HTML/CSS/DOM 解析；CSS select + computed style；整树样式；外链 CSS；NetSurf real layout/redraw；GDI plotter；滚动、viewport/DPI 自适应、点击链接导航；flex、常见 table、border、CSS attribute/sibling/static-pseudo selector 与 `<img>` alt fallback。`<img src>` fetch 骨架已源码接入，等待 TEST 18 真机验证。

当前明确缺口：真实图片/SVG 未显示（`<img>` 只有 alt/src 文本占位，fetch 到的字节尚不缓存/解码/绘制）；CSS 动态状态伪类（如 `:hover` / `:visited`）仍为 no-match；float、rowspan 精确跨行、border-collapse、forms/widgets 未完整；网络与导航仍同步阻塞；JavaScript 尚未实现但属于长期必做目标。
...
- **Engine**：TEST 6-11、15、16、18，HTML/CSS/DOM/select/style/layout/box tree/image resource scan，离线。
...
- **渲染限制**：真实 NetSurf layout/redraw、border、CSS attribute/sibling/static-pseudo selector 与 `<img>` alt fallback 已真机验证；`<img src>` fetch 骨架仍待 TEST 18 验证。真实图片/SVG 解码绘制、CSS 动态状态伪类、float、rowspan 精确跨行、border-collapse、forms/widgets 仍待补。JavaScript 是长期必做项，当前尚未实现。
