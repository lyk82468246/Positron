...
| `positron_browser.dll` | 浏览器 session、history、脚本 session/bootstrap 和 DOM 只读回调组合层 | 不拥有窗口、网络；DOM 写入、Event/form/input/location/navigation bridge 仍在迁移 |
...
bootstrap 和 DOM 只读（按 id 查询与 textContent 读取）回调分发已迁入；DOM 写入、Event/form/
input/location/navigation 适配仍在迁移中。它不是第二套引擎。
两者的关系和所有权见
