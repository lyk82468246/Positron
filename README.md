...
> **当前构建候选（2026-08-04）**：next118 在 next117/next114 Browse 基线上新增独立 `positron_script.dll`，复用仓库内 NetSurf Duktape 2.7.0 单文件源，提供 opaque-handle、UTF-8 求值、持久上下文、错误恢复和内存/次数遥测 ABI；新增 TEST80。VS2008 的 Windows Mobile 6 Professional SDK（ARMV4I）Debug 增量构建已通过，设备日志确认 TEST80 与默认选定门禁最终 `TESTBENCH PASS`。默认 testbench 为 TEST13/20/27/43/44/56/58-77/80；浏览器核心仍不执行脚本。`TrackMouseEvent` 等桌面 API 不属于 WM6 方案；宿主使用 `WM_MOUSEMOVE` 与定时器轮询离开窗口。next115 与 next116 的 float 候选均已撤回：设备 TEST79 失败，next116 的 TEST13 截图仍出现导航/正文排版回归；next117/next114 保持为当前可靠 Browse 基线。
...
| **6** | `positron_script.dll` — 独立 JavaScript 执行基础设施 | ✅ Duktape 2.7.0 稳定 C ABI 与 TEST80 已由 ARMV4I 设备 testbench 验收；浏览器 DOM/网络绑定保持关闭 |
...
独立脚本边界补充：`positron_script.dll` 已提供可供其他 WM 程序调用的 Duktape 2.7.0 UTF-8 求值服务，TEST80 已在设备通过；这不改变浏览器 JavaScript 默认关闭的策略。
...
auto=1
tests=13,20,27,43,44,56,58-77,80
```
...
- **Standalone script**：TEST 80，独立 positron_script.dll 的 ABI、持久上下文、错误恢复和 DLL 内存遥测；不初始化 positron_core，不连接 DOM/window/network，已由设备 testbench 验收。
