...
disabled、表单属性（含 `name`/`action`/`method`/`enctype`）与约束属性（含 `pattern`/`minLength`/`maxLength` 反射）、控件与受限 form-level `checkValidity()`/`reportValidity()`/`willValidate`/`validity` 查询、
