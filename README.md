...
最新设备反馈（2026-07-14）：TEST13 最后一次确认完整跑通的精确基线是 `9c5c7c7`。其后的 stylesheet metadata、首个 `<base>`、图片/链接绝对化和 redirect origin 能力各自测试正常，但组合进真实 Browse 后，`Open example` -> `Learn more` 会一分钟以上不提交，加载时反复滚动还可能崩溃。20 秒资源预算、滚动重绘合并和 TLS deadline 尝试均未解决设备问题，现已撤回；Browse 的初次样式、旋转 restyle、图片扫描、链接命中和 document origin 显式恢复到 9c 行为。现代 Ex2 API、`effective_url` 与 TEST46-48 仍保留，后续每次只重新接入一项并跑完整 TEST13。
...
读取到有效配置后，程序先提示是否只运行列出的 TEST；选 **Yes** 直接按编号升序执行，选 **No** 完整保留下面原有的 All/四组选择流程。文件缺失时直接走旧流程；文件存在但无效时提示并忽略。TEST23 已撤回，配置中出现 23 会被拒绝。`scripts\stage.bat` 会复制仓库内默认的 `test_host\test_host.ini`，当前批次运行 TEST13、46；可在共享目录直接修改后重新启动。第 3 组 GDI Render 提示已改为能力类别和编号范围，避免 WM6 小屏 MessageBox 被逐项说明撑坏。
...
- **导航卡顿**：主文档、外链 CSS、CSS `@import`、`<img>` 与已计算 CSS 背景资源的 GET 已组成分阶段 worker 事务，旧页在网络等待时可滚动。`PHttp_GetEx/PostEx` 在请求线程报告已解码正文大小和可选 `Content-Length`；父窗口进度条对已知总长显示当前响应的真实百分比，对 chunked/无长度响应保持活动动画。HTML parse、style、图片 cache copy、layout 仍在 UI 线程分四个 timer 阶段，单个调用仍可能短暂卡顿。HTTP DLL 会保留最终重定向 URL，但 Browse 暂不采用它；失败的 20 秒资源预算和 TLS deadline 尝试已经撤销，TLS socket 仍无强制取消/完整 deadline。`test_host` 暂存最多 64 个 URL、合计 2 MiB 原始字节；这是可替换的宿主预算，不是 `positron_core` ABI 限制。
