...
| `positron_browser.dll` | 浏览器 session、history、脚本 session/bootstrap 和 DOM 读写/attribute/value/checked/form-property/navigation/location/event 回调组合层 | 不拥有窗口、网络；其余 form/input 适配及导航副作用仍由宿主提供 |
...
location/history 事件分发和 event 回调分发已迁入；其余 form/input 适配以及窗口、网络、
