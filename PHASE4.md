...
- **table span 仍是分阶段能力**：有效表格的 `colspan`、有限/自动 `rowspan` 和 row-group 边界已移植 NetSurf 3.11 占位算法，TEST46 含几何、12 点像素及可见 redraw 验收；畸形表格的空单元格生成和完整 collapsed-border 冲突规则仍未实现。
