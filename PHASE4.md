...
- **图片路径正在扩展格式验收**：缓存命中且可由 WM Imaging 解码的 `<img>` 会生成 `box->object`，并走 `content_redraw -> plot_bitmap -> IImage::Draw`。BMP/PNG/GIF 可见绘制及 TEST20 author-sheet 保留已确认；JPEG 已换 16x16 4:4:4 样本待复测。背景图、SVG 尚未接通，解码失败时仍回退 alt/src 文本。
