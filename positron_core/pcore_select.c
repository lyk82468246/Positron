        ...
        pcore_html_serialize_state *state, unsigned int depth)
{
    ...
    if (result == 0 && !pcore_html_serialize_void_name(tag_name)) {
        ...
        while (result == 0 && child != NULL) {
            child_count++;
            if (child_count > PCORE_HTML_SERIALIZE_MAX_CHILDREN) {
                result = 1;
                dom_node_unref(child);
                child = NULL;
                break;
            ...
            next_child = NULL;
            if (pcore_html_serialize_node(child, state, depth + 1) != 0 ||
                    dom_node_get_next_sibling(child, &next_child) !=
                    DOM_NO_ERR) {
                result = 1;
            }
            dom_node_unref(child);
        ...
        char *value, int value_capacity, int *out_bytes)
{
    ...
    if (outer) {
        ...
        child = NULL;
        if (dom_node_get_first_child((dom_node *) element, &child) !=
                DOM_NO_ERR) {
            result = 1;
        }
        child_count = 0;
        while (result == 0 && child != NULL) {
            ...
            next_child = NULL;
            if (pcore_html_serialize_node(child, &state, 1) != 0 ||
                    dom_node_get_next_sibling(child, &next_child) !=
                    DOM_NO_ERR) {
                result = 1;
            }
        ...
        int *out_number)
{
    ...
    *out_number = 0;
    ...
    if (relation == PCORE_NODE_RELATION_LAYOUT_SCROLLABLE_X ||
            relation == PCORE_NODE_RELATION_LAYOUT_SCROLLABLE_Y) {
        if (pcore_box_overflow_axis_available(doc, (dom_node *) element,
                relation == PCORE_NODE_RELATION_LAYOUT_SCROLLABLE_X ? 0 : 1,
                out_number) != 0) {
            return 2;
        }
        return 0;
    }
    if (relation == PCORE_NODE_RELATION_LAYOUT_CLIENT_X ||
            relation == PCORE_NODE_RELATION_LAYOUT_CLIENT_Y) {
        if (pcore_box_layout_client_origin_for_node(doc,
                (dom_node *) element, &x, &y) != 0) {
            return 2;
        }
        *out_number = relation == PCORE_NODE_RELATION_LAYOUT_CLIENT_X ?
                x : y;
        return 0;
    }
        ...
        int value_capacity, int *out_bytes, int *out_number)
{
    ...
    case PCORE_NODE_RELATION_LAYOUT_SCROLLABLE_X:
    case PCORE_NODE_RELATION_LAYOUT_SCROLLABLE_Y:
    case PCORE_NODE_RELATION_LAYOUT_CLIENT_X:
    case PCORE_NODE_RELATION_LAYOUT_CLIENT_Y:
        err = pcore_relation_layout_rect((dom_document *) hDoc, element,
                relation, index, out_number);
        break;
    default:
        ...
        char *text, int text_capacity, int *out_bytes)
{
    ...
    content = NULL;
    err = dom_node_get_text_content((dom_node *) element, &content);
    if (err != DOM_NO_ERR) {
        dom_node_unref((dom_node *) element);
        return 1;
    }
...
/* ------------------------------------------------------------------ */
/* bounded parser-backed Element.innerHTML mutation                    */
...
} pcore_html_mutation_scan;

static void pcore_html_mutation_scan_release(pcore_html_mutation_scan *scan)
{
    unsigned int i;
    ...
    for (i = 0; i < scan->id_count; i++) {
        if (scan->ids[i] != NULL) {
            dom_string_unref(scan->ids[i]);
            scan->ids[i] = NULL;
        }
    }
    scan->id_count = 0;
...
/* Test a retained element against an ancestor without borrowing any parent
 * pointer across the loop.  dom_node_get_parent_node returns a new reference
 * on every successful step, which keeps this safe on libdom's pending-list
 * implementation when the old subtree is detached later. */
static int pcore_html_mutation_is_inside(dom_node *node, dom_node *ancestor)
{
    dom_node *cursor;
    dom_node *parent;
    int inside;
...
static int pcore_html_mutation_scan_id(pcore_html_mutation_scan *scan,
        dom_string *id_value)
{
    dom_element *existing;
    const char *id_data;
    unsigned int i;

    if (scan == NULL || id_value == NULL) {
        return 1;
    }
    id_data = dom_string_data(id_value);
    if (id_data == NULL || id_data[0] == '\0') {
        return 0;
    }
    for (i = 0; i < scan->id_count; i++) {
        if (dom_string_isequal(scan->ids[i], id_value)) {
            return 3;
        }
    }
    ...
    if (scan->id_count >= PCORE_NODE_HTML_MUTATION_MAX_NODES) {
        return 3;
    }
    scan->ids[scan->id_count] = dom_string_ref(id_value);
    scan->id_count++;
    return 0;
}

static int pcore_html_mutation_scan_node(pcore_html_mutation_scan *scan,
        dom_node *node, unsigned int depth)
{
    ...
    if (scan == NULL || node == NULL) {
        return 1;
    }
    if (depth == 0 || depth > PCORE_NODE_HTML_MUTATION_MAX_DEPTH) {
        return 3;
    }
    if (scan->node_count >= PCORE_NODE_HTML_MUTATION_MAX_NODES) {
        return 3;
    }
    ...
    id_value = NULL;
    err = dom_element_get_attribute((dom_element *) node, scan->id_name,
            &id_value);
    if (err != DOM_NO_ERR) {
        if (id_value != NULL) {
    ...
    if (id_value != NULL) {
        result = pcore_html_mutation_scan_id(scan, id_value);
        dom_string_unref(id_value);
        if (result != 0) {
            return result;
        }
    ...
    while (child != NULL) {
        child_count++;
        if (child_count > PCORE_NODE_HTML_MUTATION_MAX_CHILDREN) {
            dom_node_unref(child);
            return 3;
        }
        result = pcore_html_mutation_scan_node(scan, child, depth + 1);
        if (result != 0) {
            dom_node_unref(child);
            return result;
        }
...
static int pcore_html_mutation_scan_fragment(pcore_html_mutation_scan *scan,
        dom_document_fragment *fragment)
{
    ...
    if (scan == NULL || fragment == NULL) {
        return 1;
    }
    child = NULL;
    if (dom_node_get_first_child((dom_node *) fragment, &child) !=
            DOM_NO_ERR) {
        return 1;
    }
    child_count = 0;
    while (child != NULL) {
        ...
        result = pcore_html_mutation_scan_node(scan, child, 1);
        if (result != 0) {
            dom_node_unref(child);
            return result;
        }
        ...
        const char *html, size_t length, dom_document_fragment **out_fragment)
{
    ...
    dom_document_fragment *fragment;
    dom_document_fragment *normalized;
    dom_node *top;
    dom_node *body;
    dom_node *child;
    dom_node *next;
    ...
    if (doc == NULL || html == NULL || out_fragment == NULL) {
        return 1;
    }
    ...
    parser = NULL;
    fragment = NULL;
    derr = dom_hubbub_fragment_parser_create(&params, doc, &parser,
            &fragment);
    if (derr != DOM_HUBBUB_OK || parser == NULL || fragment == NULL) {
        if (fragment != NULL) {
    ...
    top = NULL;
    if (dom_node_get_first_child((dom_node *) fragment, &top) != DOM_NO_ERR) {
        dom_node_unref((dom_node *) fragment);
        return 1;
    }
    if (top == NULL || dom_node_get_node_type(top, &type) != DOM_NO_ERR ||
            type != DOM_ELEMENT_NODE ||
            !pcore_element_name_is((dom_element *) top, "html")) {
        if (top != NULL) {
            dom_node_unref(top);
        }
    ...
    body = NULL;
    child = NULL;
    if (dom_node_get_first_child(top, &child) != DOM_NO_ERR) {
        dom_node_unref(top);
        dom_node_unref((dom_node *) fragment);
        return 1;
    }
    while (child != NULL) {
        ...
        if (type == DOM_ELEMENT_NODE &&
                pcore_element_name_is((dom_element *) child, "body")) {
            body = child;
            break;
        }
        next = NULL;
    ...
    normalized = NULL;
    err = dom_document_create_document_fragment(doc, &normalized);
    if (err != DOM_NO_ERR || normalized == NULL) {
    ...
    child = NULL;
    if (dom_node_get_first_child(body, &child) != DOM_NO_ERR) {
        dom_node_unref((dom_node *) normalized);
        dom_node_unref((dom_node *) body);
        dom_node_unref(top);
        dom_node_unref((dom_node *) fragment);
        return 1;
    ...
    while (child != NULL) {
        ...
        removed = NULL;
        err = dom_node_remove_child(body, child, &removed);
        dom_node_unref(child);
        if (err != DOM_NO_ERR || removed == NULL) {
            if (removed != NULL) {
        ...
        inserted = NULL;
        err = dom_node_append_child((dom_node *) normalized, removed,
                &inserted);
        if (inserted != NULL) {
            dom_node_unref(inserted);
        }
    ...
    dom_node_unref(top);
    dom_node_unref((dom_node *) fragment);
    *out_fragment = normalized;
    return 0;
}

/* Move the old children into a same-document fragment.  The fragment keeps
 * their original order and gives the caller a transaction rollback point. */
static int pcore_html_mutation_stash_children(dom_element *target,
        dom_document_fragment *stash)
{
    ...
    if (target == NULL || stash == NULL) {
        return 1;
    }
    for (;;) {
        child = NULL;
        if (dom_node_get_first_child((dom_node *) target, &child) !=
                DOM_NO_ERR) {
            return 1;
        }
        if (child == NULL) {
        ...
        removed = NULL;
        err = dom_node_remove_child((dom_node *) target, child, &removed);
        dom_node_unref(child);
        if (err != DOM_NO_ERR || removed == NULL) {
            if (removed != NULL) {
                dom_node_unref(removed);
        ...
        inserted = NULL;
        err = dom_node_append_child((dom_node *) stash, removed, &inserted);
        if (inserted != NULL) {
            dom_node_unref(inserted);
        }
        dom_node_unref(removed);
...
static int pcore_html_mutation_restore_children(dom_element *target,
        dom_document_fragment *stash)
{
    dom_node *inserted;
    dom_exception err;
    ...
    inserted = NULL;
    err = dom_node_append_child((dom_node *) target, (dom_node *) stash,
            &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
...
/* Restore a previously snapshotted direct-child order.  The caller keeps one
 * reference for every entry in `nodes`; libdom reparents each node from the
 * temporary transaction fragment (or the target) without borrowing the
 * array.  Clear the target first so partial stash/commit failures cannot
 * leave newly-created Text nodes or a reordered subset in the document. */
static int pcore_html_mutation_restore_node_list(dom_element *target,
        dom_node **nodes, unsigned int node_count)
{
    dom_node *child;
    dom_node *removed;
    dom_node *inserted;
    ...
    for (i = 0; i < node_count; i++) {
        ...
        inserted = NULL;
        err = dom_node_append_child((dom_node *) target, nodes[i],
                &inserted);
        if (inserted != NULL) {
            dom_node_unref(inserted);
        }
        ...
        unsigned int item_count)
{
    ...
    dom_node *old_nodes[PCORE_NODE_HTML_MUTATION_MAX_CHILDREN];
    dom_node *prepared[PCORE_NODE_REPLACE_CHILDREN_MIXED_LIST_MAX];
    dom_node *inserted;
    dom_document_fragment *stash;
    dom_document_fragment *fragment;
    dom_nodelist *children;
    ...
    dom_text *new_text;
    dom_node_type parent_type;
    dom_node_type item_type;
    dom_ulong old_length;
    dom_exception err;
    size_t total_bytes;
    size_t bytes;
    ...
    for (i = 0; i < item_count; i++) {
        if (items[i].kind == PCORE_NODE_REPLACE_CHILDREN_ITEM_TEXT) {
            if (items[i].element_id != NULL || items[i].text == NULL ||
                    !pcore_contenteditable_utf8_valid(items[i].text)) {
                return 3;
            }
            bytes = strlen(items[i].text);
            if (bytes > PCORE_NODE_REPLACE_CHILDREN_MIXED_TEXT_MAX_BYTES ||
                    total_bytes >
                    PCORE_NODE_REPLACE_CHILDREN_MIXED_TEXT_MAX_BYTES -
                    bytes) {
                return 3;
            }
            total_bytes += bytes;
        } else if (items[i].kind == PCORE_NODE_REPLACE_CHILDREN_ITEM_ELEMENT) {
            if (items[i].element_id == NULL ||
                    items[i].element_id[0] == '\0' ||
                    items[i].text != NULL) {
                return 3;
            }
        } else {
    ...
    for (i = item_count; i < PCORE_NODE_REPLACE_CHILDREN_MIXED_LIST_MAX;
            i++) {
        prepared[i] = NULL;
    }
    for (i = 0; i < PCORE_NODE_HTML_MUTATION_MAX_CHILDREN; i++) {
        old_nodes[i] = NULL;
    }
    doc = (dom_document *) hDoc;
    parent = pcore_element_by_id(doc, parent_id);
    ...
    if (dom_node_get_child_nodes((dom_node *) parent, &children) !=
            ...
            dom_nodelist_get_length(children, &old_length) != DOM_NO_ERR) {
        if (children != NULL) {
            dom_nodelist_unref(children);
        }
        dom_node_unref((dom_node *) parent);
    ...
    if (old_length > (dom_ulong) PCORE_NODE_HTML_MUTATION_MAX_CHILDREN) {
        dom_nodelist_unref(children);
        dom_node_unref((dom_node *) parent);
        return 3;
    }
    old_count = (unsigned int) old_length;
    for (i = 0; i < old_count; i++) {
        if (dom_nodelist_item(children, (dom_ulong) i, &old_nodes[i]) !=
                DOM_NO_ERR || old_nodes[i] == NULL) {
            dom_nodelist_unref(children);
            for (j = 0; j < old_count; j++) {
                if (old_nodes[j] != NULL) {
                    dom_node_unref(old_nodes[j]);
                }
            }
            dom_node_unref((dom_node *) parent);
            return 1;
    ...
    dom_nodelist_unref(children);
    for (i = 0; i < item_count; i++) {
        if (items[i].kind == PCORE_NODE_REPLACE_CHILDREN_ITEM_ELEMENT) {
            element = pcore_element_by_id(doc, items[i].element_id);
            if (element == NULL) {
                result = 2;
                goto mixed_cleanup;
            }
            if (dom_node_get_node_type((dom_node *) element, &item_type) !=
                    DOM_NO_ERR || item_type != DOM_ELEMENT_NODE ||
                    pcore_document_structural_token((dom_node *) element) !=
                    NULL || element == parent) {
                dom_node_unref((dom_node *) element);
                result = 2;
                goto mixed_cleanup;
            ...
            actual_parent = NULL;
            if (dom_node_get_parent_node((dom_node *) element,
                    &actual_parent) != DOM_NO_ERR || actual_parent == NULL ||
                    actual_parent != (dom_node *) parent) {
            ...
            for (j = 0; j < i; j++) {
                if (items[j].kind ==
                        PCORE_NODE_REPLACE_CHILDREN_ITEM_ELEMENT &&
                        prepared[j] == (dom_node *) element) {
                    dom_node_unref((dom_node *) element);
                    result = 2;
                    goto mixed_cleanup;
                }
            }
            prepared[i] = (dom_node *) element;
        } else {
            content = NULL;
            err = dom_string_create((const uint8_t *) items[i].text,
                    strlen(items[i].text), &content);
            ...
            prepared[i] = (dom_node *) new_text;
        }
    }
    for (i = 0; i < item_count; i++) {
        if (items[i].kind == PCORE_NODE_REPLACE_CHILDREN_ITEM_ELEMENT) {
            for (j = 0; j < old_count; j++) {
                if (old_nodes[j] == prepared[i]) {
                    break;
                }
            }
            if (j == old_count) {
    ...
    if (pcore_html_mutation_stash_children(parent, stash) != 0) {
        (void) pcore_html_mutation_restore_node_list(parent, old_nodes,
                old_count);
        dom_node_unref((dom_node *) fragment);
        dom_node_unref((dom_node *) stash);
        result = 1;
    ...
    for (i = 0; i < item_count; i++) {
        inserted = NULL;
        err = dom_node_append_child((dom_node *) fragment, prepared[i],
                &inserted);
        if (inserted != NULL) {
            dom_node_unref(inserted);
        }
    ...
    inserted = NULL;
    err = dom_node_append_child((dom_node *) parent, (dom_node *) fragment,
            &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
...
mixed_cleanup:
    for (i = 0; i < item_count; i++) {
        if (prepared[i] != NULL) {
            dom_node_unref(prepared[i]);
        }
    }
    for (i = 0; i < old_count; i++) {
        if (old_nodes[i] != NULL) {
            dom_node_unref(old_nodes[i]);
        }
    }
    dom_node_unref((dom_node *) parent);
    return result;
        ...
        unsigned int item_count)
{
    ...
    dom_node *candidate;
    dom_node *old_nodes[PCORE_NODE_HTML_MUTATION_MAX_CHILDREN];
    dom_node *prepared[PCORE_NODE_REPLACE_CHILDREN_NODE_LIST_MAX];
    dom_node *inserted;
    dom_document_fragment *stash;
    dom_document_fragment *fragment;
    dom_nodelist *children;
    ...
    if (hDoc == NULL || parent_id == NULL || parent_id[0] == '\0' ||
            item_count > PCORE_NODE_REPLACE_CHILDREN_NODE_LIST_MAX ||
            (item_count > 0 && items == NULL)) {
        return 1;
    }
    for (i = 0; i < PCORE_NODE_REPLACE_CHILDREN_NODE_LIST_MAX; i++) {
        prepared[i] = NULL;
    }
    for (i = 0; i < PCORE_NODE_HTML_MUTATION_MAX_CHILDREN; i++) {
        old_nodes[i] = NULL;
    ...
    total_bytes = 0;
    for (i = 0; i < item_count; i++) {
        if (items[i].kind == PCORE_NODE_REPLACE_CHILDREN_NODE_ITEM_TEXT) {
            if (items[i].node_type != 0U || items[i].child_index != 0U ||
                    items[i].element_id != NULL || items[i].text == NULL ||
                    !pcore_contenteditable_utf8_valid(items[i].text)) {
                return 3;
            }
            bytes = strlen(items[i].text);
            if (bytes > PCORE_NODE_REPLACE_CHILDREN_NODE_TEXT_MAX_BYTES ||
                    total_bytes >
                    PCORE_NODE_REPLACE_CHILDREN_NODE_TEXT_MAX_BYTES -
                    bytes) {
                return 3;
            }
            total_bytes += bytes;
        } else if (items[i].kind ==
                PCORE_NODE_REPLACE_CHILDREN_NODE_ITEM_ELEMENT) {
            if (items[i].node_type != 0U || items[i].child_index != 0U ||
                    items[i].element_id == NULL ||
                    items[i].element_id[0] == '\0' ||
                    items[i].text != NULL) {
                return 3;
            }
        } else if (items[i].kind ==
                PCORE_NODE_REPLACE_CHILDREN_NODE_ITEM_CHARACTER_DATA) {
            if ((items[i].node_type != 3U && items[i].node_type != 4U &&
                    items[i].node_type != 8U) ||
                    items[i].element_id != NULL || items[i].text != NULL) {
                return 3;
            }
        } else {
    ...
    for (i = 0; i < item_count; i++) {
        if (items[i].kind == PCORE_NODE_REPLACE_CHILDREN_NODE_ITEM_TEXT) {
            ...
            dom_node_unref(actual_parent);
            for (j = 0; j < i; j++) {
                if (prepared[j] == (dom_node *) element) {
                    dom_node_unref((dom_node *) element);
                    result = 2;
                    goto node_list_cleanup;
                }
            ...
            if (items[i].child_index >= old_count) {
                result = 2;
                goto node_list_cleanup;
            }
            candidate = old_nodes[items[i].child_index];
            if (candidate == NULL ||
                    dom_node_get_node_type(candidate, &item_type) !=
                    DOM_NO_ERR ||
                    ((items[i].node_type == 3U &&
                    item_type != DOM_TEXT_NODE) ||
                    (items[i].node_type == 4U &&
                    item_type != DOM_CDATA_SECTION_NODE) ||
                    (items[i].node_type == 8U &&
                    item_type != DOM_COMMENT_NODE))) {
                result = 2;
                goto node_list_cleanup;
            }
            actual_parent = NULL;
            if (dom_node_get_parent_node(candidate, &actual_parent) !=
            ...
            dom_node_unref(actual_parent);
            for (j = 0; j < i; j++) {
                if (prepared[j] == candidate) {
                    result = 2;
                    goto node_list_cleanup;
                }
            }
            prepared[i] = dom_node_ref(candidate);
            if (prepared[i] == NULL) {
                result = 1;
                goto node_list_cleanup;
            }
        ...
        const char *element_id, const char *html)
{
    ...
    fragment = NULL;
    result = pcore_html_mutation_parse_fragment(doc, html, length, &fragment);
    ...
    if (pcore_html_mutation_stash_children(target, stash) != 0) {
        (void) pcore_html_mutation_restore_children(target, stash);
        dom_node_unref((dom_node *) stash);
        dom_node_unref((dom_node *) fragment);
        dom_node_unref((dom_node *) target);
        return 1;
    ...
    inserted = NULL;
    err = dom_node_append_child((dom_node *) target,
            (dom_node *) fragment, &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
        ...
        const char *reference_child_id)
{
    ...
    dom_element *parent;
    dom_element *child;
    dom_element *reference;
    dom_node *actual_parent;
    dom_node *actual_reference_parent;
    dom_node *inserted;
    dom_node_type parent_type;
    dom_node_type child_type;
    dom_node_type reference_type;
    const char *structural;
    dom_exception err;

    if (hDoc == NULL || parent_id == NULL || parent_id[0] == '\0' ||
            child_id == NULL || child_id[0] == '\0' ||
            (reference_child_id != NULL &&
            reference_child_id[0] == '\0')) {
        return 1;
    }
    doc = (dom_document *) hDoc;
    parent = pcore_element_by_id(doc, parent_id);
    child = pcore_element_by_id(doc, child_id);
    reference = reference_child_id == NULL ? NULL :
            pcore_element_by_id(doc, reference_child_id);
    if (parent == NULL || child == NULL ||
            (reference_child_id != NULL && reference == NULL)) {
        if (reference != NULL) {
            dom_node_unref((dom_node *) reference);
        }
        if (child != NULL) {
            dom_node_unref((dom_node *) child);
        }
    ...
    if (reference != NULL) {
        structural = pcore_document_structural_token((dom_node *) reference);
        if (structural != NULL || dom_node_get_node_type(
                (dom_node *) reference, &reference_type) != DOM_NO_ERR ||
                reference_type != DOM_ELEMENT_NODE) {
            dom_node_unref((dom_node *) reference);
            dom_node_unref((dom_node *) child);
            dom_node_unref((dom_node *) parent);
            return 2;
    ...
    if (reference != NULL) {
        actual_reference_parent = NULL;
        if (dom_node_get_parent_node((dom_node *) reference,
                &actual_reference_parent) != DOM_NO_ERR ||
                actual_reference_parent == NULL ||
                actual_reference_parent != (dom_node *) parent) {
            if (actual_reference_parent != NULL) {
                dom_node_unref(actual_reference_parent);
            }
            dom_node_unref(actual_parent);
            dom_node_unref((dom_node *) reference);
            dom_node_unref((dom_node *) child);
    ...
    if (actual_parent == (dom_node *) parent && reference == child) {
        dom_node_unref(actual_parent);
        dom_node_unref((dom_node *) reference);
        dom_node_unref((dom_node *) child);
        dom_node_unref((dom_node *) parent);
        ...
        const char *element_id, unsigned int child_index)
{
    ...
    if (hDoc == NULL || parent_id == NULL || parent_id[0] == '\0' ||
            tag_name == NULL || tag_name[0] == '\0' ||
            element_id == NULL || element_id[0] == '\0') {
        return 1;
    }
    tag_length = strlen(tag_name);
    id_length = strlen(element_id);
    if (tag_length > PCORE_NODE_CREATE_ELEMENT_TAG_MAX ||
            id_length > PCORE_NODE_CREATE_ELEMENT_ID_MAX ||
            !pcore_contenteditable_utf8_valid(tag_name) ||
            !pcore_contenteditable_utf8_valid(element_id)) {
        return 3;
    }
    doc = (dom_document *) hDoc;
    parent = pcore_element_by_id(doc, parent_id);
    ...
    parent_structural = pcore_document_structural_token((dom_node *) parent);
    if (dom_node_get_node_type((dom_node *) parent, &parent_type) !=
            DOM_NO_ERR || parent_type != DOM_ELEMENT_NODE ||
            (parent_structural != NULL && strcmp(parent_structural,
            PCORE_DOCUMENT_BODY_TOKEN) != 0)) {
        dom_node_unref((dom_node *) parent);
        return 2;
    }
    existing = pcore_element_by_id(doc, element_id);
    if (existing != NULL) {
        dom_node_unref((dom_node *) existing);
        dom_node_unref((dom_node *) parent);
        return 2;
    ...
    dom_nodelist_unref(children);
    tag = NULL;
    if (dom_string_create((const uint8_t *) tag_name, tag_length, &tag) !=
            DOM_NO_ERR || tag == NULL) {
        if (reference != NULL) {
            dom_node_unref(reference);
        }
    ...
    element = NULL;
    err = dom_document_create_element(doc, tag, &element);
    dom_string_unref(tag);
    if (err != DOM_NO_ERR || element == NULL) {
        if (element != NULL) {
            dom_node_unref((dom_node *) element);
        }
        if (reference != NULL) {
        ...
        dom_node_unref((dom_node *) parent);
        return err == DOM_INVALID_CHARACTER_ERR ? 2 : 1;
    }
    id_name = NULL;
    id_value = NULL;
    if (dom_string_create((const uint8_t *) "id", 2, &id_name) !=
            DOM_NO_ERR || id_name == NULL ||
            dom_string_create((const uint8_t *) element_id, id_length,
    ...
    err = dom_element_set_attribute(element, id_name, id_value);
    if (err == DOM_NO_ERR) {
        err = dom_element_set_id_attribute(element, id_name, true);
    }
    dom_string_unref(id_value);
    dom_string_unref(id_name);
    if (err != DOM_NO_ERR) {
    ...
    inserted = NULL;
    err = dom_node_insert_before((dom_node *) parent, (dom_node *) element,
            reference, &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
...
PCORE_API int PCore_NodeCreateCDATAChildAtById(HANDLE hDoc,
        const char *parent_id, unsigned int child_index, const char *data)
{
    dom_document *doc;
    dom_element *parent;
    ...
    data_length = strlen(data);
    if (data_length > PCORE_NODE_CREATE_CDATA_DATA_MAX_BYTES ||
            !pcore_contenteditable_utf8_valid(data)) {
        return 3;
    }
    doc = (dom_document *) hDoc;
    ...
    dom_nodelist_unref(children);
    {
        ...
        cdata = NULL;
        err = dom_document_create_cdata_section(doc, content, &cdata);
        dom_string_unref(content);
    }
    if (err != DOM_NO_ERR || cdata == NULL) {
        if (cdata != NULL) {
            dom_node_unref((dom_node *) cdata);
        }
        if (reference != NULL) {
            dom_node_unref(reference);
        }
    ...
    inserted = NULL;
    err = dom_node_insert_before((dom_node *) parent, (dom_node *) cdata,
            reference, &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
...
{
    dom_document *doc;
    dom_element *source_parent;
    dom_element *target_parent;
    dom_node *child;
    dom_node *reference;
    dom_node *actual_parent;
    dom_node *inserted;
    dom_nodelist *children;
    dom_ulong length;
    dom_node_type source_parent_type;
    dom_node_type target_parent_type;
    dom_node_type child_type;
    dom_exception err;
    int result;

    if (hDoc == NULL || source_parent_id == NULL ||
            source_parent_id[0] == '\0' ||
            target_parent_id == NULL || target_parent_id[0] == '\0' ||
            (node_type != 3U && node_type != 4U && node_type != 8U)) {
        return 1;
    }
    doc = (dom_document *) hDoc;
    source_parent = pcore_element_by_id(doc, source_parent_id);
    target_parent = pcore_element_by_id(doc, target_parent_id);
    if (source_parent == NULL || target_parent == NULL) {
        if (target_parent != NULL) {
            dom_node_unref((dom_node *) target_parent);
        }
        if (source_parent != NULL) {
            dom_node_unref((dom_node *) source_parent);
        }
        return 2;
    }
    if (dom_node_get_node_type((dom_node *) source_parent,
            &source_parent_type) != DOM_NO_ERR ||
            dom_node_get_node_type((dom_node *) target_parent,
            &target_parent_type) != DOM_NO_ERR ||
            source_parent_type != DOM_ELEMENT_NODE ||
            target_parent_type != DOM_ELEMENT_NODE) {
        dom_node_unref((dom_node *) target_parent);
        dom_node_unref((dom_node *) source_parent);
        return 2;
    }
    child = NULL;
    result = pcore_relation_child_node_at((dom_node *) source_parent,
            source_index, &child);
    if (result != 0) {
        dom_node_unref((dom_node *) target_parent);
        dom_node_unref((dom_node *) source_parent);
    ...
    if (dom_node_get_node_type(child, &child_type) != DOM_NO_ERR ||
            ((node_type == 3U && child_type != DOM_TEXT_NODE) ||
            (node_type == 4U && child_type != DOM_CDATA_SECTION_NODE) ||
            (node_type == 8U && child_type != DOM_COMMENT_NODE))) {
        dom_node_unref(child);
        dom_node_unref((dom_node *) target_parent);
        dom_node_unref((dom_node *) source_parent);
        return 2;
    ...
    actual_parent = NULL;
    if (dom_node_get_parent_node(child, &actual_parent) != DOM_NO_ERR ||
            actual_parent == NULL || actual_parent !=
            (dom_node *) source_parent) {
        if (actual_parent != NULL) {
            dom_node_unref(actual_parent);
        }
        dom_node_unref(child);
    ...
    children = NULL;
    length = 0;
    if (dom_node_get_child_nodes((dom_node *) target_parent, &children) !=
            DOM_NO_ERR || children == NULL ||
            dom_nodelist_get_length(children, &length) != DOM_NO_ERR) {
        if (children != NULL) {
            dom_nodelist_unref(children);
    ...
    if ((dom_ulong) target_index > length) {
        dom_nodelist_unref(children);
        dom_node_unref(child);
        dom_node_unref((dom_node *) target_parent);
        dom_node_unref((dom_node *) source_parent);
    ...
    reference = NULL;
    if ((dom_ulong) target_index < length &&
            (dom_nodelist_item(children, (dom_ulong) target_index,
            &reference) != DOM_NO_ERR || reference == NULL)) {
        dom_nodelist_unref(children);
        dom_node_unref(child);
        dom_node_unref((dom_node *) target_parent);
    ...
    inserted = NULL;
    err = dom_node_insert_before((dom_node *) target_parent, child,
            reference, &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
...
PCORE_API int PCore_NodeReplaceCharacterDataChildById(HANDLE hDoc,
        const char *target_parent_id, unsigned int target_index,
        ...
        unsigned int source_index, unsigned int source_node_type)
{
    ...
    dom_node_type new_type;
    dom_exception err;
    int result;

    if (hDoc == NULL || target_parent_id == NULL ||
            target_parent_id[0] == '\0' || source_parent_id == NULL ||
            source_parent_id[0] == '\0' ||
            (target_node_type != 3U && target_node_type != 4U &&
            target_node_type != 8U) ||
            (source_node_type != 3U && source_node_type != 4U &&
            source_node_type != 8U)) {
        return 1;
    }
    doc = (dom_document *) hDoc;
    ...
    if (dom_node_get_node_type((dom_node *) target_parent,
            &target_parent_type) != DOM_NO_ERR ||
            dom_node_get_node_type((dom_node *) source_parent,
            &source_parent_type) != DOM_NO_ERR ||
            target_parent_type != DOM_ELEMENT_NODE ||
            source_parent_type != DOM_ELEMENT_NODE) {
        dom_node_unref((dom_node *) source_parent);
    ...
    old_child = NULL;
    result = pcore_relation_child_node_at((dom_node *) target_parent,
            target_index, &old_child);
    if (result != 0) {
        dom_node_unref((dom_node *) source_parent);
        dom_node_unref((dom_node *) target_parent);
        return result == 2 ? 2 : 1;
    ...
    if (dom_node_get_node_type(old_child, &old_type) != DOM_NO_ERR ||
            dom_node_get_node_type(new_child, &new_type) != DOM_NO_ERR ||
            ((target_node_type == 3U && old_type != DOM_TEXT_NODE) ||
            (target_node_type == 4U &&
            old_type != DOM_CDATA_SECTION_NODE) ||
            (target_node_type == 8U && old_type != DOM_COMMENT_NODE) ||
            (source_node_type == 3U && new_type != DOM_TEXT_NODE) ||
            (source_node_type == 4U &&
            new_type != DOM_CDATA_SECTION_NODE) ||
            (source_node_type == 8U && new_type != DOM_COMMENT_NODE))) {
    ...
    old_parent = NULL;
    if (dom_node_get_parent_node(old_child, &old_parent) != DOM_NO_ERR ||
            old_parent == NULL || old_parent !=
            (dom_node *) target_parent) {
        if (old_parent != NULL) {
            dom_node_unref(old_parent);
        }
        dom_node_unref(new_child);
        dom_node_unref(old_child);
        dom_node_unref((dom_node *) source_parent);
    ...
    dom_node_unref(old_parent);
    new_parent = NULL;
    if (dom_node_get_parent_node(new_child, &new_parent) != DOM_NO_ERR ||
            new_parent == NULL || new_parent !=
            (dom_node *) source_parent) {
        if (new_parent != NULL) {
            dom_node_unref(new_parent);
        }
        dom_node_unref(new_child);
        dom_node_unref(old_child);
        dom_node_unref((dom_node *) source_parent);
    ...
    replaced = NULL;
    err = dom_node_replace_child((dom_node *) target_parent, new_child,
            old_child, &replaced);
    if (replaced != NULL) {
        dom_node_unref(replaced);
    }
        ...
        unsigned int node_type)
{
    ...
    doc = (dom_document *) hDoc;
    parent = pcore_element_by_id(doc, parent_id);
    old_child = pcore_element_by_id(doc, old_child_id);
    source_parent = pcore_element_by_id(doc, source_parent_id);
    if (parent == NULL || old_child == NULL || source_parent == NULL) {
        if (source_parent != NULL) {
            dom_node_unref((dom_node *) source_parent);
        }
        if (old_child != NULL) {
            dom_node_unref((dom_node *) old_child);
        }
        if (parent != NULL) {
            dom_node_unref((dom_node *) parent);
        }
    ...
    if (dom_node_get_node_type((dom_node *) parent, &parent_type) !=
            DOM_NO_ERR || dom_node_get_node_type((dom_node *) old_child,
            &old_type) != DOM_NO_ERR || dom_node_get_node_type(
            (dom_node *) source_parent, &source_parent_type) != DOM_NO_ERR ||
            parent_type != DOM_ELEMENT_NODE || old_type != DOM_ELEMENT_NODE ||
            source_parent_type != DOM_ELEMENT_NODE) {
        dom_node_unref((dom_node *) source_parent);
        dom_node_unref((dom_node *) old_child);
        dom_node_unref((dom_node *) parent);
    ...
    structural = pcore_document_structural_token((dom_node *) old_child);
    if (structural != NULL || old_child == parent) {
        dom_node_unref((dom_node *) source_parent);
        dom_node_unref((dom_node *) old_child);
        dom_node_unref((dom_node *) parent);
        return 2;
    ...
    old_parent = NULL;
    if (dom_node_get_parent_node((dom_node *) old_child, &old_parent) !=
            DOM_NO_ERR || old_parent == NULL || old_parent !=
            (dom_node *) parent) {
        if (old_parent != NULL) {
            dom_node_unref(old_parent);
        }
    ...
    if (dom_node_get_node_type(new_child, &new_type) != DOM_NO_ERR ||
    ...
    dom_node_unref(new_parent);
    replaced = NULL;
    err = dom_node_replace_child((dom_node *) parent, new_child,
            (dom_node *) old_child, &replaced);
    if (replaced != NULL) {
        dom_node_unref(replaced);
    }
    dom_node_unref(new_child);
        ...
        const char *old_child_id)
{
    ...
    if (hDoc == NULL || parent_id == NULL || parent_id[0] == '\0' ||
            new_child_id == NULL || new_child_id[0] == '\0' ||
            old_child_id == NULL || old_child_id[0] == '\0') {
        return 1;
    }
    doc = (dom_document *) hDoc;
    parent = pcore_element_by_id(doc, parent_id);
    new_child = pcore_element_by_id(doc, new_child_id);
    old_child = pcore_element_by_id(doc, old_child_id);
    if (parent == NULL || new_child == NULL || old_child == NULL) {
        if (old_child != NULL) {
            dom_node_unref((dom_node *) old_child);
        }
        if (new_child != NULL) {
            dom_node_unref((dom_node *) new_child);
        }
        if (parent != NULL) {
            dom_node_unref((dom_node *) parent);
        }
    ...
    if (dom_node_get_node_type((dom_node *) parent, &parent_type) !=
            DOM_NO_ERR || dom_node_get_node_type((dom_node *) new_child,
            &new_type) != DOM_NO_ERR || dom_node_get_node_type(
            (dom_node *) old_child, &old_type) != DOM_NO_ERR ||
            parent_type != DOM_ELEMENT_NODE || new_type != DOM_ELEMENT_NODE ||
            old_type != DOM_ELEMENT_NODE) {
        dom_node_unref((dom_node *) old_child);
        dom_node_unref((dom_node *) new_child);
        dom_node_unref((dom_node *) parent);
        return 2;
    }
    structural = pcore_document_structural_token((dom_node *) new_child);
    if (structural != NULL) {
        dom_node_unref((dom_node *) old_child);
        dom_node_unref((dom_node *) new_child);
        dom_node_unref((dom_node *) parent);
    ...
    structural = pcore_document_structural_token((dom_node *) old_child);
    if (structural != NULL || old_child == parent || new_child == parent) {
        dom_node_unref((dom_node *) old_child);
        dom_node_unref((dom_node *) new_child);
        dom_node_unref((dom_node *) parent);
        return 2;
    ...
    new_parent = NULL;
    if (dom_node_get_parent_node((dom_node *) new_child, &new_parent) !=
            DOM_NO_ERR || new_parent == NULL) {
        if (new_parent != NULL) {
            dom_node_unref(new_parent);
        }
        dom_node_unref((dom_node *) old_child);
    ...
    dom_node_unref(old_parent);
    replaced = NULL;
    err = dom_node_replace_child((dom_node *) parent,
            (dom_node *) new_child, (dom_node *) old_child, &replaced);
    if (replaced != NULL) {
        dom_node_unref(replaced);
    }
    dom_node_unref(new_parent);
        ...
        const char *parent_id, const char *old_child_id, const char *text)
{
    ...
    if (hDoc == NULL || parent_id == NULL || parent_id[0] == '\0' ||
            old_child_id == NULL || old_child_id[0] == '\0' ||
            text == NULL || !pcore_contenteditable_utf8_valid(text)) {
        return 1;
    }
    doc = (dom_document *) hDoc;
    parent = pcore_element_by_id(doc, parent_id);
    old_child = pcore_element_by_id(doc, old_child_id);
    if (parent == NULL || old_child == NULL) {
        if (old_child != NULL) {
            dom_node_unref((dom_node *) old_child);
        }
        if (parent != NULL) {
    ...
    if (dom_node_get_node_type((dom_node *) parent, &parent_type) !=
            DOM_NO_ERR || dom_node_get_node_type((dom_node *) old_child,
            &old_type) != DOM_NO_ERR || parent_type != DOM_ELEMENT_NODE ||
            old_type != DOM_ELEMENT_NODE) {
        dom_node_unref((dom_node *) old_child);
        dom_node_unref((dom_node *) parent);
        return 2;
    ...
    replaced = NULL;
    err = dom_node_replace_child((dom_node *) parent, (dom_node *) new_text,
            (dom_node *) old_child, &replaced);
    if (replaced != NULL) {
        dom_node_unref(replaced);
    }
        ...
        const char *parent_id, unsigned int child_index, const char *text)
{
    ...
    ref = NULL;
    if ((dom_ulong) child_index < length) {
        if (dom_nodelist_item(children, (dom_ulong) child_index, &ref) !=
                DOM_NO_ERR || ref == NULL) {
            dom_nodelist_unref(children);
            dom_node_unref((dom_node *) parent);
            return 1;
    ...
    inserted = NULL;
    err = dom_node_insert_before((dom_node *) parent, (dom_node *) new_text,
            ref, &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
        ...
        const char *const *texts, unsigned int text_count)
{
    ...
    inserted = NULL;
    err = dom_node_insert_before((dom_node *) parent, (dom_node *) fragment,
            ref, &inserted);
    if (inserted != NULL) {
        dom_node_unref(inserted);
    }
