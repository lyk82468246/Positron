 ...
 * element children retain their DOM positions.  Returns 0 after a Text node
 * was updated, 2 when the parent or index is unavailable or the child is not
 * a Text node, and 1 for invalid input or a DOM failure.  A successful update
 * invalidates retained layout; callers must style/layout/paint again before
 * using geometry or native-control snapshots.  This primitive does not
 * insert, remove or reparent nodes and does not dispatch events. */
...
/* Normalize the direct child list of one addressed element. Empty Text and
 * CDATASection children are removed and adjacent Text/CDATA children are
 * merged into the first non-empty node in each run; other children are
 * boundaries. The operation is intentionally bounded to this element's
 * direct children so a caller can control recursive traversal and cache
 * reconciliation. Returns 0 after normalization, 2 when the element is
 * unavailable/non-element and 1 for invalid input or another DOM failure. A
 * changed list invalidates retained layout; callers must style/layout/paint
 * again. No events, resource fetches, reparenting or general live collection
 * are provided. */
...
/* Create one new CDATASection and insert it at an unfiltered childNodes
 * index. `parent_id` and `data` are borrowed UTF-8; the parent must be a
 * connected addressable Element and `child_index` may equal its current child
 * count. The returned CDATASection has no public Core handle: Browser owns any
 * detached wrapper and later materializes it through this primitive. Returns
 * 0 after insertion, 2 when the parent or index is unavailable, 3 for
 * malformed UTF-8 or data over PCORE_NODE_CREATE_CDATA_DATA_MAX_BYTES, and 1
 * for invalid input or another DOM/allocation failure. A successful insertion
 * invalidates retained layout; no events, script, resource fetches or
 * DocumentFragment exposure are part of this primitive. */
#define PCORE_NODE_CREATE_CDATA_DATA_MAX_BYTES 65535u
...
/* Replace one existing direct CharacterData child with another connected
 * CharacterData node from this document. `target_parent_id`/`target_index`
 * identify the old direct Text, CDATA or Comment child and
 * `source_parent_id`/`source_index` identify the new direct child; the two
 * explicit node types must match their indexed nodes. The new node keeps its
 * identity and may move between element parents. Replacing a node with
 * itself is a successful no-op. Document structure tokens, detached nodes,
 * wrong node types and unavailable relations are rejected. Returns 0 after
 * replacement, 2 when a parent, relation, type or index is unavailable, and
 * 1 for invalid input or another DOM failure. A successful replacement
 * invalidates retained layout; callers must re-query and style/layout/paint
 * again. No events, resource fetches or live collections are dispatched. */
 ...
 * all supported listed input, select, textarea, button, fieldset, object and
 * output form-associated elements owned by the form
 * in document order, including controls outside the form that use that explicit
 * attribute. Fieldsets, object and output elements are included for
 * HTMLFormElement.elements but remain excluded from the separate
 * successful-control visitor used by submission and FormData. Img has an owner
 * projection but is not listed in either form collection. Fieldset, img, object
 * and output results
 * are read snapshots for one relation query.
 ...
 * a valid value supplies the step base when min is absent.
 ...
 * type/min/max/step checks; a valid value supplies the step base when min is
...
/* Read an application-owned custom validity message from an EDIT or
 * password control. The UTF-8 message is copied into message when capacity
 * is positive and NUL-terminated; truncation leaves room for the terminator.
 * Returns the complete message byte length (excluding NUL), or -1 when the
 * document/control cannot be resolved or the length cannot be represented. */
...
typedef struct PCoreFormDataEntryInfo {
    int kind;
    int name_bytes;
    int value_bytes;
