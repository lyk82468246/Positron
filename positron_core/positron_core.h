 ...
 * all supported input, select, textarea, button and fieldset form-associated
 * elements owned by the form
 * in document order, including controls outside the form that use that explicit
 * attribute. Fieldsets are included for HTMLFormElement.elements but remain
 * excluded from the separate successful-control visitor used by submission
 * and FormData. Their results are read snapshots for one relation query.
