...
- `test_host.exe` 是回归宿主和示例消费者，不是公共 API 或可复用业务语义的所有者；不得在其中编译产品实现源文件。
...
- 每次功能改动先判定语义所有者：可复用的 URL、文档、资源、history、事件、表单、图像、脚本或生命周期语义必须进入对应公共 DLL；`test_host` 只能提供平台接线、网络/线程调度、应用策略、fixture 和断言。若改动只落在宿主却改变了产品行为，必须先停止并迁移，而不是把宿主实现当作临时位置。
...
- 提交前必须通过 `python scripts/audit_repo.py` 的文档结构门和 `test_host` 边界门；不得通过放宽规则掩盖流水账或把产品实现藏进宿主。
