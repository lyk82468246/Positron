...
每个候选资源在宿主内沿 `pending` 单向进入 `ready`、`failed` 或 `cancelled`。失败摘要区分 resolve、transport、HTTP、budget 和 memory；transport 失败每项最多重试 2 次（总计最多 3 次尝试），HTTP、resolve、budget、memory 和取消不重试；预算耗尽仍是 transport failure。取消单独计数，不会被重新当作网络失败，也不会通过缓存回调提供数据。样式表与 `@import` 标为 required，脚本与图片标为 optional；style pass 发现新的 pending 时回到 worker，layout/swap 前重新计算 gate。相同 URL（包括重复脚本/图片和深层 `@import`）共享一个资源条目，并保留 stylesheet/script/image role bitmask。required 失败、未收敛或取消保留旧页，optional 失败允许候选提交并交给 Core fallback。宿主日志可输出最多 4 项不含原始 URL 的 `role/failure#hash` 摘要，并在 layout 成功后记录 image/script/other fallback family 的粗粒度观测；这些摘要、计数和 gate 只用于日志与导航调度，不改变 Core/Browser 的公共 ABI；宿主没有通用逐资源提交 UI。
