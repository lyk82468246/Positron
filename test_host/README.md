...
- 离线 compatibility corpus 覆盖 contenteditable、dialog/form、navigation、scroll/geometry、
  ...
  detached Text/Comment/Element 的数据、生命周期、clone、关系和 child-query；
...
TEST1244–1252 覆盖 detached Text/Element/Comment 的属性/文本、数据、clone、物化、生命周期、
关系和 child-query。TEST1250 断言空/重复 id 不合并；TEST1251 断言 `hasChildNodes()`
跟随 direct-Text staging；TEST1252 断言 `style.cssText` 写入各 wrapper 状态。宿主只提供
fixture/断言。
