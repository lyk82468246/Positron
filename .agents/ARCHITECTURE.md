...
- `positron_image.dll`：统一 WM Imaging 位图与 SVG 能力，可被 core 或其他 WM 程序独立调用。公共 API 同时提供 opaque bitmap/SVG create/info/draw/free、ABI 查询，以及 retained 位图到 PNG/JPEG 内存编码；ABI 1.2 的兼容扩展可为 JPEG 指定 WM Imaging 原生 quality。DLL 复制位图输入字节并持有原生 `IImage`，编码结果由 DLL 分配并以 `PImage_FreeBuffer` 释放，不向调用方暴露 COM、NetSurf 对象或跨 CRT 所有权。`positron_core` 的旧 `PCore_Image*` 保留为兼容转发。
