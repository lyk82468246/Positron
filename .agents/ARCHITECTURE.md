...
- `positron_script.dll`：独立的 Duktape 2.7.0 JavaScript 执行 DLL，提供 opaque `HANDLE`、UTF-8 源码求值、持久上下文、错误恢复、预算和内存/执行次数遥测；ABI 1.5 又增加固定 16 槽的同步 JSON 宿主回调注册/注销，ABI 1.6 再增加复制式 structured JSON global setter。回调结果最多 255 字节且不得重入/异步持有上下文，JSON setter 输入最多 64 KiB。它不依赖 `positron_core.dll`，也不自动提供 DOM、window、fetch、网络或浏览器宿主绑定；TEST80-99 只验证这个独立消费边界。浏览器内联/外链 JavaScript 仍保持关闭。
...
WM application ---------> positron_script.dll -> Duktape 2.7.0
