...
- `positron_browser.dll` 已拥有独立 history/session 产品层、PScript context、host JSON callback
  ...
  （defaultValue/defaultChecked/selectedIndex）、navigation、同文档 location/history 事件分发、event JSON 分发、native input/composition、keyboard、focus-family、EDIT change/post-change input、click 和 SELECT input/change typed dispatch entry；显式开启时仍有 classic inline/external
...
validation、keyboard/focus-family/EDIT change/post-change input/click/SELECT input/change typed dispatch 和部分 composition bridge；WM 控件与 core 事件传播仍由宿主负责。
...
`TEST13/20/27/43/44/56/58-77/80-220/999`（168 项）全量门验证 native click typed dispatch contract；next250 的 SELECT input、next249 的 SELECT change、next248 的 focus-family、next247 的 native keyboard、next246 的 native input/composition contract 保持通过；其余 form/input callback 实现、
