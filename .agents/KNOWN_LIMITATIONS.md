...
- **独立消费与编码**：next46 已确认只导入 `positron_image.dll`/`COREDLL.dll` 的 ABI 1.0 示例横竖屏工作；其 SVG 被缩放到较窄目标矩形后曲线略粗，用户判定可接受，不改变 TEST26/27 的抗锯齿基线。next47 又确认 ABI 1.1 的 PNG/JPEG 内存编码、释放输出后重新解码与 retained SVG 均能闭环；PNG 与源 BMP 正常，但设备默认 JPEG 在 16x16 高饱和四象限上有明显紫/黄色度串扰，不能表述为轻微有损。ABI 1.2 兼容追加 `PImage_EncodeBitmapEx`，JPEG 接受 quality 0..100 或 -1 默认值，next48 用 quality=100 待设备确认。编码器仍来自 WM Imaging 安装列表；设备缺少对应 codec 或拒绝参数时返回 unsupported，不自行实现格式。质量 100 不等价于承诺 codec 禁用色度子采样；原始像素输入和 GIF/BMP 输出仍未提供。
