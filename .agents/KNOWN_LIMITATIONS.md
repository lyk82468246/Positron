...
| author-level inline CSS | 外部 author stylesheet 正常参与 libcss 选择；TEST57 使用外部类规则通过。 | `nsshim/utils/nsoption.h` 当前把所有 `nsoption_bool` 固定为 false，因此 HTML `style=` 属性路径被关闭；启用前必须为选项建立具名默认值并跑 TEST13 全流程。 |
