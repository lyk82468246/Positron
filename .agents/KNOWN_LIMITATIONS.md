...
| table span/归一化/折叠边框 | NetSurf 3.11 span occupancy 与匿名 row/cell 已由 TEST46/47 验收。next64/TEST53 的 cell/row/row-group/table 宽度、样式、hidden、来源、方向 tie 和 separate 对照已由纵横屏确认。next65/TEST54 正验证 rowspan 终止 row、`rowspan=0`、colspan 及 row-group 共享边。 | 尚不覆盖任意 inline/float/form 畸形组合、caption/column 归一化、`col`/`colgroup` border 来源或所有复杂 span 边界。 |
