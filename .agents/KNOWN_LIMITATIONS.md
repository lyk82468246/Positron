...
| Forms/widgets | next85/93 至 next104/TEST71 已完成 checkbox/radio、文本、textarea、single/multiple select、button、GET/POST、reset/Enter/label 与 multipart/file；next106/TEST72 已确认首批 `required/valueMissing`、首个无效控件定位、提交阻止与 bypass。 | multipart 仍整体缓冲且 MIME 固定；尚无流式上传、上传进度、MIME 推断、multiple file、`pattern`、类型/范围约束、custom validity、`invalid` 事件、验证气泡和完整事件系统。自动断言也不等于真实手指、原生选择器或公网 POST 已人工验收。 |
...
| Forms/widgets | next85/93 已完成 checkbox/radio；next94/TEST65 已确认单行输入；next97/TEST66 已确认 textarea；next98/TEST67 已确认单选；next99/TEST68 已确认 button、successful-controls 与 WM GET/POST；next101/TEST69 已确认 reset、Enter 和 label；next103/TEST70 已确认 file/multipart；next104/TEST71 已确认 WM 原生 multiple LISTBOX、disabled 回滚、重复值和重建/reset。 | multipart 会在 WM 宿主内整体缓冲请求体，文件 MIME 固定为 `application/octet-stream`；尚无流式上传、上传方向进度、MIME 推断、`multiple` 文件、约束验证和完整事件系统。自动桥断言也不等于控件外观、真实手指操作、原生文件选择器/多选列表或公网 POST 端点已人工验收。 |
