...
| Forms/widgets | next85/93 已完成 checkbox/radio；next94/TEST65 已确认单行输入；next97/TEST66 已确认 textarea；next98/TEST67 已确认 select；next99/TEST68 已确认 NetSurf button gadget、successful-controls 编码与 WM GET/URL-encoded POST。 | multipart/file、reset 恢复、Enter 隐式提交、WM 多选列表、label 激活和完整事件系统未实现。自动桥断言也不等于控件外观、真实手指操作或公网 POST 端点已人工验收。 |
