...
| 图片 | TEST18-20 已确认 WM Imaging 四格式缓存链；TEST25-30/13 已确认 SVG parse/draw/cache/fallback/fill-rule、网络相对 SVG 与 CSS 背景图正式链。 | SVG text、复杂 SVG/CSS 背景或任意网络图片均已通过。 |
...
- **当前结论**：BMP/PNG/JPEG/GIF 四格式与 TEST20 缓存 `<img>` 已由设备视觉确认。TEST25-30/13 已真机确认 SVG parse、公共 DLL retained object、NanoSVG 抗锯齿 draw、缓存/fallback、真实网络相对资源、nonzero/evenodd 与 CSS background-image。CSS 背景仍不含 background-size、多层背景和异步资源事务。TEST31 已构建待设备，基础 SVG text 使用 WM GDI 字体并保持 path/text 顺序；暂不支持复杂 shaping、`textPath`、逐字 dx/dy、任意 shear 或完整 SVG text 规范。单次栅格源缓冲限制为 1,048,576 像素，超大输出会降低内部采样分辨率后再缩放。
