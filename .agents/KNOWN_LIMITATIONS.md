...
- **独立消费与编码**：next46 已确认只导入 `positron_image.dll`/`COREDLL.dll` 的 ABI 1.0 示例横竖屏工作；其 SVG 被缩放到较窄目标矩形后曲线略粗，用户判定可接受，不改变 TEST26/27 的抗锯齿基线。next47 又确认 ABI 1.1 的 PNG/JPEG 内存编码、释放输出后重新解码与 retained SVG 均能闭环；PNG 与源 BMP 正常，但设备默认 JPEG 在 16x16 高饱和四象限上有明显紫/黄色度串扰。next48 的 WM quality=100 只有有限改善，不能称为修复。ABI 1.2 现保留 quality=-1 的 WM 默认兼容路径，显式 0..100 切到静态 libjpeg-turbo 1.5.3 压缩器并强制 4:4:4；该库只接收 WM 已解码的 24bpp 像素，不承担不可信 JPEG 解码。next49 待确认像素行方向和颜色，未通过前不宣称问题关闭。原始像素输入和 GIF/BMP 输出仍未提供。
