...
- Browser/Core 只支持有界 DOM mutation：`textContent`/非编辑 `innerText`、CharacterData
  ...
  时恢复旧树。目标结构 token、对象、mixed/nested fragment、CharacterData、跨父/重复/自身
  element、超限和 callback 缺失均 fail closed；失败不会消费 fragment 或改变原树。
...
fail closed 和注销后的静默均已自动断言。该门不执行自动资源替换，也不覆盖通用动态 DOM
  ...
  视觉或触摸/SIP 风险。
- TEST1201–1242 覆盖有界 DOM/CharacterData removal、normalize、clone/equality、
  Ex4–Ex14 insertion/replacement、`insertAdjacent*()`、mixed wrapper/snapshot、detached、
  UTF-16、parser-backed HTML mutation、text-only DocumentFragment staging、
  `Element.replaceChildren()`（含 Ex13 文本/fragment 与 Ex14 同父 mixed element/text）和
