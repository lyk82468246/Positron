...
| 列表 marker | next57/59 已确认基础 marker 与字体；next61/TEST50 已确认 libcss 上游 47 种 counter formatter、document-cache `list-style-image` 与失败类型回退；next62/TEST51 已确认 inline-first `list-style-position:inside`。next63 已实现 block-first、空条目、嵌套与图片 marker 的匿名首行，待 TEST52 设备验收。 | 不代表 float 邻接 marker、自定义 `@counter-style`、所有语言字体或完整 CSS Lists。 |
