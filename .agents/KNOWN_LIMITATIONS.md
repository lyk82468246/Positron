...
**当前状态更正（next133，2026-08-07）**：next132 在 `480x640 dpi=192` 下于
TEST58 停止。失败值 `article=320 rows=40/80/40` 是该离线几何测试继承了前一个
设备-backed render 的待布局上下文；next133 已在 TEST58 几何阶段显式使用
`230x260 @ 96 DPI` CSS 契约，并在可见渲染阶段恢复真实设备视口。ARMV4I 构建/staging
通过，尚未获得设备日志；TEST58 的断言和布局实现均未放宽或改写。
