...
| 外部脚本资源 | next114/TEST77 在 core 中建立了非空 script-src 扫描、宿主 URL resolver/fetch/free 回调、document 生命周期缓存、重复引用去重和只读枚举 ABI；ARMV4I 构建已通过，设备日志待确认。 | 不执行 inline/external JavaScript，不解释 type，不抓取或执行脚本到冻结的 TEST13 网络事务；尚无模块依赖图、动态插入、错误事件、CSP、跨源策略或持久 HTTP cache。 |
