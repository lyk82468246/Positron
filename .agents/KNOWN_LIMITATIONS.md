...
- `contenteditable` 目前只覆盖单元素的祖先继承、`isContentEditable`、有界纯文本 mutation 以及宿主编排的 `beforeinput`/`input`。带 id 且已布局的有效 editing host 可由宿主映射为最多 16 个 WM multiline EDIT 代理，文本上限为 8192 UTF-8 字节；嵌套继承后代不重复代理。caret/selection、富文本、design mode、剪贴板和完整 IME 组合仍未实现。
...
- synthetic `WM_CHAR`/key/composition 测试只能证明 WM EDIT 代理的事务边界，不能证明 caret/selection、候选词窗口、完整 IME、真实硬键盘或 SIP 视觉。
