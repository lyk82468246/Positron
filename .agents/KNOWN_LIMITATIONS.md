...
| Forms/widgets | next85 已按 NetSurf gadget 数据形态接入 checkbox/radio 静态 redraw；next93/TEST64 又确认点击切换、disabled、同表单同名 radio 互斥、跨组隔离、DOM 同步和旋转重排保持。next94/TEST65 已在设备确认单行 text/password 的 NetSurf 几何、UTF-8 状态、WM 原生 `EDIT`、maxlength/read-only/disabled 和旋转重排保持。 | 已验收基线仍不含 label 激活、textarea/select/button、表单编码或提交；next94 也不等于完整焦点/事件系统。 |
...
- **next94 单行输入已验收**：core 以 UTF-8 ABI 暴露 text/password 的值与状态，WM 宿主使用原生 `EDIT`，并通过真实 `EN_CHANGE` 把 UTF-16 编辑结果同步回 libdom。设备无人值守日志确认 TEST65 及同批 TEST13/20/27/43/44/56/58-64 全部 PASS；textarea、select/button、label 激活、提交和完整事件系统仍未实现。
