...
| 图片 | TEST18-20 已确认 WM Imaging 四格式缓存链；TEST25-28/13 已确认 SVG parse、抗锯齿 draw、缓存 `<img>`、损坏 fallback 与网络相对 SVG 正式链。 | 复合 fill-rule、SVG text、背景图或任意网络图片均已通过。 |
...
- **当前结论**：BMP/PNG/JPEG/GIF 四格式与 TEST20 缓存 `<img>` 已由设备视觉确认。TEST25-28/13 已真机确认 SVG parse、公共 DLL retained object、NanoSVG 抗锯齿 draw、缓存 `<img>`、损坏 fallback 和真实网络相对资源链。官方最新 libsvgtiny（审计提交 `7ede71b`）仍未保留 `fill-rule`；本地扩展与 TEST29 已构建，复合路径 nonzero/evenodd 等待设备验收。SVG text 和 CSS background image 尚未验证。单次栅格源缓冲限制为 1,048,576 像素，超大输出会降低内部采样分辨率后再缩放。
