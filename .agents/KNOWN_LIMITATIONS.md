...
| Forms/widgets | next85/93 已完成 checkbox/radio；next94/TEST65 已确认单行输入；next97/TEST66 已确认 textarea；next98/TEST67 已确认 select；next99/TEST68 已确认 NetSurf button gadget、successful-controls 编码与 WM GET/URL-encoded POST；next101/TEST69 已确认 reset、Enter 和 label；next103/TEST70 已确认 file gadget、multipart successful-controls、二进制 body 和 reset 清理。 | multipart 会在 WM 宿主内整体缓冲请求体，文件 MIME 固定为 `application/octet-stream`；尚无流式上传、上传方向进度、MIME 推断、`multiple` 文件、WM 多选列表、约束验证和完整事件系统。自动桥断言也不等于控件外观、真实手指操作、原生文件选择器或公网 POST 端点已人工验收。 |
