...
| table span/归一化/折叠边框 | NetSurf 3.11 span occupancy 与匿名 row/cell 已由 TEST46/47 验收。next64/TEST53 使用上游 `table.c` 原生算法自动检查 cell/row/row-group/table 的宽度、样式、hidden、来源和方向 tie，并保留 separate 对照，待设备验收。 | 尚不覆盖任意 inline/float/form 畸形组合、caption/column 归一化、`col`/`colgroup` border 来源或所有复杂 span 边界。 |
