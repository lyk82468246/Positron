...
| 图片 | TEST18-20 已确认 WM Imaging 四格式缓存链；TEST25-29/13 已确认 SVG parse/draw/cache/fallback/fill-rule 与网络相对 SVG 正式链。 | SVG text、CSS 背景图或任意网络图片均已通过。 |
...
- **当前结论**：BMP/PNG/JPEG/GIF 四格式与 TEST20 缓存 `<img>` 已由设备视觉确认。TEST25-29/13 已真机确认 SVG parse、公共 DLL retained object、NanoSVG 抗锯齿 draw、缓存/fallback、真实网络相对资源与 nonzero/evenodd。TEST30 已构建待设备：单一 CSS background-image 使用原始尺寸，支持 repeat/position 并按 URL 去重；background-size、多层背景和资源异步事务未实现。SVG text 尚未验证。单次栅格源缓冲限制为 1,048,576 像素，超大输出会降低内部采样分辨率后再缩放。
