...
| Forms/widgets | next85/93 已完成 checkbox/radio 静态 redraw 与交互；next94/TEST65 已确认单行 text/password；next97/TEST66 已确认 textarea；next98/TEST67 已确认 select option/core 状态 ABI 与 WM 单选 `COMBOBOX`。 | 尚不含 WM 多选列表、label 激活、button、表单编码、提交或完整焦点/事件系统；自动桥断言也不等于下拉展开外观和真实手指操作已人工验收。 |
