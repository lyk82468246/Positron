...
**next204 已验收边界（2026-08-12）**：`TEST999` 只是显式选择后的序列完成提示，不是进程
...
**next208 基线边界（2026-08-12）**：根相对 URL 只新增单个内嵌完整 `/%2e/` segment；末尾
编码点段、多个编码点段、`%2E%2E`、字面/编码 double-dot 与混合字面/编码点段仍不规范化。
...
**next210 基线边界（2026-08-12）**：只新增绝对 URL 中多个内嵌完整 `/%2e/` segment；末尾
混合编码点段、`%2E%2E`、字面/编码 double-dot 与根相对重复编码仍不规范化。TEST173 中
...
**next212 基线边界（2026-08-12）**：只新增绝对 URL 中单个内嵌完整 `/%2e%2e/` segment，
且只在存在一个非空前驱目录时连同该目录折叠。根相对、末尾、重复或混合 double-dot，
`.%2e`/`%2e.` 拼写，以及规范化后不同 query/path 仍不进入同文档队列。本批不宣称完整 URL
...
**next214 基线边界（2026-08-12）**：只新增绝对 URL path 末尾的单个完整 `/%2e%2e`
segment，且只在存在一个非空前驱目录时连同该目录折叠并保留 `/`。根相对末尾、重复或混合
double-dot、`.%2e`/`%2e.` 拼写，以及规范化后不同 query/path 仍不进入同文档队列。本批不
宣称完整 URL Standard parser。C89、ARMV4I Debug 构建和 staging 哈希已通过，33 项定向
设备日志得到零 ERROR/FAIL 与最终 PASS；ini 保持 1357 字节。

**next215 基线边界（2026-08-12）**：只新增根相对 URL path 末尾的单个完整 `/%2e%2e`
segment，且只在存在一个非空前驱目录时连同该目录折叠并保留 `/`。重复或混合 double-dot、
`.%2e`/`%2e.` 拼写，以及规范化后不同 query/path 仍不进入同文档队列。本批不宣称完整 URL
Standard parser。C89、ARMV4I Debug 构建和 staging 哈希已通过，34 项定向设备日志得到零
ERROR/FAIL 与最终 PASS；ini 保持 1357 字节。

**next216 基线边界（2026-08-12）**：只新增绝对 URL 中单个内嵌完整 `/.%2e/` 或 `/%2e./`
segment，且只在存在一个非空前驱目录时连同该目录折叠。根相对、末尾、重复、与完整编码
double-dot 混合，以及规范化后不同 query/path 仍不进入同文档队列。本批不宣称完整 URL
Standard parser。C89、ARMV4I Debug 构建和 staging 哈希已通过，131 项全量设备日志得到零
ERROR/FAIL 与最终 PASS；ini 为 1294 字节。

**next217 基线边界（2026-08-12）**：只新增根相对 URL 中单个内嵌完整 `/.%2e/` 或 `/%2e./`
segment，且只在存在一个非空前驱目录时连同该目录折叠。末尾、重复、与完整编码 double-dot
混合，以及规范化后不同 query/path 仍不进入同文档队列。本批不宣称完整 URL Standard
parser。C89、ARMV4I Debug 构建和 staging 哈希已通过，36 项定向设备日志得到零 ERROR/FAIL
与最终 PASS；ini 为 1278 字节。

**next218 基线边界（2026-08-12）**：只新增绝对 URL path 末尾的单个完整 `/.%2e` 或
`/%2e.` segment，且只在存在一个非空前驱目录时连同该目录折叠并保留 `/`。根相对末尾、
重复、与完整编码 double-dot 混合，以及规范化后不同 query/path 仍不进入同文档队列。本批
不宣称完整 URL Standard parser。C89、实际 JavaScript 探针、ARMV4I Debug 构建和 staging
哈希已通过，37 项定向设备日志得到零 ERROR/FAIL 与最终 PASS；ini 为 1278 字节。

**next219 基线边界（2026-08-12）**：只新增根相对 URL path 末尾的单个完整 `/.%2e` 或
`/%2e.` segment，且只在存在一个非空前驱目录时连同该目录折叠并保留 `/`。重复、与完整编码
double-dot 混合，以及规范化后不同 query/path 仍不进入同文档队列。本批不宣称完整 URL
Standard parser。首包的 20,991 字符 DOM bootstrap 在 TEST162 触发既有 1000ms 超时；修正版
以共享 helper 合并重复分类，降至 19,735 字符，不放宽预算。C89、14 组实际 JavaScript 探针、
ARMV4I Debug 构建和 staging 哈希已通过；修正版 38 项定向设备日志得到 37 条标准数字 OK、
1 条 TEST13 overview、零 ERROR/FAIL 与最终 PASS；ini 为 1278 字节。
...
**next198 父目录后多位置内嵌 `./` 片段 URL 基线（2026-08-12）**：连续前导 `../` 逐级上移后，
允许余下路径多个位置出现内嵌 `./` 的 href/assign/replace URL；只有移除这些单点段并解析后
path/query 与当前基址完全相同且改变 fragment，或当前确有 fragment 时用匹配目标清除它，才走
同 document 队列。当前无 fragment 的同 URL、不同 path/query 和内嵌 `../` 目标保持普通导航。
TEST166 覆盖三入口、清除、same-value、history/state、hashchange、无网络与分类边界。无父目录
前缀的内嵌点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批。C89、ARMV4I
Debug 构建及 `C:\WMShare\Positron-next198` 七个二进制 SHA-256 核对已通过。240x240/96 DPI
日志得到 TEST166 OK、配置所选 113 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13 使用
`OK (overview)`，其余 112 项使用标准数字 OK 行。

**next199 根相对单个内嵌 `./` 片段 URL 基线（2026-08-12）**：根相对 href/assign/replace URL 允许
路径中出现一个内嵌 `./`；只有移除该单点段并解析后 path/query 与当前基址完全相同且改变
fragment，或当前确有 fragment 时用匹配目标清除它，才走同 document 队列。当前无 fragment 的
同 URL、不同 path/query、连续内嵌 `././` 和内嵌 `../` 目标保持普通导航。TEST167 覆盖三入口、
清除、same-value、history/state、hashchange、无网络与分类边界。根相对多个点段、绝对 URL
点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批。C89、ARMV4I Debug
构建及 `C:\WMShare\Positron-next199-fix` 七个二进制 SHA-256 核对已通过。原共享目录 fixture
...
**next201 根相对多位置内嵌 `./` 片段 URL 基线（2026-08-12）**：根相对 href/assign/replace URL 允许
多个分离路径位置出现内嵌 `./`；只有移除这些点段并解析后 path/query 与当前基址完全相同且改变
fragment，或当前确有 fragment 时用匹配目标清除它，才走同 document 队列。当前无 fragment 的
同 URL、不同 path/query、绝对 URL 点段和内嵌 `../` 目标保持普通导航。TEST169 覆盖三入口、
清除、same-value、history/state、hashchange、无网络与分类边界。绝对 URL 点段、内嵌父目录、
百分号标准化、锚点滚动和其他组件 setter 不在本批。首包因 TEST167 的过期负边界停止；修正版
保持 TEST169 正向断言并把 TEST167 负边界换成绝对 URL 点段。C89、ARMV4I Debug 构建及
`C:\WMShare\Positron-next201-fix` 七个二进制 SHA-256 核对已通过。640x480/192 DPI 日志得到
TEST70、TEST167、TEST168 与 TEST169 OK、配置所选 116 项全部 OK、零 ERROR、零 FAIL 与最终
PASS；TEST13 使用 `OK (overview)`，其余 115 项使用标准数字 OK 行。

**next202 绝对 URL 单个内嵌 `./` 片段导航基线（2026-08-12）**：绝对 href/assign/replace URL 允许路径
中出现一个内嵌 `./`；只有移除该点段后 origin/path/query 与当前基址完全相同且改变 fragment，
或当前确有 fragment 时用匹配目标清除它，才走同 document 队列。当前无 fragment 的同 URL、
不同 path/query、连续内嵌 `././` 和内嵌 `../` 目标保持普通导航。TEST170 覆盖三入口、清除、
same-value、history/state、hashchange、无网络与分类边界。绝对 URL 连续/多位置点段、内嵌父目录、
百分号标准化、锚点滚动和其他组件 setter 不在本批。C89、ARMV4I Debug 构建及
...
hashchange、无 GET；next206 日志当时还覆盖重复编码点段、`%2E%2E`、不同 query/path 与
`..` 排除边界。next210 基线只取代重复 single-dot 的旧反向断言。默认
...
**next208 根相对 URL 编码单点段基线（2026-08-12）**：片段分类器新增根相对单个内嵌
`%2e` segment；TEST175 覆盖三入口、清除、same-value、state/length、hashchange、无 GET、
重复编码点段、`%2E%2E`、不同 query/path 与 `..` 排除边界。默认 javascript=0、TEST13、
...
**next210 绝对 URL 重复编码单点段基线（2026-08-12）**：片段分类器遍历多个内嵌 `%2e`
segment；TEST177 覆盖三入口、清除、same-value、state/length、hashchange、无 GET、不同
query/path 与父目录排除边界。TEST173 不再反向断言重复编码；`%2E%2E`、混合内嵌/末尾编码和
...
**next212 绝对 URL 编码双点段基线（2026-08-12）**：片段分类器新增单个内嵌 `%2e%2e`
segment 的前驱目录折叠；TEST179 覆盖三入口、清除、same-value、state/length、hashchange、
无 GET、不同 query/path、额外父目录边界和 14/16 callback 槽位。根相对、末尾、重复、混合
...
**next214 绝对 URL 末尾编码双点段基线（2026-08-12）**：片段分类器新增绝对 path 末尾
`%2e%2e` segment 的前驱目录折叠；TEST181 覆盖三入口、清除、same-value、state/length、
hashchange、无 GET、不同 query/path、混合 single/double、重复 double-dot、字面父目录和
14/16 callback 槽位。根相对末尾和 `.%2e`/`%2e.` 仍不支持。默认 javascript=0、TEST13、
core ABI 和 callback 数不变；C89、ARMV4I Debug 构建与 `C:\WMShare\Positron-next214` 七个
二进制哈希已通过。定向门选择 TEST13/151-181/999，共 33 项；ini 为 1357 字节且设备日志
全部通过，next211 仍是最近全量检查点。

**next215 根相对 URL 末尾编码双点段基线（2026-08-12）**：片段分类器新增根相对 path 末尾
`%2e%2e` segment 的前驱目录折叠；TEST182 覆盖三入口、清除、same-value、state/length、
hashchange、无 GET、不同 query/path、混合 single/double、重复 double-dot、字面父目录和
14/16 callback 槽位。`.%2e`/`%2e.` 仍不支持。默认 javascript=0、TEST13、core ABI 和
callback 数不变；C89、ARMV4I Debug 构建与 `C:\WMShare\Positron-next215` 七个二进制哈希
已通过。定向门选择 TEST13/151-182/999，共 34 项；ini 为 1357 字节，设备日志全部通过，
next211 仍是最近全量检查点。

**next216 绝对 URL 半编码双点段基线（2026-08-12）**：片段分类器新增绝对 URL 中单个内嵌
完整 `.%2e` 或 `%2e.` segment 的前驱目录折叠；TEST183 覆盖两种拼写、三入口、清除、
same-value、state/length、hashchange、无 GET、不同 query/path、混合完整/半编码 double-dot、
重复半编码 double-dot、字面父目录和 14/16 callback 槽位。根相对和末尾半编码 double-dot
仍不支持。默认 javascript=0、TEST13、core ABI 和 callback 数不变；C89、ARMV4I Debug 构建
与 `C:\WMShare\Positron-next216` 七个二进制哈希已通过。全量门选择
TEST13/20/27/43/44/56/58-77/80-183/999，共 131 项；ini 为 1294 字节；
`screen=240x320 dpi=96` 设备日志全部通过，next216 是最近全量检查点。

**next217 根相对 URL 半编码双点段基线（2026-08-12）**：片段分类器新增根相对 URL 中单个
内嵌完整 `.%2e` 或 `%2e.` segment 的前驱目录折叠；TEST184 覆盖两种拼写、三入口、清除、
same-value、state/length、hashchange、无 GET、不同 query/path、混合完整/半编码 double-dot、
重复半编码 double-dot、字面父目录和 14/16 callback 槽位。末尾半编码 double-dot 仍不支持。
默认 javascript=0、TEST13、core ABI 和 callback 数不变；C89、ARMV4I Debug 构建与
`C:\WMShare\Positron-next217` 七个二进制哈希已通过。定向门选择 TEST13/151-184/999，共
36 项；ini 为 1278 字节，设备日志全部通过，next216 仍是最近全量检查点。

**next218 绝对 URL 末尾半编码双点段基线（2026-08-12）**：片段分类器新增绝对 path 末尾
完整 `.%2e` 或 `%2e.` segment 的前驱目录折叠；TEST185 覆盖两种拼写、三入口、清除、
same-value、state/length、hashchange、无 GET、不同 query/path、混合完整/半编码 double-dot、
重复半编码 double-dot、字面父目录和 14/16 callback 槽位。根相对末尾半编码 double-dot 仍
不支持。默认 javascript=0、TEST13、core ABI 和 callback 数不变；C89、实际 JavaScript 探针、
ARMV4I Debug 构建与 `C:\WMShare\Positron-next218` 七个二进制哈希已通过。定向门选择
TEST13/151-185/999，共 37 项；ini 为 1278 字节，设备日志全部通过，next216 仍是最近全量检查点。

**next219 根相对 URL 末尾半编码双点段基线（2026-08-12）**：片段分类器新增根相对 path
末尾完整 `.%2e` 或 `%2e.` segment 的前驱目录折叠；TEST186 覆盖两种拼写、三入口、清除、
same-value、state/length、hashchange、无 GET、不同 query/path、混合完整/半编码 double-dot、
重复半编码 double-dot、字面父目录和 14/16 callback 槽位。默认 javascript=0、TEST13、
core ABI 和 callback 数不变。首包在 TEST162 遇到 bootstrap 1000ms 超时，未运行 TEST186/999；
修正版用共享 `ppartial` helper 把 bootstrap 从 20,991 降至 19,735 字符。C89、14 组实际
JavaScript 探针、ARMV4I Debug 构建与 `C:\WMShare\Positron-next219-fix` 七个二进制哈希已
通过。定向门选择 TEST13/151-186/999，共
38 项；ini 为 1278 字节，修正版设备日志得到 37 条标准数字 OK、1 条 TEST13 overview、
零 ERROR/FAIL 与最终 PASS，next216 仍是最近全量检查点。
