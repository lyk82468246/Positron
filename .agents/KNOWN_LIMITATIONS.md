...
| 字体 fallback | next58 随包部署约 814 KiB 的静态 Positron Symbols/Emoji（来自 Noto OFL），并统一 GDI 测量、换行命中与绘制 run；补充平面 emoji 经生成的 BMP PUA 别名送入 WM6 GDI。 | TEST49 待设备验收；没有复杂 ZWJ/variation shaping、彩色 emoji、网页 `@font-face`、字体下载或通用语言字体 fallback。 |
