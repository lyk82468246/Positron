...
- **下一步**：用户已再次确认 next37/`9c5c7c7` 的 TEST13 完整正常。仅把 core/host 改成语义兼容仍不足；失败包继续装载了扩大响应结构并设置 WinInet timeout options 的新版 HTTP DLL。当前 HTTP 源码与响应 ABI 已逐行恢复到 9c，先复测 TEST13，再逐项重新设计重定向 origin。Grid/gap 仍须审计可移植上游实现，float 也不能复用 TEST23 的简化断言。
...
- **资源预算**：`test_host` 最多暂存 64 个去重 URL、合计 2 MiB 原始字节，成功提交时 core 会复制所需数据后立刻释放事务。可选资源阶段 20 秒总预算、WinInet 15 秒 options 和 TLS deadline 均未解决 TEST13，现已撤销。上述值都是可替换的宿主/传输策略，不是 `positron_core` ABI 或最终页面的硬上限；网络请求目前仍没有可靠的强制取消和完整 deadline。
...
- **stylesheet/base 边界**：普通 Ex2 API 继续实现 metadata 和首个 `<base href>` 语义，TEST46-47 已由设备确认。真实 Browse 为恢复已验收稳定性，暂用 `PCORE_STYLE_COMPAT_9C5C7C7`、旧图片扫描和旧链接命中：不采用 base/redirect origin，`rel` 精确匹配，并忽略 stylesheet 的 type/disabled/media 元数据。`PHttpResponse.effective_url` 与 TEST48 已撤回；此妥协只属于 WM Browse 宿主，不降低公共 Ex2 API。
