...
| Forms/widgets | next85/93 至 next104/TEST71 已完成 checkbox/radio、文本、textarea、single/multiple select、button、GET/POST、reset/Enter/label 与 multipart/file；next106/TEST72 已确认首批 `required/valueMissing`；next109/TEST73 已确认动态 `:focus/:active/:checked/:enabled/:disabled` 重样式。 | multipart 仍整体缓冲且 MIME 固定；尚无流式上传、上传进度、MIME 推断、multiple file、`pattern`、类型/范围约束、custom validity、`invalid` 事件、验证气泡和完整 DOM 事件传播/取消/default-action。空且无 CSS 尺寸的 text input 也缺浏览器默认 intrinsic size。自动断言不等于真实手指、原生选择器或公网 POST 已人工验收。 |
...
| Forms/widgets 状态边界 | next109 把宿主维护的 focus/active 节点与 live checked/selected/disabled 状态交给 libcss callback，并在交互后执行 cache-only restyle。 | 这不是 DOM FocusEvent/MouseEvent：没有事件对象、捕获/冒泡、取消、默认动作、hover/visited/target/indeterminate，也不保证所有控件具备浏览器默认 intrinsic size。 |
