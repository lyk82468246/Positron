...
- `test_host.exe` 已经是公共 DLL 的消费者；next607 已将程序化 `HTMLElement.click()` 的
  ...
  到 input、dirty tracking 和 blur/change 顺序迁入同一 DLL。仍保存 native SELECT、WM
  控件消息、composition 生命周期和平台副作用胶水，必须逐项区分产品语义与宿主副作用，
  不能笼统搬迁。
...
- `PBrowser_ScriptSessionRegisterProgrammaticClickCallbacksEx()` 只迁移程序化表单激活的
  ...
  生命周期和 OEM SIP/IME 行为仍由宿主提供，下一批优先审计 SELECT 键盘纵切，不把 OEM
  行为误标为产品兼容性。
