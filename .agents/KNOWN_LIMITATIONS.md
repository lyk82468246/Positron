...
- **独立消费与编码**：next46 已确认只导入 `positron_image.dll`/`COREDLL.dll` 的 ABI 1.0 示例横竖屏工作；其 SVG 被缩放到较窄目标矩形后曲线略粗，用户判定可接受，不改变 TEST26/27 的抗锯齿基线。next47 又确认 ABI 1.1 的 PNG/JPEG 内存编码、释放输出后重新解码与 retained SVG 均能闭环；PNG 与源 BMP 正常，但设备默认 JPEG 在 16x16 高饱和四象限上有明显紫/黄色度串扰。next48 的 WM quality=100 只有有限改善，不能称为修复。ABI 1.2 保留 quality=-1 的 WM 默认兼容路径，显式 0..100 切到静态 libjpeg-turbo 1.5.3 压缩器并强制 4:4:4；next49 已确认行方向、颜色和 SOF 采样均正确。该库只接收 WM 已解码的 24bpp 像素，不承担不可信 JPEG 解码；ARMV4I 构建无 SIMD，且显式编码会额外生成约 `width*height*3` 字节的 24bpp 中间位图。next49 Debug DLL 比 next48 增加约 238 KiB，但静态 `.lib`、源码和独立 JPEG DLL 均不部署。ABI 1.3 的复制式 BGR24/BGRA32 原始像素入口及 alpha PNG 回读待 next50 验收；GIF/BMP 输出和跨线程句柄仍未提供。
