...
- `details`/`summary`、`hidden` 等只有受限静态或交互子集；`dialog` 已有 Browser 脚本的 show/showModal/close/requestClose、returnValue、cancel/close 事件、活动 modal id 和宿主驱动的 Escape 请求桥接。宿主可以组合 Core 的 scoped focus snapshot 实现顺序 Tab/Shift+Tab 子树范围，但没有完整 top layer、backdrop、背景点击策略或跨文档生命周期。Browser 不自动接管平台消息；宿主必须显式调用这些边界。
