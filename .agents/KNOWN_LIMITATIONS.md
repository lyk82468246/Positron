...
| 列表 marker | next57/59 已确认基础 marker 与字体；next61/TEST50 已确认 libcss 上游 47 种 counter formatter、document-cache `list-style-image` 与失败类型回退。next62 已实现 inline-first `list-style-position:inside` 首行占位和悬挂换行，待 TEST51 设备验收。 | 不代表复杂 block-first/float inside marker、自定义 `@counter-style`、所有语言字体或完整 CSS Lists。 |
