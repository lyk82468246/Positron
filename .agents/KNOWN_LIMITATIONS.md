...
- `positron_browser.dll` 已拥有独立 history/session 产品层、PScript context、host JSON callback
  ...
  （defaultValue/defaultChecked/selectedIndex）、navigation、同文档 location/history 事件分发、event JSON 分发、native input/composition、keyboard、focus-family 和 SELECT input/change typed dispatch entry；显式开启时仍有 classic inline/external
...
validation、keyboard/focus-family/SELECT input/change typed dispatch 和部分 composition bridge；WM 控件与 core 事件传播仍由宿主负责。
...
next250 的 TEST217、`TEST112-135,137-152,189-217,999`（70 项）定向门和
`TEST13/20/27/43/44/56/58-77/80-217/999`（165 项）全量门验证 SELECT input typed dispatch
contract；next249 的 SELECT change、next248 的 focus-family、next247 的 native keyboard、next246 的 native input/composition contract 保持通过；其余 form/input callback 实现、
