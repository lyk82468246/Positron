...
| 图片 | TEST18-20 已确认 WM Imaging 四格式缓存链；TEST25-27 已确认 SVG parse、抗锯齿 draw 与缓存 `<img>` 正式链。 | TEST28 fallback、TEST13 网络 SVG、背景图或任意网络图片均已通过。 |
...
- **当前结论**：BMP/PNG/JPEG/GIF 四格式与 TEST20 缓存 `<img>` 已由设备视觉确认。TEST25-27 已真机确认 SVG parse、公共 DLL retained object、NanoSVG 抗锯齿 draw 和缓存 `<img>` 正式链。TEST28 损坏 SVG fallback 已构建待设备；TEST13 新增仓库托管的网络 HTML + 相对 SVG fixture。真实网络结果、复合路径孔洞/非零 winding、SVG text 和 CSS background image 尚未验证。单次栅格源缓冲限制为 1,048,576 像素，超大输出会降低内部采样分辨率后再缩放。
