...
- **下一步**：先确认扩展 TEST21，再用 TEST13 判断响应式规则是否实际改善页面。随后仍须对比上游 `box_construct.c` 的 float 前后处理、匿名盒 normalisation 与 list marker，并独立处理 SVG/custom properties，而不是复用 TEST23 的简化断言。
