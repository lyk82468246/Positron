...
- `test_host.exe` 已经是公共 DLL 的消费者；next607 已将程序化 `HTMLElement.click()` 的
  ...
  DLL，next611 再将单选下拉的 begin/candidate/confirm/cancel 事务状态迁入同一 DLL。仍由
  宿主持有 native SELECT 的 WM 键盘/控件默认动作、原生下拉窗口与视觉、Core selection
  mutation、取消回滚、composition 生命周期和平台副作用胶水，必须逐项区分产品语义与
  宿主副作用，不能笼统搬迁。
...
- `PBrowser_ScriptSessionRegisterProgrammaticClickCallbacksEx()` 只迁移程序化表单激活的
  ...
  以及 next611 的单选下拉事务闸门；WM SELECT 键盘默认动作、下拉窗口/视觉、Core selection
  mutation、原生取消回滚和 OEM SIP/IME 行为仍由宿主提供，不把 OEM 行为误标为产品兼容性。
...
- URL 解析、相对地址解析和 history/navigation 只覆盖项目需要的子集，不是完整 WHATWG URL Standard。
- 没有完整页面缓存、持久历史、跨进程恢复、滚动位置恢复、表单状态恢复和 POST 重提交模型。
- 导航是否实际发起请求，以及窗口如何替换，仍需要宿主网络和窗口层参与。
- 非法 URL、旧设备编码、重定向链、特殊 scheme 与同源策略的边缘行为覆盖有限。
