...
| table span/归一化/折叠边框 | NetSurf 3.11 span occupancy 与匿名 row/cell 已由 TEST46/47 验收。next64/TEST53 的基本冲突已由纵横屏确认；next65/TEST54 又确认 rowspan 终止 row、`rowspan=0`、colspan 及 row-group 共享边。next67/TEST55 正验证 cell vertical alignment 与 separated `empty-cells`。 | 尚不覆盖任意 inline/float/form 畸形组合、caption/column 归一化、`col`/`colgroup` border 来源、跨行 baseline 或所有复杂表格边界。 |
