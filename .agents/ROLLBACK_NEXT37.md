...
- 短期：冻结 Browse 导航路径；next45 已确认 retained 位图 ABI 与 TEST13 集成门槛，next46 已确认 ABI 1.0 独立 WM 消费，next47 已确认 ABI 1.1 PNG/JPEG 内存编码闭环并暴露默认 JPEG 串色，next48 又确认 WM quality=100 改善有限，next49 已确认静态 libjpeg-turbo 4:4:4 后端修复大面积色带。当前仅在独立图片 DLL 扩展 ABI 1.3 原始像素入口，next50 不要求重跑 Browse。
