...
- BMP/PNG/JPEG/GIF 的 WM Imaging 可见绘制已确认；2026-07-12 用户确认 TEST20 的四格式缓存 `<img>` 正式 NetSurf 链在横竖屏均颜色、顺序一致，位图格式覆盖闭环完成。背景图、SVG 仍未接通；解码失败或缓存未命中时 `<img>` 继续显示 alt/src 文本占位。
- SVG 使用仓库内 NetSurf `libsvgtiny`，不另写解析器。`positron_libsvgtiny.vcproj` 已建立并由 VS2008 ARMV4I 编译通过；缺失的 `autogenerated_colors.c` 由 `scripts/gen_svgtiny_colors.py` 从上游 `colors.gperf` 可重复生成。
- libsvgtiny 的真正 parse/link 仍依赖当前尚未构建的 libdom XML binding，因此此刻只能称为“静态库编译基线”，不能称为 SVG 已解析或已显示。
...
5. 接通 libdom XML binding，使 libsvgtiny 完成内存 SVG 解析；再将 path/shape 映射到 GDI 并接入缓存 `<img>`。
6. 图片能力采用两层边界：上游解析库保持静态 `.lib`；新增稳定的 `positron_image.dll` 统一封装 WM Imaging 与 libsvgtiny，供 `positron_core.dll` 和其他 WM 程序调用。对象必须由同一 DLL 创建/释放，不能暴露 NetSurf 内部结构或跨 CRT 所有权；现有 `PCore_Image*` 暂保留为兼容转发。
