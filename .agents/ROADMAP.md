...
当前待验收候选为 next154：显式脚本 context 新增 WM_SYSKEYDOWN/UP 与 ASCII WM_SYSCHAR
事件桥，TEST120 尚未经过设备和人工视觉门禁；IME/composition 仍未实现。
...
桥已在设备通过 TEST112-119，但不能把它与完整浏览器 JavaScript 混为一谈。next154
候选的 WM_SYSKEY/WM_SYSCHAR 桥只在显式 `javascript=1` 下生效，等待设备验收；
...
### 6z. next154：原生 WM_SYSKEY/WM_SYSCHAR 事件（设备待验收）
...
- `c89ize.py`、C89 回归、仓库审计和 VS2008 ARMV4I Debug 增量构建已通过；设备包与
  `screen`/DPI 轮换验收待运行。默认 `javascript=0`、TEST13 网络路径和 next153
  已验收行为保持不变。
...
1. 以 next153 的 TEST13/20/27/43/44/56/58-77/80-119 设备日志作为已验证自动化基线；
   next154 的 TEST120 先做设备验收，再决定是否提升为基线；后续每批继续以 TEST13
   深层导航和旋转作为浏览器门禁。
2. 在显式开关默认关闭期间不得让 TEST13 平白增加脚本网络请求；WM_CHAR keypress
   已完成设备门禁，next154 的 WM_SYSKEY/WM_SYSCHAR 候选确认后再评估 IME/composition。
