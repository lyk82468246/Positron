...
并在 Core 与 DOM write Ex14 增加按未过滤 `childNodes` 索引创建 live CDATA 的入口。wrapper
复用 CharacterData 的 data/offset、clone、primitive relative、remove/reinsert 和单一
existing source move/replace；TEST1284 与 `1284,999` Debug ARMV4I 外置卡自动门通过，
双空间预检、完整日志回收、清理和 crash check 均通过；证据与预算见 `.agents/HANDOFF.md`
和 `docs/TESTING.md`。

下一批（next845）的选择必须先从 compatibility corpus、源码、设备日志或截图固定一个新的、可
