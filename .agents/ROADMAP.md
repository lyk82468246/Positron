...
结构 mutation 目前承诺七条窄路径：带 id 元素的单值 `Element.append()`/`prepend()` 文本插入，
...
`Element.append()`/`prepend()` 位置插入和目标相对的单值 primitive 文本插入，以及 Ex7 的
`Element.replaceWith(value)` 原位 primitive 文本替换。前四条分别由
  write Ex6、Ex2、Ex3 和 Ex4 复用既有
...
并允许同父级重排与跨父级迁移；Ex7 在 old element 的 direct-parent childNodes 位置创建
新的 Text，保留旧 wrapper 为 detached。七条路径都由 Core 拥有 DOM 语义并在成功后使
retained layout 失效。DocumentFragment、Text/Comment/
