...
下一条纵向能力是 next674：在资源终态已经可区分之后，只对明确可重试的 transport 失败建立固定次数的重试，并在预算耗尽后保持 `failed` 终态。HTTP、resolve、budget、memory 和 cancelled 不得被误重试；同一候选的资源计数、响应释放、旧页保留和 generation 提交规则必须保持一致。

next674 的语料应固定一次可恢复的 transport 失败、一次预算耗尽和一次不可重试 HTTP/resolve 结果，自动断言每项尝试次数、最终分类、重试预算、回收路径和最终提交。不得把无限退避、阻塞 socket 的强制中断或外网端点写入契约；真实网络仍只作为独立哨兵。通用资源结果若暴露公共语义缺口，应先进入 Core/HTTP，再由宿主负责 WM 日志与调度。

完成 next673 后，再按风险累计条件安排更宽回归或新的全量 checkpoint；不要把一次全量门当成永久覆盖，也不要为增加测试编号拆分能力。
