...
24. **stylesheet 元数据批次已完成并真机验收**：内联 `<style>` 与外链 `<link>` 现把完整 `media` 字符串交给 libcss 的 selection context；`rel` 按 ASCII 空白 token 识别 stylesheet 并排除 alternate，非空 `type` 仅接受 `text/css`，带 `disabled` 的表不解析也不 fetch。TEST46 在 320/299px 同时验证内联/外链 media 切换、两个合法 URL 仅 fetch/free 一次、alternate/非 CSS/disabled 三个 URL 零 fetch，以及 cache-only 重选；2026-07-14 用户确认整个批次通过。
25. **文档 base URL 批次已完成当前验收**：新增共享首个 `<base href>` 解析器，仍由宿主 resolver 执行 URL 规范化，core 不绑定 WinInet。`PCore_StyleDocumentEx2` 现让外链 CSS、嵌套 import 与 CSS URL 使用有效 document base；新增兼容扩展 `PCore_FetchImageResourcesEx2` 与 `PCore_LinkAtEx2`，分别处理图片 URL/原始别名和点击链接绝对化，旧 API 保留原始 URL 行为。图片缓存用别名链覆盖多个相对写法映射到同一规范 URL，避免重复 fetch 且保证 NetSurf 构盒仍能按原始 `src` 取到字节。TEST47 离线覆盖首个 base 优先、第二个 base 忽略、CSS/背景/两个等价图片引用、原始/绝对链接和 fetch/free/去重；2026-07-14 用户确认全部通过。
26. **重定向 origin 与导航活性批次第二轮待设备验收**：TEST13 的 `example.com` Learn more 实际经历 `https://iana.org/domains/example` 到 `http://www.iana.org/help/example-domains` 的跨 scheme/host 重定向。第一轮让 HTTP DLL 返回 `effective_url`、修正 document origin、为 WinInet/资源阶段加入有限等待并合并加载期滚动重绘；用户复测确认 TEST13 仍可长期不提交，说明该修复不足。审计发现第一跳 HTTPS 仍经完全阻塞的 `positron_tls`，此前预算无法中断 TCP connect、TLS handshake 或 read/write。第二轮按 vendored mbedTLS nonblocking 模式和 WinCE Winsock 契约使用 `ioctlsocket(FIONBIO)` + `select`，新增 `PTls_ConnectWithTimeout`/`PTls_ConnectVerifiedWithTimeout` 兼容 API；旧 API 使用 30 秒默认值，HTTP 使用 15 秒。TEST48 离线覆盖最终 origin，TEST13 验收应同时高频拖动旧页滚动条。同步 `gethostbyname` 与外部主动取消仍不受该 deadline 覆盖。默认设备批次为 TEST13/43/44/47/48。
