...
### 6d. next121：独立 JavaScript 模块生命周期（待设备验收）
...
- next121 已完成 VS2008 ARMV4I Debug/Release 增量构建和 `C:\WMShare\Positron-next121` staging；设备验收通过前，自动化已验证基线仍是 next120，TEST13 不因该批增加脚本请求。
...
1. 以 next120 / TEST80-82 加 next114 Browse 门禁作为当前已验证自动化基线；next121 的 TEST83 设备验收完成后再提升基线，后续每批继续以 TEST13 深层导航和旋转作为浏览器门禁。
2. 在 TEST83 通过的基础上，评估脚本模块的宿主源提供器，再评估把独立 DLL 接入显式的浏览器 JavaScript 开关；在 JS 默认关闭期间不得让 TEST13 平白增加脚本网络请求。
