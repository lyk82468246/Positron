...
26. **重定向 origin 与导航活性批次已挂起并回退 Browse 接入**：`effective_url`、base-aware Ex2 图片/链接 API 与 TEST46-48 均保留，独立测试也正常；但从已完整验收的 `9c5c7c7` 之后把它们组合进 TEST13，导致 `Open example` -> `Learn more` 一分钟以上不提交。20 秒资源预算、加载时合并 `UpdateWindow` 和 `1e06105` TLS deadline 尝试均未改变设备故障，现已撤回。真实 Browse 通过新增 Ex3 flag 显式恢复 9c stylesheet 发现规则，并恢复旧图片扫描、旧链接命中、原请求 origin 和同步滚动重绘；TEST46 同时断言现代 Ex2 与 9c 兼容契约。下一步先设备验收 TEST13/46，再把 metadata、base、redirect origin 每次只接回一项并跑完整 TEST13。
