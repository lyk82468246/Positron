...
- 依据实际流程在已有有界 `dialog` 脚本生命周期、`method="dialog"` 默认动作、活动 modal id、Escape 请求桥接、宿主顺序 Tab 子树范围和 backdrop 指针策略之上继续补 top-layer/backdrop 视觉、跨文档生命周期或 contenteditable 等缺口；保持已实现的有界 `tabindex` 排序与 Core/宿主事务边界。
