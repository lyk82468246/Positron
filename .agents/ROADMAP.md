...
26. **重定向 origin 与导航活性批次已挂起并恢复 next37 HTTP ABI**：用户再次运行 next37，确认 TEST13 完整正常。失败的 next42 虽已让 core/host 走 9c 兼容路径，仍装载 20,480 字节新版 HTTP DLL；next37 为 19,968 字节。现已把 `positron_http.c/.h` 逐行恢复到 `9c5c7c7`，撤销 1536 字节 `effective_url` 字段、WinInet timeout options 和 TEST48，产物尺寸恢复。Ex2 base-aware core API 与 TEST46-47 保留。下一步先验收单独 TEST13；重定向 origin 必须改成独立、低内存 API 后再逐项接入，不能再次扩大每个响应对象并一次合并多条导航路径。
