...
21. **NetSurf overflow scrollbar 已接入、待 TEST42 真机验收**：移植并 C89 化上游 `desktop/scrollbar.c`，恢复 `descendant_x1/y1` 溢出判定、`box_handle_scrollbars` 创建/更新/成对销毁、祖先 scroll offset 坐标和 GDI redraw。公开 `PCore_OverflowPointer` 把 WM 的 DOWN/MOVE/UP 转发给箭头、page well 与 thumb drag；TEST42 先离屏绘制并自动断言点右箭头后首个 cell 左移 16px，再打开可见页面合并检查箭头、拖动和旋转。VS2008 ARM 增量构建已 0 错误；尚不宣称触摸惯性、overlay scrollbar 或完整 Grid。
