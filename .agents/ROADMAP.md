...
6. **首版设备视觉失败，替代实现已构建、待复测**：首版 libnsfb 固定 30 点 + GDI Polygon/Polyline 的几何正确，但蓝色 cubic 有明显阶梯边缘。现保留 libsvgtiny 作为解析器，将其路径适配到固定提交的 NanoSVG 5 倍子像素软件栅格器，再用预乘 BGRA DIB 和 WM `AlphaBlend` 输出。增强 TEST26 除红/绿内部像素外还必须找到蓝色部分覆盖边缘像素，旧实现不能通过。
7. 增强 TEST26 真机通过后，把 retained SVG 对象接入文档图片缓存与 NetSurf replaced box；复合路径孔洞需另加最小测试后才能宣称支持，SVG text 随后按真实页面需求推进。
...
- `positron_http`
- `positron_core`
- `positron_image`（公共 DLL；SVG retained object + NanoSVG 抗锯齿 draw 已构建，增强 TEST26 待设备，位图迁移待完成）
