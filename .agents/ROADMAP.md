...
结构 mutation 目前承诺九条窄路径：带 id 元素的 `Element.append()`/`prepend()` 零至四值文本/元素插入，
...
文本插入，以及 Ex7 的 `Element.replaceWith(value)` 原位 primitive 文本替换。write Ex6
...
`insertAdjacentText()`/`insertAdjacentElement()`，Ex7 保留旧 wrapper 为 detached。九条路径
都由 Core 拥有 DOM 语义并在成功后使 retained layout 失效；DocumentFragment、Text/Comment/
