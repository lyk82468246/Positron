...
26. **ABI 1.4 原生 BMP/GIF 输出与示例真退出待 next51 验收**：`PIMAGE_ENCODE_BMP/GIF` 继续走 WM Imaging 已安装 encoder，缺失时返回 `PIMAGE_ERROR_UNSUPPORTED`，不自写格式 codec。示例启动时自动检查 `BM`/`GIF8` 签名、重新解码和 16x16 尺寸；屏幕保持 next50 六项不变。窗口只新增不占客户区的 `WM_CLOSE -> DestroyWindow`，标题栏 X、既有 Esc 都必须进入 `WM_DESTROY -> PostQuitMessage`；按用户要求不创建左右软键。跨线程句柄仍未提供。
