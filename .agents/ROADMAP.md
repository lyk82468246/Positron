...
更新时间：2026-07-30
基线：正式 Browse 路径走 NetSurf `layout_document` + `html_redraw`；TEST13 深层导航保持 next37 冻结语义。图片/SVG、字体 fallback、列表 marker/counter/inside flow、table 常见路径及表单纵切已推进到 next104 / TEST71 的设备自动化基线。next104 的 TEST13/20/27/43/44/56/58-71 全部通过，multiple select 已接入 WM 原生 LISTBOX。正文按时间保留已完成工作的来龙去脉，末尾“建议执行顺序”才是当前优先级；详细边界见 `KNOWN_LIMITATIONS.md`。
...
1. **表单交互纵切**：next93 已完成 checkbox/radio，next94/TEST65 已完成单行 text/password，next97/TEST66 已完成 textarea，next98/TEST67 已完成单选，next99/TEST68 已完成 button 与 GET/URL-encoded POST，next101/TEST69 已完成 reset、Enter 和 label，next103/TEST70 已完成 multipart/file，next104/TEST71 已完成 WM multiple select；均由设备无人值守日志确认。下一缺口是约束验证。
...
- forms/widgets 剩余部分：约束验证、完整事件取消/传播与焦点状态；multipart/file 与 WM 多选列表已由 TEST70/71 通过设备门禁
...
1. 以 next104 / TEST71 为设备自动化基线；自动 testbench、TEST13 深层导航和旋转继续作为每批门禁。
2. 下一批优先补表单约束验证，再推进事件状态基础。真实触屏 label/Enter/multiple select、原生文件选择器和控件视觉验收放入后续人工检查批次。
