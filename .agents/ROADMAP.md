...
连接中 direct Text wrapper 的 `Text.remove()`。前者由 Ex6 在未过滤 `childNodes` 的末尾
或零位创建 Text，后者由 Ex2 按原索引删除一个 Text；两者都由 Core 拥有 DOM 语义并在
成功后使 retained layout 失效。通用 Node/DocumentFragment 插入、已有节点 reparent、
Comment/CDATA/其他删除、observer 和 live collection 仍需由真实页面缺口驱动，不能从这两条
窄路径外推。
