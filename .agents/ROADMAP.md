...
next819–next825 已在 Browser-owned `DocumentFragment` staging 上依次补齐 bounded
clone、lookup、selector、`children`/命名属性、Node 关系和原子 `replaceChildren()`；当前事实、设备证据和剩余
...
下一批（next826）的选择必须先从 compatibility corpus、源码、设备日志或截图固定一个新的、可
