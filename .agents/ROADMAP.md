...
26. **重定向 origin 与导航活性批次待设备验收**：TEST13 的 `example.com` Learn more 实际经历 `https://iana.org/domains/example` 到 `http://www.iana.org/help/example-domains` 的跨 scheme/host 重定向。旧 `PHttpResponse` 没有返回最终 URL，宿主错误地继续以原请求作为 document origin；加载期间每个滚动 thumb 消息还同步 `UpdateWindow`，会与 worker 的网络分配叠加 WM6 GDI/堆压力。现由 HTTP DLL 返回 `effective_url`，成功主文档在 parse 前更新 host/path/port/document URL；WinInet 每跳连接/接收等待设为 15 秒，宿主可选资源阶段总预算 20 秒，超时资源保留 fallback 后继续提交；加载期间滚动只失效并让 WM 合并重绘。TEST48 离线覆盖 HTTPS 到 HTTP/www 的最终 origin 及根相对 CSS 解析。TLS socket 仍没有强制取消和完整 deadline，不能把这些预算表述为所有请求均能硬超时。默认设备批次为 TEST13/43/44/47/48。
