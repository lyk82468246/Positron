...
TEST1139 与定向设备门已验证该合同。下一条纵向能力是 next695，仍须从源码、
日志或截图固定一个新的可复现用户可见组合缺口；不能凭空扩张 ABI，也不能把
页面业务规则塞回 `test_host`。嵌套 overflow 容器、
...
next693 和 next694 的完成证据都包括定向自动断言、直接相邻回归和风险相称的
设备门；下一批 next695 必须沿用同一完成标准。
