...
next239 已迁移 DOM attribute callback，next240 已迁移 Event callback，next241 已迁移 input value callback；下一批迁移 checked/form、其余 input/location/navigation callback 实现。`test_host` 只能作为
宿主适配和测试消费者；
