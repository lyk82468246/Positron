...
24. **stylesheet 元数据批次待设备验收**：内联 `<style>` 与外链 `<link>` 现把完整 `media` 字符串交给 libcss 的 selection context；`rel` 按 ASCII 空白 token 识别 stylesheet 并排除 alternate，非空 `type` 仅接受 `text/css`，带 `disabled` 的表不解析也不 fetch。TEST46 在 320/299px 同时验证内联/外链 media 切换、两个合法 URL 仅 fetch/free 一次、alternate/非 CSS/disabled 三个 URL 零 fetch，以及 cache-only 重选。C89 脚本 0 改动，VS2008 ARM 增量构建 0 错误；默认设备批次为 TEST13/24/45/46。
