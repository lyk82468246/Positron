...
25. **文档 base URL 批次待设备验收**：新增共享首个 `<base href>` 解析器，仍由宿主 resolver 执行 URL 规范化，core 不绑定 WinInet。`PCore_StyleDocumentEx2` 现让外链 CSS、嵌套 import 与 CSS URL 使用有效 document base；新增兼容扩展 `PCore_FetchImageResourcesEx2` 与 `PCore_LinkAtEx2`，分别处理图片 URL/原始别名和点击链接绝对化，旧 API 保留原始 URL 行为。图片缓存用别名链覆盖多个相对写法映射到同一规范 URL，避免重复 fetch 且保证 NetSurf 构盒仍能按原始 `src` 取到字节。TEST47 离线覆盖首个 base 优先、第二个 base 忽略、CSS/背景/两个等价图片引用、原始/绝对链接和 fetch/free/去重；c89ize 0 改动，VS2008 ARM 增量构建 0 错误。默认设备批次为 TEST13/18/24/45/46/47。
