...
当前自动化设备基线为 next202：`screen=640x480 dpi=192` 默认日志中 TEST13 三段导航及
TEST20/27/43/44/56/58-77/80-170 全部通过，配置所选 117 项全部 OK、零 ERROR、零 FAIL，并记录
...
1. 以 next202 的 TEST13/20/27/43/44/56/58-77/80-170 设备日志作为已验证自动化基线；
   后续每批继续以 TEST13 深层导航、动态 DPI 和定期旋转/真实点击作为浏览器门禁。
2. 继续把绝对片段 URL 的点段归一化按单一边界逐批扩展；后续浏览器 JavaScript 绑定仍按一个
   纵切一个设备门推进，不把完整 URL 标准化一次性并入。
