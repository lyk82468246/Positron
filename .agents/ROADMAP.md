...
下一条纵向能力是 next684：从现有真实页面哨兵和离线 compatibility corpus 中选定一个可复现的用户可见组合缺口，完成从公共 DLL 语义到宿主接线、自动断言和设备证据的一条完整路径。选题必须先由源码、日志或截图证明问题边界，不能凭空扩张 API，也不能把页面业务规则塞回 `test_host`。next683 已补齐 page-level 横向 viewport；后续工作不得把嵌套 overflow 容器或视觉猜测误写成已支持能力。
