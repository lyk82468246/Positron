...
- 依据实际流程在已有有界 `dialog` 脚本生命周期、`method="dialog"` 默认动作、活动 modal id、Escape 请求桥接、宿主顺序 Tab 子树范围、实体色 modal paint、backdrop 指针策略和单元素 contenteditable WM EDIT 接线之上，评估 caret/selection 或 IME 组合的下一个高价值缺口；保持已实现的有界 `tabindex` 排序与 Core/宿主事务边界。
