...
更新时间：2026-08-04
基线：正式 Browse 路径走 NetSurf `layout_document` + `html_redraw`；TEST13 深层导航保持 next37 冻结语义。图片/SVG、字体 fallback、列表 marker/counter/inside flow、table 常见路径、表单、最小 DOM Event 纵切、基础 relative/absolute positioning、动态 `:hover` 与脚本资源发现/缓存 ABI 已推进到 next114 / TEST77 的设备自动化基线。next118 已加入独立 `positron_script.dll`，本地 ARMV4I 构建及 TEST80 设备验证均已通过；当前源码候选 next119 再加入 TEST81 的 timeout/source-size/recovery 边界，ARMV4I 构建与 staging 已通过，设备待验；它不改变浏览器 JS 默认关闭的基线。默认配置为 TEST13/20/27/43/44/56/58-77/80-81，其中 next118 的 80 及此前门禁已通过，81 尚未取得设备日志；通用 Event 已有捕获/目标/冒泡、取消、停止传播、监听器生命周期和宿主 click default-action 门。next115 与 next116 的 float 候选均已因 TEST79/TEST13 真实回归否决，next114 的 Browse 路径保持为浏览器回归基线。失败/暂挂方向总索引见 `FAILED_EXPERIMENTS.md`；正文按时间保留已完成工作的来龙去脉，末尾“建议执行顺序”才是当前优先级；详细边界见 `KNOWN_LIMITATIONS.md`。
...
### 6b. next119：独立 JavaScript 安全边界（设备待验收）
...
- next119 已完成 VS2008 ARMV4I Debug 增量构建并 staging 到 `C:\WMShare\Positron-next119`；设备运行日志确认前，TEST81 不计入已验收基线。超时后的 UI 调度、脚本模块图、DOM/window/fetch/native bridge 仍不在此批次。
...
1. 以 next118 / TEST80 加 next114 Browse 门禁作为当前已验证自动化基线；先运行 next119 的 TEST81，确认超时/source-size/recovery 设备边界，再把它纳入已验收基线；后续每批继续以 TEST13 深层导航和旋转作为浏览器门禁。
2. 在 TEST81 通过后，评估真正的总内存配额和脚本模块生命周期，再评估把独立 DLL 接入显式的浏览器 JavaScript 开关；在 JS 默认关闭期间不得让 TEST13 平白增加脚本网络请求。
