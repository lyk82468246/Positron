...
- BMP/PNG/JPEG/GIF 的 WM Imaging 可见绘制已确认；2026-07-12 用户确认 TEST20 四格式缓存 `<img>` 正式链。SVG retained draw、缓存 `<img>`、损坏资源 alt fallback 与网络相对资源链已由 TEST26-28/13 真机确认；背景图仍未接通。
...
9. **已完成构建、待设备验收**：审计 NetSurf 官方最新 libsvgtiny（提交 `7ede71b`）后确认其仍未保留 SVG `fill-rule`。本地最小扩展支持 presentation attribute、inline style 与组继承，并把 nonzero/evenodd 传给 NanoSVG；TEST29 用同向复合路径离屏断言实心/孔洞语义并提供可见窗口。
10. 图片能力采用两层边界：上游解析库保持静态 `.lib`；稳定的 `positron_image.dll` 统一封装 WM Imaging 与 libsvgtiny，供 `positron_core.dll` 和其他 WM 程序调用。对象必须由同一 DLL 创建/释放，不能暴露 NetSurf 内部结构或跨 CRT 所有权；现有 `PCore_Image*` 暂保留为兼容转发。
