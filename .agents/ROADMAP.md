...
- BMP/PNG/JPEG/GIF 的 WM Imaging 可见绘制已确认；2026-07-12 用户确认 TEST20 四格式缓存 `<img>` 正式链。SVG retained draw 与缓存 `<img>` 已由 TEST26/27 真机确认；背景图仍未接通，损坏 SVG fallback 由 TEST28 待设备验收。
...
8. **已完成构建、待设备验收**：TEST28 将已 fetch 的损坏 SVG 缓存后布局，要求拒绝 `120x60` replaced object 并保留非空 alt 文本盒。TEST13 起始页新增仓库托管的 HTTPS HTML + 同目录相对 SVG fixture，用于验证真实页面 origin、相对 URL、网络 fetch 和正式绘制。之后再做复合路径孔洞最小测试。
9. 图片能力采用两层边界：上游解析库保持静态 `.lib`；稳定的 `positron_image.dll` 统一封装 WM Imaging 与 libsvgtiny，供 `positron_core.dll` 和其他 WM 程序调用。对象必须由同一 DLL 创建/释放，不能暴露 NetSurf 内部结构或跨 CRT 所有权；现有 `PCore_Image*` 暂保留为兼容转发。
