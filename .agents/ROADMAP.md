...
- BMP/PNG/JPEG/GIF 的 WM Imaging 可见绘制已确认；2026-07-12 用户确认 TEST20 的四格式缓存 `<img>` 正式 NetSurf 链在横竖屏均颜色、顺序一致，位图格式覆盖闭环完成。SVG retained draw 已由 TEST26 真机确认，缓存 SVG `<img>` 链由 TEST27 待设备验收；背景图仍未接通，解码失败或缓存未命中时 `<img>` 继续显示 alt/src 文本占位。
...
7. **已完成构建、待设备验收**：统一图片载体增加 WM Imaging/SVG 类型与跨 DLL 析构，缓存 SVG `<img>` 生成 NetSurf replaced box，并经 `content_redraw -> plot_bitmap -> positron_image.dll` 绘制。TEST27 自动检查 fetch/free、`120x60` 布局和离屏红绿像素，再显示真实窗口。通过后验证真实网络 Browse SVG 与失败 fallback；复合路径孔洞另加最小测试后才能宣称支持。
...
- `positron_http`
- `positron_core`
- `positron_image`（公共 DLL；SVG retained object + NanoSVG 抗锯齿 draw 已由 TEST26 真机确认，缓存 SVG `<img>` 的 TEST27 待设备）
