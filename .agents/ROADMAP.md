...
### 6ap. next170：JavaScript location 赋值桥（自动设备验收通过）

- 在 next169 的异步消息入口上加入 `location.assign()`、`location.href=`、
  `window.location=` 和 `document.location=`；native callback 只复制最后一个 URL 请求，
  不在 Duktape 调用栈内启动网络或同步改写当前 URL/history。
- 导航空闲后窗口消息取出请求并复用现有 GET 事务；连续请求采用 last-request-wins。
  TEST138 离线断言四个入口、同步 URL/history 不变和 14/16 native callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next169 已验收行为不变。C89、仓库/文档审计和
  ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next170` staging 的七个 ARMV4I
  二进制与构建产物 SHA-256 一致。320x320/128 DPI 日志得到 TEST138 OK、86 条 OK、
  零 ERROR、零 FAIL 和最终 PASS。replace/reload、URL 分量、go/forward、重定向历史与
  页面状态恢复不在本批。

### 6aq. next171：JavaScript location.reload GET 纵切（自动设备验收通过）

- `location.reload()` 从 bootstrap 闭包取 canonical document URL，通过 next170 的同一
  native callback/窗口消息入口排队；不在 Duktape 调用栈内重入网络。
- 导航空闲后复用普通 GET 事务。同一当前 URL 成功提交时沿用 history 的 duplicate-current
  guard，不增加条目、不移动 index，也不截断已有 forward branch。TEST139 离线断言返回
  `undefined`、URL 身份、同步 history 不变和 14/16 native callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next170 已验收行为不变。C89、仓库/文档审计
  和 ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next171` staging 的七个
  ARMV4I 二进制与构建产物 SHA-256 一致。320x320/128 DPI 日志得到 TEST139 OK、
  87 条 OK、零 ERROR、零 FAIL 与最终 PASS。真实网络重载、缓存验证、POST 重提交、
  `forceGet` 和页面状态恢复不在本批。

### 6ar. next172：JavaScript location.replace GET 纵切（自动设备验收通过）

- `location.replace(url)` 复用现有 native callback/窗口消息，退出 Duktape 调用栈且导航空闲
  后才启动 GET；URL 校验和 last-request-wins 与 assign/reload 一致。
- history 成功提交入口新增具名 replace-current 模式。失败保持当前条目；成功只替换当前
  URL，保留 count/index 和相邻 back/forward 条目。TEST140 离线断言返回 `undefined`、
  callback 前 URL/history 不变、成功替换结果和 14/16 native callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next171 已验收行为不变。C89、仓库/文档审计
  和 ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next172` staging 的七个
  ARMV4I 二进制与构建产物 SHA-256 一致。320x320/128 DPI 日志得到 TEST140 OK、
  88 条 OK、零 ERROR、零 FAIL 与最终 PASS。真实网络 replace、重定向历史、POST replace
  与页面状态恢复不在本批。

### 6as. next173：JavaScript history.forward 纵切（自动设备验收通过）

- 新增只读 forward target 查询和宿主前进 helper；查询与脚本 callback 都不移动 index，
  退出 Duktape 调用栈且导航空闲后才通过现有 GET 事务加载目标。
- 只有成功 document 提交才把 history index 更新到 forward target，失败保持当前项。
  TEST141 离线断言 `history.forward()` 返回 `undefined`、同步 URL/index 不变、目标查询、
  成功提交和 14/16 native callback 槽位。
- 默认 javascript=0、TEST13、core ABI、键盘 UI 与 next172 已验收行为不变。C89、
  仓库/文档审计及 ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next173` staging
  的七个 ARMV4I 二进制与构建产物 SHA-256 一致。320x320/128 DPI 日志得到 TEST141 OK、
...
### 6au. next175：JavaScript read-only history.length 纵切（自动设备验收通过）

- bootstrap 新增只读 `history.length`，值由宿主在 script 执行前按当前 document 成功提交后
  的状态计算，并作为 document 快照注入；首次 document 至少为 1，沿用 16 项历史上限。
- 普通成功 GET 的预期长度反映新增条目和 forward 分支截断；back/forward/go target、
  replace-current、POST 或失败提交不增加长度。TEST143 离线断言首次/追加/分支/replace/
  target/POST projection、只读赋值、同步 traversal 不变和 14/16 callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next174 已验收行为不变。C89 回归及 ARMV4I
  Debug 增量构建已通过；`C:\WMShare\Positron-next175` staging 的七个 ARMV4I 二进制与
  构建产物 SHA-256 一致。history state/push/replaceState、popstate、页面缓存和滚动/表单
  恢复不在本批。320x320/128 DPI 日志得到 TEST143 OK、91 条 OK、零 ERROR、零 FAIL 与
  最终 PASS，next175 已成为自动设备基线。

### 6av. next176：JavaScript initial history.state 纵切（自动设备验收通过）

- 在尚无 state mutation API 的最小 History 对象上增加只读 `state` getter；初始和网络
  document 返回 `null`，脚本赋值不会改变值。
- TEST144 离线断言 null 身份、无 setter 的属性描述符、赋值后不变、同步 go(0) 排队不改变
  state/history，以及 14/16 callback 槽位。默认 javascript=0、TEST13、core ABI 与
  next175 已验收行为不变，C89 回归及 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next176` staging 的七个 ARMV4I 二进制与构建产物 SHA-256 一致。
- pushState、replaceState、structured clone、跨 document state、popstate、同文档 URL
  历史和页面状态恢复不在本批。320x320/128 DPI 日志得到 TEST144 OK、92 条 OK、零 ERROR、
  零 FAIL 与最终 PASS，next176 已成为自动设备基线。

### 6aw. next177：JavaScript controlled history.replaceState 纵切（自动设备验收通过）

- 为现有最多 16 个成功 GET 条目保存小于 1024 字节的 JSON state；新增不改 URL 的受控
  `history.replaceState(state, title)`，title 忽略，URL 只允许省略、空串或当前绝对 URL，
  getter 每次返回 JSON clone。
- 初始脚本只更新候选 bridge，document 最终成功提交才落入对应条目；活动页面同步替换
  当前条目，back/forward/go/reload 按条目恢复且不增加 length。TEST145 离线断言 URL 拒绝、
  clone 隔离、成功提交、活动替换、逐项恢复和 14/16 callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next176 已验收行为不变；C89 回归及 ARMV4I Debug
  增量构建已通过；`C:\WMShare\Positron-next177` staging 的七个 ARMV4I 二进制与构建产物
  SHA-256 一致。完整 structured clone、pushState、非当前 URL 改写、popstate、POST state
  和页面缓存不在本批。320x320/128 DPI 日志得到 TEST145 OK、93 条 OK、零 ERROR、零 FAIL
  与最终 PASS，next177 已成为自动设备基线。

### 6ax. next178：JavaScript same-URL history.pushState 纵切（自动设备验收通过）

- 在 next177 的逐项 JSON state 上新增不联网的受控 `history.pushState(state, title)`；state
  仍须序列化为小于 1024 字节的 JSON，title 忽略，URL 只允许省略、空串或当前绝对 URL。
  调用同步追加条目、更新 length/state、截断 forward 分支，最多保留 16 项。
- 初始 GET 脚本的多次 push/replace 先按顺序留在候选 bridge，document 最终成功后才提交；
  活动页立即提交。TEST146 离线断言成功提交隔离、多次操作顺序、clone、URL 拒绝、同步
  length、活动追加、前向截断、逐项恢复和 14/16 callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next177 已验收行为不变；C89 回归及 ARMV4I Debug
  增量构建已通过；`C:\WMShare\Positron-next178` staging 的七个 ARMV4I 二进制与构建产物
  SHA-256 一致。遍历仍复用现有 GET 重载；完整 structured clone、非当前 URL、POST state、
  同 document 生命周期、popstate 和页面缓存不在本批。320x320/128 DPI 日志得到 TEST146
  OK、94 条 OK、零 ERROR、零 FAIL 与最终 PASS，next178 已成为自动设备基线。

### 6ay. next179：JavaScript same-document history traversal（自动设备验收通过）

- 为成功网络 document 分配内部 identity，pushState 条目继承当前 identity；back/forward 和
  非零 history.go 命中同 identity sibling 时保留现有 DOM/runtime，不启动 GET，只切换
  history index 和逐项 JSON state，length 不变。
- go(0)、reload 和跨 identity 条目仍走网络路径，成功网络 document 获得新 identity。
  TEST147 离线断言 back/forward/go 无 GET 切换、DOM/runtime 身份、state/length、go(0)
  排除以及 reload/跨 document 隔离。
- 默认 javascript=0、TEST13、core ABI 与 next178 已验收行为不变；C89 回归及 ARMV4I Debug
  增量构建已通过；`C:\WMShare\Positron-next179` staging 的七个 ARMV4I 二进制与构建产物
  SHA-256 一致。popstate、逐项滚动/表单恢复、非当前 URL、POST state 和跨 document 页面
...
### 6bb. next182：JavaScript minimal hashchange（已设备验收）

- 片段变化的同 document history traversal 在 popstate 后派发最小 hashchange；支持
  window.onhashchange 以及 window add/removeEventListener('hashchange')，事件包含
  oldURL/newURL、window target/currentTarget、不可取消元数据，handler 异常相互隔离。
- pushState/replaceState 自身及相同片段 traversal 静默。TEST150 离线断言事件顺序、属性/
  listener、重复去重、remove、异常/取消隔离、URL 元数据、静默边界和 14/16 callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next181 已验收行为不变；location.hash/片段赋值、
  跨 document hashchange、完整 Window EventTarget/HashChangeEvent、逐项滚动/表单恢复和页面
  缓存不在本批。C89 回归及 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next182` staging 的七个 ARMV4I 二进制与构建产物 SHA-256 一致，
...
### 6bd. next184：JavaScript location.hash 导航（已完成）

- location.hash setter 复用 WM 脚本导航消息，延迟创建同 document null-state history entry；
  提交后同步 href/components/history.length，只派发 hashchange，不启动 GET 或 popstate。
- 相同值静默，空字符串清除片段；后续 back 保持 popstate→hashchange。TEST152 离线断言
  异步边界、same-value、清除、state/length、事件顺序、无网络和 14/16 callback 槽位。
- 默认 javascript=0、TEST13、core ABI 与 next183 已验收行为不变；location.href/assign/replace
  相对片段、百分号编码/标准化、锚点滚动、跨 document 片段导航及其他组件 setter 不在本批。
  C89 回归及 ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next184` staging 的七个
  ARMV4I 二进制与构建产物 SHA-256 一致。320x320/128 DPI 日志得到 TEST152 OK、配置所选
  99 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13 使用 `OK (overview)`，其余 98 项
  使用标准数字 OK 行。

### 6bf. next186：JavaScript 绝对同文档片段 URL（已完成）

- href/assign/replace 的绝对 URL 与当前基址相同且仅改变 fragment 时复用同 document 队列；
  当前确有 fragment 时，绝对基址可清除它。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL 导航及
  query/path/origin 不同目标仍走普通导航。TEST154 固定三入口、清除、state/length、事件、
  无网络、普通导航边界和 14/16 callback 槽位。
- 相对 path+fragment、百分号编码/标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89、仓库审计和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next186` staging 的七个 ARMV4I 二进制与构建产物 SHA-256 一致。
...
### 6bh. next188：JavaScript query-relative 同文档片段 URL（已完成）

- query-relative href/assign/replace URL 解析后与当前 pathname/query 完全相同且仅改变 fragment
  时复用同 document 队列；当前确有 fragment 时也可用匹配 query 清除。href/assign push，
  replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 query 导航、
  不同 query 和普通 path-relative 目标仍走普通导航。TEST156 固定三入口、清除、same-value、
  state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 普通 path-relative、dot-segment、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next188` staging 的七个 ARMV4I 二进制与构建产物 SHA-256 一致。
  240x240/96 DPI 日志得到 TEST156 OK、配置所选 103 项全部 OK、零 ERROR、零 FAIL 与最终
  PASS；TEST13 使用 `OK (overview)`，其余 102 项使用标准数字 OK 行。

### 6bi. next189：JavaScript 同目录 path-relative 同文档片段 URL（已完成）

- 不带 `./` 或 `../` 前缀的同目录 path-relative href/assign/replace URL，解析后与当前 path/query
  完全相同且仅改变 fragment 时复用同 document 队列；当前确有 fragment 时也可用匹配相对
  文件名清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和点段前缀目标仍走普通导航。TEST157 固定三入口、清除、same-value、state/
  length、事件、无网络、分类边界和 14/16 callback 槽位。
- 点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认 javascript=0、TEST13
  与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next189`
  staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。修复后 240x240/96 DPI 日志得到 TEST156/157
  OK、配置所选 104 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13 使用 `OK (overview)`，
  其余 103 项使用标准数字 OK 行。
- 首次设备门在 TEST156 停止，因为其旧负向断言仍把 next189 新纳入范围的同 path/query
  `page?x=1#network` 期望为普通导航；现已改用真正不同 path 的 `other?x=1#network` 保留分类
  负向门。重新构建后的设备复跑已通过。

### 6bj. next190：JavaScript 单个 `./` 同目录片段 URL（已完成）

- 带单个 `./` 前缀的 href/assign/replace URL，移除前缀并解析后与当前 path/query 完全相同且仅
  改变 fragment 时复用同 document 队列；当前确有 fragment 时也可用匹配目标清除。href/assign
  push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和 `../` 父目录目标仍走普通导航。TEST158 固定三入口、清除、same-value、state/
  length、事件、无网络、分类边界和 14/16 callback 槽位。
- `../`、重复/混合点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next190` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST158 OK、配置所选 105 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 104 项使用标准数字 OK 行。

### 6bk. next191：JavaScript 单个 `../` 父目录片段 URL（已完成）

- 带单个 `../` 前缀的 href/assign/replace URL，上移一个目录并解析后与当前 path/query 完全相同
  且仅改变 fragment 时复用同 document 队列；当前确有 fragment 时也可用匹配目标清除。
  href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和重复 `../../` 目标仍走普通导航。TEST159 固定三入口、清除、same-value、state/
  length、事件、无网络、分类边界和 14/16 callback 槽位。
- 重复/混合点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next191` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST159 OK、配置所选 106 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 105 项使用标准数字 OK 行。

### 6bl. next192：JavaScript 连续前导父目录片段 URL（已完成）

- 连续多个前导 `../` 的 href/assign/replace URL，逐级上移目录并解析后与当前 path/query 完全
  相同且仅改变 fragment 时复用同 document 队列；越过 origin 根的额外父目录段钳制在根。
  当前确有 fragment 时也可用匹配目标清除；href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和混合 `.././` 目标仍走普通导航。TEST160 固定三入口、清除、same-value、state/
  length、事件、无网络、分类边界和 14/16 callback 槽位。
- 混合/内嵌点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next192` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST160 OK、配置所选 107 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 106 项使用标准数字 OK 行。

### 6bm. next193：JavaScript 父目录后单个 `./` 片段 URL（已完成）

- 连续前导 `../` 之后允许一个 `./` 的 href/assign/replace URL，先逐级上移目录、再移除该单点段，
  解析后与当前 path/query 完全相同且仅改变 fragment 时复用同 document 队列；当前确有
  fragment 时也可用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和重复 `../././` 目标仍走普通导航。TEST161 固定三入口、清除、same-value、state/
  length、事件、无网络、分类边界和 14/16 callback 槽位。
- 重复/任意内嵌点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next193` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST161 OK、配置所选 108 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 107 项使用标准数字 OK 行。

### 6bn. next194：JavaScript 父目录后连续 `./` 片段 URL（已完成）

- 连续前导 `../` 之后允许连续多个 `./` 的 href/assign/replace URL，先逐级上移目录、再移除这些
  单点段，解析后与当前 path/query 完全相同且仅改变 fragment 时复用同 document 队列；当前
  确有 fragment 时也可用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和路径中部 `segment/../` 目标仍走普通导航。TEST162 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 任意内嵌点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认 javascript=0、
  TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next194` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST162 OK、配置所选 109 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 108 项使用标准数字 OK 行。

### 6bo. next195：JavaScript 连续前导 `./` 片段 URL（已完成）

- 无父目录前缀时允许连续多个前导 `./` 的 href/assign/replace URL，移除这些单点段后与当前
  path/query 完全相同且仅改变 fragment 时复用同 document 队列；当前确有 fragment 时也可用
  匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和路径中部 `segment/../` 目标仍走普通导航。TEST163 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 任意内嵌点段归一化、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认 javascript=0、
  TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next195` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST163 OK、配置所选 110 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 109 项使用标准数字 OK 行。

### 6bp. next196：JavaScript 父目录后单个内嵌 `./` 片段 URL（已完成）

- 连续前导 `../` 逐级上移后，允许余下路径中出现一个内嵌 `./` 的 href/assign/replace URL；
  移除该单点段后与当前 path/query 完全相同且仅改变 fragment 时复用同 document 队列，当前确有
  fragment 时也可用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和连续内嵌 `././` 目标仍走普通导航。TEST164 固定三入口、清除、same-value、
  state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 多个内嵌点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next196` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST164 OK、配置所选 111 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 110 项使用标准数字 OK 行。

### 6bq. next197：JavaScript 父目录后连续内嵌 `./` 片段 URL（已完成）

- 连续前导 `../` 逐级上移后，允许余下路径同一位置出现连续多个内嵌 `./` 的 href/assign/replace
  URL；移除该连续单点段后与当前 path/query 完全相同且仅改变 fragment 时复用同 document 队列，
  当前确有 fragment 时也可用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query、分离位置的多个 `./` 和内嵌 `../` 目标仍走普通导航。TEST165 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 任意多位置点段归一化、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next197` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST165 OK、配置所选 112 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 111 项使用标准数字 OK 行。

### 6br. next198：JavaScript 父目录后多位置内嵌 `./` 片段 URL（已完成）

- 连续前导 `../` 逐级上移后，允许余下路径多个位置出现内嵌 `./` 的 href/assign/replace URL；
  移除这些单点段后与当前 path/query 完全相同且仅改变 fragment 时复用同 document 队列，当前确有
  fragment 时也可用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query 和内嵌 `../` 目标仍走普通导航。TEST166 固定三入口、清除、same-value、state/
  length、事件、无网络、分类边界和 14/16 callback 槽位。
- 无父目录前缀的内嵌点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批；
  默认 javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next198` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。240x240/96
  DPI 日志得到 TEST166 OK、配置所选 113 项全部 OK、零 ERROR、零 FAIL 与最终 PASS；TEST13
  使用 `OK (overview)`，其余 112 项使用标准数字 OK 行。

### 6bs. next199：JavaScript 根相对单个内嵌 `./` 片段 URL（已完成）

- 根相对 href/assign/replace URL 允许路径中出现一个内嵌 `./`；移除该单点段后与当前 path/query
  完全相同且仅改变 fragment 时复用同 document 队列，当前确有 fragment 时也可用匹配目标清除。
  href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query、连续内嵌 `././` 和内嵌 `../` 目标仍走普通导航。TEST167 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 根相对多个点段、绝对 URL 点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在
  本批；默认 javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  原共享目录 fixture 连续两次在 TEST70 `WriteFile` 返回 0/0 字节且错误码为 0；现改用设备本地
  `\Temp` 并保留精确失败诊断。`C:\WMShare\Positron-next199-fix` staging 的七个 ARMV4I 二进制
  SHA-256 全部匹配。240x240/96 DPI 日志得到 TEST70 与 TEST167 OK、配置所选 114 项全部 OK、
  零 ERROR、零 FAIL 与最终 PASS；TEST13 使用 `OK (overview)`，其余 113 项使用标准数字 OK 行。

### 6bt. next200：JavaScript 根相对连续内嵌 `././` 片段 URL（已完成）

- 根相对 href/assign/replace URL 允许同一路径位置连续出现 `././`；移除连续点段后与当前 path/query
  完全相同且仅改变 fragment 时复用同 document 队列，当前确有 fragment 时也可用匹配目标清除。
  href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query、多个分离内嵌 `./` 位置和内嵌 `../` 目标仍走普通导航。TEST168 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 根相对多个分离点段、绝对 URL 点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在
  本批；默认 javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next200` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。640x480/192
  DPI 日志得到 TEST70、TEST167 与 TEST168 OK、配置所选 115 项全部 OK、零 ERROR、零 FAIL 与
  最终 PASS；TEST13 使用 `OK (overview)`，其余 114 项使用标准数字 OK 行。

### 6bu. next201：JavaScript 根相对多位置内嵌 `./` 片段 URL（已完成）

- 根相对 href/assign/replace URL 允许多个分离路径位置出现内嵌 `./`；移除所有这些点段后与当前
  path/query 完全相同且仅改变 fragment 时复用同 document 队列，当前确有 fragment 时也可用
  匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query、绝对 URL 点段和内嵌 `../` 目标仍走普通导航。TEST169 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 绝对 URL 点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。首包因 TEST167 的过期负边界停止；修正版保持 TEST169
  正向断言并改用绝对 URL 点段负边界。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next201-fix` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。640x480/192
...
### 6bv. next202：JavaScript 绝对 URL 单个内嵌 `./` 片段导航（已完成）

- 绝对 href/assign/replace URL 允许路径中出现一个内嵌 `./`；移除该点段后与当前
  origin/path/query 完全相同且仅改变 fragment 时复用同 document 队列，当前确有 fragment 时也可
  用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query、连续内嵌 `././` 和内嵌 `../` 目标仍走普通导航。TEST170 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 绝对 URL 连续/多位置点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批；
  默认 javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next202` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。640x480/192
  DPI 日志得到 TEST70、TEST169 与 TEST170 OK、配置所选 117 项全部 OK、零 ERROR、零 FAIL 与
  最终 PASS；TEST13 使用 `OK (overview)`，其余 116 项使用标准数字 OK 行。

### 6bw. next203：JavaScript 绝对 URL 连续内嵌 `././` 片段导航（已完成）

- 绝对 href/assign/replace URL 允许同一路径位置连续出现 `././`；移除连续点段后与当前
  origin/path/query 完全相同且仅改变 fragment 时复用同 document 队列，当前确有 fragment 时也可
  用匹配目标清除。href/assign push，replace 替换当前 entry。
- 三入口保持延迟提交、无 GET/popstate、只派发 hashchange；当前无 fragment 的同 URL、不同
  path/query、多个分离内嵌 `./` 位置和内嵌 `../` 目标仍走普通导航。TEST171 固定三入口、清除、
  same-value、state/length、事件、无网络、分类边界和 14/16 callback 槽位。
- 绝对 URL 多位置点段、内嵌父目录、百分号标准化、锚点滚动和其他组件 setter 不在本批；默认
  javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 增量构建已通过；
  `C:\WMShare\Positron-next203` staging 的七个 ARMV4I 二进制 SHA-256 全部匹配。640x480/192
  DPI 日志得到 TEST70、TEST170 与 TEST171 OK、配置所选 118 项全部 OK、零 ERROR、零 FAIL 与
  最终 PASS；TEST13 使用 `OK (overview)`，其余 117 项使用标准数字 OK 行。

### 6bx. next204：test_host 专用完成提示音（已完成）

- 配置解析器只额外接受精确编号 999；该批尚未开放 172-998，next205/206 后续将 172/173
  变为正式测试编号，当前 174-998 仍无效。TEST999 排在普通所选测试与 TEST7b 之后，只调用一次
...
### 6bz. next206：JavaScript 绝对 URL 百分号编码单点段片段导航（已完成）

- 绝对 href/assign/replace URL 中，单个完整 segment 若 ASCII 大小写不敏感匹配 `%2e`，则按
  single-dot 移除；规范化后与当前 origin/path/query 相同且 fragment 改变或清除时复用同文档
  队列。实现依据 WHATWG URL Standard 的 single-dot URL path segment 定义。
- TEST173 固定 `%2E`/`%2e`、三入口、清除、same-value、history/state、hashchange、无网络、
  多个编码点段、`%2E%2E`、不同 query/path、`..` 排除边界和 14/16 callback 槽位。
- double-dot 的 `.%2e`/`%2e.`/`%2e%2e`、混合字面/编码点段、锚点滚动和其他组件 setter
  不在本批。默认 javascript=0、TEST13 与 core ABI 不变。C89 和 ARMV4I Debug 构建已通过，
...
### 6cb. next208：JavaScript 根相对 URL 编码单点段片段导航（已完成）

- 根相对 href/assign/replace URL 中单个内嵌完整 segment 若 ASCII 大小写不敏感匹配 `%2e`，
  则按 single-dot 移除；与当前 origin/path/query 匹配且 fragment 改变或清除时复用同文档队列。
- TEST175 固定三入口、清除、same-value、history/state、hashchange、无网络、多个编码点段、
  `%2E%2E`、不同 query/path、`..` 排除边界和 14/16 callback 槽位。
- 默认定向门为 TEST13/151-175/999（27 项）；next206 的 121 项日志保留为最近全量检查点。
  C89、ARMV4I Debug 构建、`C:\WMShare\Positron-next208` 七个哈希和设备日志均已通过。

### 6cc. next209：JavaScript 根相对 URL 末尾编码单点段片段导航（已完成）

- 根相对 href/assign/replace URL 的 path 末尾 segment 若 ASCII 大小写不敏感匹配 `%2e`，且
  正好终止于 query/fragment 或 URL 结尾，则按 single-dot 移除；同文档判定继续要求
  origin/path/query 匹配且 fragment 改变或清除。
- TEST176 固定三入口、清除、same-value、history/state、hashchange、无网络、混合编码点段、
  `%2E%2E`、不同 query/path、`..` 排除边界和 14/16 callback 槽位。
...
### 6ce. next211：JavaScript 根相对 URL 重复编码单点段片段导航（已完成）

- 根相对 href/assign/replace URL 中多个内嵌完整 `%2e` segment 依次按 single-dot 移除；同文档
  判定继续要求 origin/path/query 匹配且 fragment 改变或清除。
- TEST178 固定三入口、清除、same-value、history/state、hashchange、无网络、不同 query/path、
  父目录排除边界和 14/16 callback 槽位。TEST175 的重复编码旧反向断言已移除；`%2E%2E` 与
  混合内嵌/末尾编码仍不规范化。
- 本批是 full checkpoint 后第 5 个低风险批次，因此恢复全量门
  TEST13/20/27/43/44/56/58-77/80-178/999（126 项）。C89、ARMV4I Debug 构建和
  `C:\WMShare\Positron-next211` 七个哈希和设备日志均已通过，next211 成为最近全量检查点。

### 6cf. next212：JavaScript 绝对 URL 编码双点段片段导航（已完成）

- 绝对 href/assign/replace URL 中单个内嵌完整 `%2e%2e` segment 连同前一个非空目录折叠；
  同文档判定继续要求 origin/path/query 匹配且 fragment 改变或清除。
- TEST179 固定大小写、三入口、清除、same-value、history/state、hashchange、无网络、不同
  query/path、额外父目录边界和 14/16 callback 槽位。根相对、末尾、重复、混合点段以及
  `.%2e`/`%2e.` 拼写仍不规范化。
- 本批是 next211 full checkpoint 后第 1 个低风险定向批次，门为 TEST13/151-179/999
  （31 项）。C89、ARMV4I Debug 构建、`C:\WMShare\Positron-next212` 七个哈希和设备日志均已通过。

### 6cg. next213：JavaScript 根相对 URL 编码双点段片段导航（已完成）

- 根相对 href/assign/replace URL 中单个内嵌完整 `%2e%2e` segment 连同前一个非空目录折叠；
  同文档判定继续要求 origin/path/query 匹配且 fragment 改变或清除。
- TEST180 固定大小写、三入口、清除、same-value、history/state、hashchange、无网络、不同
  query/path、额外父目录边界和 14/16 callback 槽位。末尾、重复、混合点段以及
  `.%2e`/`%2e.` 拼写仍不规范化。
- 本批是 next211 full checkpoint 后第 2 个低风险定向批次，门为 TEST13/151-180/999
  （32 项）。C89、ARMV4I Debug 构建和 `C:\WMShare\Positron-next213-fix` 七个哈希已通过。
  首包 ini 的 4120 字节超过 4096 字节读取上限，未执行测试；修正版保持配置值不变，只把
  注释精简至 1357 字节并在新目录正式暂存；修正版设备日志已全部通过。

### 6ch. next214：JavaScript 绝对 URL 末尾编码双点段片段导航（已完成）

- 绝对 href/assign/replace URL 的 path 末尾 segment 若完整匹配 `%2e%2e`，连同前一个非空
  目录折叠并保留 `/`；同文档判定继续要求 origin/path/query 匹配且 fragment 改变或清除。
- TEST181 固定大小写、三入口、清除、same-value、history/state、hashchange、无网络、不同
  query/path、混合 single/double、重复 double-dot、字面父目录和 14/16 callback 槽位。根相对
  末尾以及 `.%2e`/`%2e.` 拼写仍不规范化。
- 本批是 next211 full checkpoint 后第 3 个低风险定向批次，门为 TEST13/151-181/999
...
### 6cj. next216：JavaScript 绝对 URL 半编码双点段片段导航（已完成）

- 绝对 href/assign/replace URL 中单个内嵌完整 `.%2e` 或 `%2e.` segment 连同前一个非空目录
  折叠；同文档判定继续要求 origin/path/query 匹配且 fragment 改变或清除。
- TEST183 固定两种拼写、大小写、三入口、清除、same-value、history/state、hashchange、无
  网络、不同 query/path、混合完整/半编码 double-dot、重复半编码 double-dot、字面父目录和
  14/16 callback 槽位。根相对和末尾半编码 double-dot 仍不规范化。
- 本批是 next211 full checkpoint 后第 5 个低风险批次，门恢复为
  TEST13/20/27/43/44/56/58-77/80-183/999（131 项）。C89、ARMV4I Debug 构建和
  `C:\WMShare\Positron-next216` 七个哈希已通过；ini 为 1294 字节，`screen=240x320 dpi=96`
  设备日志已全部通过，成为新的 full checkpoint。

### 6ck. next217：JavaScript 根相对 URL 半编码双点段片段导航（已完成）

- 根相对 href/assign/replace URL 中单个内嵌完整 `.%2e` 或 `%2e.` segment 连同前一个非空目录
  折叠；补回当前 origin 后，同文档判定继续要求 path/query 匹配且 fragment 改变或清除。
- TEST184 固定两种拼写、大小写、三入口、清除、same-value、history/state、hashchange、无
  网络、不同 query/path、混合完整/半编码 double-dot、重复半编码 double-dot、字面父目录和
  14/16 callback 槽位。末尾半编码 double-dot 仍不规范化。
- 本批是 next216 full checkpoint 后第 1 个低风险定向批次，门为 TEST13/151-184/999
  （36 项）。C89、ARMV4I Debug 构建和 `C:\WMShare\Positron-next217` 七个哈希已通过；ini
  为 1278 字节，设备日志已全部通过。

### 6cl. next218：JavaScript 绝对 URL 末尾半编码双点段片段导航（已完成）

- 绝对 href/assign/replace URL 的 path 末尾 segment 若完整匹配 `.%2e` 或 `%2e.`，连同前一个
  非空目录折叠并保留 `/`；同文档判定继续要求 origin/path/query 匹配且 fragment 改变或清除。
- TEST185 固定两种拼写、大小写、三入口、清除、same-value、history/state、hashchange、无
  网络、不同 query/path、混合完整/半编码 double-dot、重复半编码 double-dot、字面父目录和
  14/16 callback 槽位。根相对末尾半编码 double-dot 仍不规范化。
- 本批是 next216 full checkpoint 后第 2 个低风险定向批次，门为 TEST13/151-185/999
  （37 项）。C89、实际 JavaScript 探针、ARMV4I Debug 构建和
  `C:\WMShare\Positron-next218` 七个哈希已通过；ini 为 1278 字节，设备日志已全部通过。

### 6cm. next219：JavaScript 根相对 URL 末尾半编码双点段片段导航（已完成）

- 根相对 href/assign/replace URL 的 path 末尾 segment 若完整匹配 `.%2e` 或 `%2e.`，连同前一个
  非空目录折叠并保留 `/`；补回当前 origin 后，同文档判定继续要求 path/query 匹配且 fragment
  改变或清除。
- TEST186 固定两种拼写、大小写、三入口、清除、same-value、history/state、hashchange、无
  网络、不同 query/path、混合完整/半编码 double-dot、重复半编码 double-dot、字面父目录和
  14/16 callback 槽位。
- 本批是 next216 full checkpoint 后第 3 个低风险定向批次，门为 TEST13/151-186/999
  （38 项）。首包 20,991 字符 bootstrap 在 TEST162 触发 1000ms 超时；修正版不放宽预算，
  以共享 helper 合并重复分类并降至 19,735 字符。C89、14 组实际 JavaScript 探针、ARMV4I
  Debug 构建和 `C:\WMShare\Positron-next219-fix` 七个哈希已通过；ini 为 1278 字节，修正版
  设备日志得到 37 条标准数字 OK、1 条 TEST13 overview、零 ERROR/FAIL 与最终 PASS。

### 6be. next185：JavaScript location 片段引用入口（已完成）

- `location.href='#...'` 与 `location.assign('#...')` 复用片段队列新增 null-state 同 document
  entry；`location.replace('#...')` 替换当前 entry，保留 document identity 且不增加 length。
- 三入口都延迟更新 location/history，只派发 hashchange，不启动 GET/popstate；相同目标静默，
  后退仍为 popstate→hashchange。TEST153 离线断言三入口、state/length、replace identity、
  事件、无网络和 14/16 callback 槽位。
- 其他相对/绝对 URL 仍走既有跨 document 导航；百分号编码/标准化、锚点滚动、跨 document
  片段导航和其他组件 setter 不在本批。默认 javascript=0、TEST13 与 core ABI 不变。C89、
  仓库审计和 ARMV4I Debug 增量构建已通过；`C:\WMShare\Positron-next185` staging 的七个
...
1. 以 next211 的 TEST13/20/27/43/44/56/58-77/80-178/999 设备日志作为已验证全量检查点，
   next211 同时作为当前定向能力基线；
   后续每批继续以 TEST13 深层导航、动态 DPI 和定期旋转/真实点击作为浏览器门禁。
2. 继续根相对 URL 末尾编码 double-dot 纵切；随后再选择其他 double-dot 拼写。低风险批次
   采用风险相关
...
6. next144-184 已利用同一个 Duktape DLL 逐项完成脚本执行、DOM 查询/修改、页面级 context、事件/表单/输入桥和最小 location/history 导航、受控 replace/pushState、同 document traversal、最小 popstate、history 片段 URL、最小 hashchange、动态 location URL 组件及 location.hash 同文档导航；后续绑定仍按一个纵切一个设备门推进，浏览器 JavaScript 默认保持关闭。
