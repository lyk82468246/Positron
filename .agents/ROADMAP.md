...
next819–next828 已在 Browser-owned `DocumentFragment` staging 上依次补齐 bounded
clone、lookup、selector、`children`/命名属性、Node 关系、原子 `replaceChildren()`/`replaceChild()`
和 Fragment-to-Fragment 的 append/insert/replaceChildren 组合，其中 next827 增加了
在旧节点位置展开另一个有界 Fragment 的 `replaceChild()`，next828 扩展了 Fragment 自身
的有界组合；当前事实、设备证据和剩余
...
text-only `replaceChildren()`；next833 再补齐 detached Element 的 `replaceChild()`；next834
补齐 detached Element 的稳定 `NamedNodeMap`/`Attr` facade、值 mutation、namespace lookup
和跨 owner copy；next835 修正 detached/attached Element `textContent` 的原子替换、
`childNodes` SameObject 与 Text wrapper owner 同步，均不新增 Core ABI。TEST1271–1275 与
...
next839 补齐 Browser-owned Text/Comment 的 element-sibling views：新建、Fragment staging、
parser-backed 物化和移除后的 CharacterData wrapper 都按当前 childNodes 顺序跳过非 Element，
未归属返回 `null`；TEST1279 与 `1278-1279,999` Debug ARMV4I 外置卡自动门通过，未新增
Core ABI。下一批先从 compatibility corpus 固定 next840 的可复现缺口。

next840 补齐 Browser-owned `document.createTextNode()` 的 bounded `wholeText`、`splitText()`
和 `replaceWholeText()`：detached 路径在 Browser 快照中完成，live regular Element 复用既有
Core CharacterData callback 并保持 wrapper identity；Fragment/staged Element 的 split
fail closed，未连接 staged Element 的 replacement 只更新本地快照。TEST1280 与
...
live regular Element 复用既有 nodeType=8 Core insertion/replacement callback，detached 调用
inert。TEST1282 与 `1281-1282,999` Debug ARMV4I 外置卡自动门通过，未新增 Core ABI；证据与
预算见 `.agents/HANDOFF.md` 和 `docs/TESTING.md`。

next843 为 Browser-created Text/Comment 增加单一现有 CharacterData source 的 bounded
`before()`/`after()`/`replaceWith()`：live regular Element 复用既有 Ex10/Ex11 Core callback，
跨父迁移更新两侧 owner/index 和旧 snapshot，detached target inert；detached source、
staging、Element、混合列表继续 fail closed。TEST1283 与 `1281-1283,999` Debug ARMV4I
外置卡自动门通过，未新增 Core ABI；证据与预算见 `.agents/HANDOFF.md` 和 `docs/TESTING.md`。

next844 为 Browser 增加 `document.createCDATASection(data)` 的 bounded detached wrapper，
并以 Ex14 物化 live CDATA；TEST1284 与 `1284,999` Debug ARMV4I 外置卡自动门通过，
双空间预检、完整日志回收、清理和 crash check 均通过；证据与预算见 `.agents/HANDOFF.md`
和 `docs/TESTING.md`。

next845 补齐 CDATASection 继承的 Text 结构合同：`wholeText` 合并相邻 Text/CDATA，
`splitText` 接受 CDATA 并返回 Text suffix，`replaceWholeText` 保留目标并移除相邻
...
next847 修正 Fragment `textContent` 投影：Comment 不再贡献文本，Text/CDATA 仍保留；
...
next851 补齐已物化 Browser-created Element 的 HTML mutation coherence：attached
`innerHTML` setter 在 Core parser 成功后原地刷新 created wrapper 的 child snapshot，
使旧 child wrapper detached；attached `outerHTML` setter 通过 public alias 进入既有
replacement，成功后清理 created-element alias 并允许同一 staged wrapper 重新设置 id、
...
next853 补齐已物化 Browser-created Element 的四位置 `insertAdjacentText()` 一致性：复用
...
next856 补齐已物化 Browser-created Element 的四位置 `insertAdjacentElement()` 一致性：
Browser 复用既有 Core element-child mutation，成功后刷新目标 `childNodes` snapshot，并覆盖
regular 与另一个已物化 Browser-created source 的移动、四位置父级重排、wrapper identity 和
非法/未物化/detached/arity fail-closed。TEST1295 与 `1294-1295,999` Debug ARMV4I 外置卡
