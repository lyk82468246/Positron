...
下一条纵向能力是 next673：在候选导航已经具备 generation 退休和协作式取消后，把资源阶段的终态区分为成功、失败和取消，并为宿主日志提供有界、可验证的摘要。取消不得被误记为网络失败或成功；同一候选的资源计数、响应释放和最终导航结果必须保持一致，且不改变当前页的提交规则。

next673 的语料应固定至少一个成功资源、一个可分类的失败资源和一个被取消的候选，自动断言终态计数、错误类别、回收路径与旧页保留。它不要求把阻塞中的 PHttp 调用强行中断，也不把外网端点写入契约；真实网络仍只作为独立哨兵。通用资源结果若暴露公共语义缺口，应先进入 Core/HTTP，再由宿主负责 WM 日志与调度。
