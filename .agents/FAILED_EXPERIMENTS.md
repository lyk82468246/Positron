...
`TESTBENCH PASS/FAIL`。当时连续使用同一批默认 `test_host.exe` 的超时目录，不能把
启动头当作 Core 选择器失败；四个目录的日志都只有 327 字节，且没有安全的 RAPI
远端终止接口。

处置：设备门改为把每次复制的宿主以时间戳唯一 basename 启动，并保留默认的“不完整
日志不删除”规则。按用户已授权的空间回收约束，先把四个精确 next756 picture 目录的
现有日志复制到本机，再删除这些目录（未强杀进程、未重置或重连 WMDC）。随后 Debug
ARMV4I 的 `20260907-105822-next756-picture-rerun2` 只选 TEST1198，完整日志稳定返回
`TESTBENCH PASS`。这证明候选选择实现和部署包可通过，但不足以单独证明首轮停滞的
具体环境根因。

决定：以后遇到同类启动头，先核对远端 basename、关闭设备端遗留宿主并保留可取得日志；
设备门不得强杀未知进程。日志不完整的目录仍默认保留，只有用户明确授权且目标目录已
精确列出的空间回收才可删除，并须先保存现有日志。

### next745 首轮设备门：默认 `\Temp` 目标卷空间不足 — 已替代

问题：next745 以默认 `\Temp\Positron-device-gate` 部署时，RAPI 预检报告目标卷仅
8,759,296 字节可用，而本批 payload 加保留量需要 10,782,217 字节。脚本先读取旧门
日志；因日志不完整而保留目录，随后有界应急回收仍未能释放足够空间，门在部署前停止。
WMDC/RAPI 会话和产品构建没有出现故障。

替代方案：改用已有可写的独立 `\Storage Card\Positron-gate-745` 路径；该卷可用
65,358,004,224 字节，内部 object store 仍只作为缓存风险检查。正式 Debug ARMV4I
payload 随后通过 TEST1184–1187、999（5/5、唯一 `TESTBENCH PASS`、零 `ERROR`/`FAIL`），
完整日志取得后才清理当前目录。

决定：外部部署优先使用已验证的路径专属卷；内部 object store 不足时只在日志完整且
目录可识别的前提下回收，不能为强行部署删除不完整证据或重置/重连 WMDC。
