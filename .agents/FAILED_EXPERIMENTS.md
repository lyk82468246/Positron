...
| next157/158 TEST122：UTF-16 代理对输入桥首次设备包 | 已定位，待替代 | TEST13 与 TEST20/27/43/44/56/58-121 通过；next158 证明 WM 合并和标量代码正确，但 Duktape 把直接注入的四字节 UTF-8 暴露为 length 1，而非两个 UTF-16 code unit。 | next159 在事件 JSON 中将 non-BMP 标量改写为两个 `\uXXXX`，保持断言不变并等待设备验收。next156 保持设备基线。 |
