...
- 截图由用户手工保存到 `tmp/`；只有用户明确告知有新截图时才查找、读取，不自动轮询或重复读取旧图。
- `TESTBENCH PASS` 只证明断言阶段结束。系统崩溃报告与之冲突时，不能把该候选称为验收通过；即使观察进程取得退出码 0，也不能覆盖同一测试进程的崩溃证据。
- `tmp/` 只保存本地证据，不加入 Git。
...
- WinCE 的 `SetWindowPos(SWP_FRAMECHANGED)` 可能同步重入 `WM_SIZE`；在滚动条样式
  settling 期间不得调用 Browser 脚本通知或重建 native child，必须由最外层消息在最终
  client rect 和 clamp 完成后统一发布。
- 看到 `positron_script.dll` 的异常地址时，优先检查 Duktape 数值转换等大自动变量是否
  压到 WM6 原生线程栈；移到稳定的受控堆并保留有界预算，不以扩大栈或放宽断言掩盖。
- 保持失败候选可辨认，不把它写成基线。
