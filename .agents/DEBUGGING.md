...
2026-07-13：增强 TEST26 会在打开可见窗口前，先离屏检查红绿内部像素和蓝色 cubic 的部分覆盖边缘像素；用户截图既出现平滑曲线又进入该窗口，因此自动断言与视觉结果都已通过。缓存 SVG 不另走旁路：TEST27 把 retained object 放进 typed image carrier，仍经 `box->object -> content_redraw -> plot_bitmap`，并在开窗前检查 fetch/free、`120x60` 布局及离屏红绿像素。
