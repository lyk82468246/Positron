...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。当前 next160 已在 `screen=640x480 dpi=192` 通过至 TEST122；next157-159 的代理对失败/诊断包均已由 next160 替代。next161/TEST123 是 WM6 IME composition 待验收候选；自动探针不创建系统 IME context，不能替代真实 SIP 输入。TEST79/float 候选已撤回。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
