...
2026-07-14：TEST47 已由用户确认通过。随后 TEST13 复现 `Open example` → `Learn more` 偶发一分钟以上不提交；等待时反复拖动旧页滚动条还可能直接崩溃。线上核对发现链接先到 `https://iana.org/domains/example`，最终重定向为 `http://www.iana.org/help/example-domains`。旧 `PHttpResponse` 只返回状态和正文，没有最终 URL，宿主便以原始 HTTPS/non-www URL 解析新文档资源；资源串行抓取会反复承担错误 origin/重定向成本。同时 `pcore_scroll_by` 在每个 thumb 消息后同步 `UpdateWindow`，加载期间高频强制 NetSurf/GDI paint 会和 worker 分配叠加 WM6 资源压力。修复为：响应追加 `effective_url`；成功主文档 parse 前更新待提交 host/path/port/document URL；WinInet 每跳连接/接收选项 15 秒；可选资源阶段总预算 20 秒；加载期间滚动仅 invalidate，由 WM 合并重绘。TEST48 离线断言 IANA 式 HTTPS→HTTP/www 最终 origin 和 `/_css/site.css` 解析。`c89ize.py` 对两个 C 文件均为 0 改动，VS2008 ARM 增量构建三个项目 0 错误，仅保留既有 libcss C4244。TLS socket 强制取消仍未解决，必须保留为显式限制。

2026-07-14 回退结论：用户确认 `9c5c7c7` 曾完整跑完 TEST13，而 `c01a349` 当时没有完成末端验收。上段 redirect-origin/20 秒预算/滚动合并方案及随后 `1e06105` TLS deadline 在设备上均未解决一分钟以上等待，故不得继续当作根因结论。TLS deadline 已用 no-commit revert 撤销；真实 Browse 初次 style 与旋转 restyle 使用 `PCORE_STYLE_COMPAT_9C5C7C7`，恢复 exact rel、忽略 stylesheet metadata、忽略 `<base>`；图片扫描和链接命中恢复旧 API，主文档不采用 `effective_url`，加载滚动恢复同步 `UpdateWindow`，资源总预算撤销。现代 Ex2/base/effective_url API 与 TEST46-48 保留；TEST46 新增兼容契约断言。`c89ize.py` 两文件均 0 修改，VS2008 ARM 增量构建 0 错误；设备包必须先验收 TEST13/46，随后每次只重新接入一项。

2026-07-14 二次回退：用户运行 next42 后 TEST13 仍无法加载，并重新运行 next37 确认完整正常。包时间戳把 next37 精确定位到 `9c5c7c7` 提交前工作树；其 `positron_http.dll` 为 19,968 字节，next42 为 20,480 字节。进度条持续移动表明 worker 仍卡在网络而非 UI commit，因此不再只做 core/host 语义兼容。`positron_http.c/.h` 已逐行恢复到 9c，撤销 1536 字节 `PHttpResponse.effective_url`、WinInet connect/receive options 和 TEST48；HTTP 产物恢复 19,968 字节。Ex2/base API 与 TEST46-47 保留。`c89ize.py` 对 HTTP/host 均 0 修改，增量构建 HTTP 0 警告、host 仅 3 条既有 C4244；新包只运行 TEST13，设备确认前不得记为通过。
