...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。next115 候选使用 `tests=13,20,27,43,44,56,58-77,79`，其中 TEST79 是普通非替换 float 离线断言；next114 的已验证设备基线仍使用去掉 `,79` 的 `tests=13,20,27,43,44,56,58-77`，TEST77 及整批日志已确认 `TESTBENCH PASS`。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
...
- 2026-08-04：next115/TEST79 是新的受控 float 候选，不是 TEST23 的恢复。`pcore_box.c` 仅处理普通非替换 `float:left/right`，先 blockify 普通/flex/table 子项，再包装为 NetSurf `BOX_FLOAT_*`；图片和表单控件排除。TEST79 的设备门禁尚未执行，若 TEST13 或冻结回归失败，应整体回退 next115，不要放宽几何断言掩盖问题。
