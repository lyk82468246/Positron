...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。next94 基线使用 `tests=13,20,27,43,44,56,58-65`，先跑冻结 Browse/图片/事务/table/inline/flex/selector/option/forms 门禁，再自动验证 WM 单行输入桥。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
