...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。当前 next156 已在 `screen=640x480 dpi=192` 通过至 TEST121；next159 已确认 TEST122 的 CESU-8/UTF-16 功能结果，失败来自 target 取消状态 oracle 写反，next160 为只修 oracle 的待验收包。next155 的 TEST121 失败包已由 next156 的 JSON UTF-8 修复替代；TEST79/float 候选已撤回。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
