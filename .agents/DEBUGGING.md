...
2026-07-14：TEST24/45/46 已由用户确认通过，stylesheet 的 rel token、alternate/type/disabled 过滤、完整 media query 与 cache-only 断点重选可作为当前离线基线。用户后来明确澄清：当时没有把 TEST13 的深层点击导航测到底，因此不得写成“TEST13/24/45/46 全部通过”，也不能用它证明 `c01a349` 前后的完整 Browse 行为。随后新增共享首个 `<base href>` 解析器，并让 `StyleDocumentEx2`、`FetchImageResourcesEx2`、`LinkAtEx2` 分别覆盖 CSS/import、图片与点击链接；旧 API 保持原始 URL 语义。图片缓存不能只保存一个 raw alias：多个相对写法可规范化到同一 URL，因此采用 per-resource alias 链，缓存命中时补登记别名而不重复抓取。TEST47 同时覆盖首个 base、第二个 base 忽略、CSS 背景、两个等价图片引用和 raw/absolute link，之后已由用户确认通过。

2026-07-14：TEST47 已由用户确认通过。随后 TEST13 复现 `Open example` → `Learn more` 偶发一分钟以上不提交；等待时反复拖动旧页滚动条还可能直接崩溃。线上核对发现链接先到 `https://iana.org/domains/example`，最终重定向为 `http://www.iana.org/help/example-domains`。第一轮为响应追加 `effective_url`，成功主文档 parse 前更新 host/path/port/document URL，为 WinInet 与可选资源加入预算，并让加载期滚动由 WM 合并重绘；TEST48 离线断言最终 origin。用户复测确认 TEST13 仍失败，说明第一轮归因和修复都不完整。进一步审计发现第一跳 HTTPS 使用的 `positron_tls` 从阻塞 `connect` 到 mbedTLS handshake/read/write 均无 deadline，之前的 WinInet/资源预算无法中断它。第二轮参照 vendored mbedTLS `net_sockets.c` 的 nonblocking/WANT 语义和 WinCE Winsock 文档，使用 `ioctlsocket(FIONBIO)`、`select`、`SO_ERROR` 驱动带 deadline 的 TCP/TLS；新增两个 timeout API，旧 API 转发到 30 秒默认，HTTP 使用 15 秒。`c89ize.py` 为 0 改动，VS2008 ARM 增量构建 TLS/HTTP/host 全部 0 错误，host 仅保留既有 libcss C4244。同步 DNS 与主动取消仍是限制，结果待设备复测。
