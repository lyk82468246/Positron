...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。当前 next156 已在 `screen=640x480 dpi=192` 通过至 TEST121；next158 已定位 TEST122 为 Duktape 四字节 UTF-8/UTF-16 表示差异，next159 以 JSON 代理项修复并等待设备日志。next155 的 TEST121 失败包已由 next156 的 JSON UTF-8 修复替代；TEST79/float 候选已撤回。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
