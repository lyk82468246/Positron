...
- 快速复测时同时核对 EXE 同目录的 `test_host.ini`。当前 next156 已在 `screen=640x480 dpi=192` 通过至 TEST121，next157 的默认配置扩展为 `tests=13,20,27,43,44,56,58-77,80-122`，但 TEST122 设备失败；next158 仅增加实际 result 诊断，设备日志待回传。next155 的 TEST121 失败包已由 next156 的 JSON UTF-8 修复替代；TEST79/float 候选已撤回。配置也支持 `tests=1-5 7b` 一类范围；启动提示选择 No 会回到原四组路由。`stage.bat` 会覆盖 staging 目录中的该配置文件。
