...
5. 每轮开发完成后复核 `ROADMAP.md`：移除已完成/过时候选，更新候选状态，或补充有证据支持的未来规划；若无需改动，也要在交接中确认已复核。
6. 运行 `python scripts/audit_repo.py`，检查职责、整体规模、异常膨胀的语义段落、UTF-8、链接和 `test_host` 产品边界。
...
`test_host` 是验证平台，不是产品实现的暂存区。凡是能被另一个 WM6 应用复用的 URL、资源事务、候选 generation/取消/退休/提交资格/结果分类、candidate/resource 组合 decision、文档、history、DOM/Event、表单、图像、脚本或生命周期语义，都必须由对应顶层 DLL 的源文件和公共头文件承载；宿主只能接入这些 API，并提供窗口、WM 消息、网络/线程调度、应用策略、fixture 与断言。审查 host-only 改动时，若它改变了上述语义，应先迁移到产品 DLL，再补宿主测试。`scripts/audit_repo.py` 会拒绝宿主编译产品 `.c` 文件或定义产品公共入口，但静态门不能替代语义审查。
