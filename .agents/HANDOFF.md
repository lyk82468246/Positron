...
- GDI Render：TEST 12、14、17、19、20、26，离线窗口渲染、WM Imaging 位图与 SVG path；TEST26 本地 ARM 构建通过、待设备确认。
...
6. 图片/SVG：TEST20 四格式缓存 `<img>` 与 TEST25 SVG parse 已真机确认。`positron_image.dll` 现新增 opaque SVG create/info/draw/free，采用 libnsfb 曲线细分 + WM GDI Polygon/Polyline；TEST26 待设备确认红/绿填充、蓝色 cubic stroke 和缩放。通过后接入缓存 `<img>`；复合孔洞、抗锯齿与 SVG text 仍是显式缺口。
