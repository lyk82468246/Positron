...
更新时间：2026-07-26
当前分支：`main`  
当前设备基线：next94，配置的 TEST13/20/27/43/44/56/58-65 已由无人值守设备日志确认全部 PASS。next92 的重叠文档 SVG 复用与 next93 的 checkbox/radio 点击、disabled、radio 分组、DOM checked 同步、局部 dirty rect、重排状态保持继续成立；next94 又新增单行 text/password 的 core UTF-8 ABI 与 WM 原生 `EDIT` 子窗口，TEST65 覆盖真实 `EN_CHANGE` 消息同步、maxlength、read-only/disabled、非法 UTF-8 拒绝和重排保持。label 激活、textarea、select/button 与提交仍不存在。**next78 仍是已撤回的失败实验，不得使用**。
...
- 快速配置：`test_host.exe` 同目录的 `test_host.ini` 当前使用 `tests=13,20,27,43,44,56,58-65`；也支持 `tests=1-5 7b` 一类语法。`auto=1` 时不弹 Yes/No/OK，窗口首帧后自动关闭，TEST13 自动跑 example.com → IANA Example Domains → Reserved Domains，并把每个原始结果和逐页遥测覆盖写入同目录 `test_host.log`；`auto=0` 保留 Yes/No 与原四组路由。自动首帧冒烟不替代新视觉能力的人工截图验收。缺失/无效配置不会静默改变测试范围，TEST23 不可选。
...
- GDI Render：TEST 12、14、17、19、20、26-37、39、46-58、62-65，离线窗口渲染、WM Imaging 位图、SVG path/cache/fallback/fill-rule、CSS background-image、原生 GDI text、线性/径向渐变、继承/透明 stop、同文档及重叠文档缓存复用、IANA token 间距、table span/匿名归一化/collapsed border/cell alignment/height distribution、列表 marker/counter/image/inside flow、HTML inline author CSS、checkbox/radio 交互、WM 原生单行输入桥与随包静态字体 fallback；TEST65 已由 next94 设备日志验收。
...
- next93 的 form 激活入口使用 document CSS px，与 `PCore_LinkAt` 相同；宿主先处理 overflow scrollbar，再处理 form control，最后才处理链接/空白关闭。控件状态同步回 libdom，因而 `WM_SIZE` 重建盒树后仍保留。next94 在此之上接通单行 text/password 与 WM 原生 `EDIT`；下一批从 textarea 开始，不要重写 radio 分组或单行输入同步逻辑。
...
> 2026-07-26 优先级调整：先解决“有没有”，再解决已有小范围“好不好”。checkbox/radio 与单行 text/password 已完成首批交互纵切；下一主线是 textarea，随后扩展 select/button、提交、事件/动态状态和重大布局缺口。已有 NetSurf Duktape backend 的 JavaScript 最小纵切提前到中期。首屏 SVG 冷解析、抗锯齿、渐变高级参数、视觉微调和全面性能优化后置，除非它们造成崩溃、数据错误或阻塞基本操作。
