...
- `TEST_MAX_NUMBER` 已为 1262。tracked `test_host/test_host.ini` 仍是窄 smoke：
  `auto=1`、`javascript=0`、选择 `13,20,27,56,58,62,64-67,73,75,1217-1262,999`。
...
- next817 的 Core-backed `document.title`、next818 的 bounded Element/Text
  ...
  bounded `getElementById()`、next821 的 bounded fragment selector 查询与 next822 的
  SameObject `children` collection 均已完成桌面
  构建、离线自动断言和正式设备门；下一短期目标是从 compatibility corpus、源码或新设备
  证据中选择一个可复现的 next823 用户可见缺口，并把它实现为一项边界清楚的公共 DLL 能力。
...
- `TEST_MAX_NUMBER`：1262。
- tracked `test_host/test_host.ini`：`auto=1`、`javascript=0`，选择 `13,20,27,56,58,62,64-67,73,75,1217-1262,999`。
- tracked INI 是窄 smoke，不是全量目录；nightly 打包脚本从源码 dispatch 动态生成全量自动清单。
- 设备连接必须先由用户在 WMDC/Device Emulator GUI 手动完成；RAPI gate 只使用当前唯一会话。

## 最新有效设备证据

`tmp/device-runs/20260916-092920-next822` 是本批最新证据：当前 GUI 连接的仿真器、Debug
ARMV4I、自动模式选择 `1261-1262,999`，3/3 PASS，零 ERROR/FAIL；外置
...
可用 `51,042,353,152` 字节，内部 object-store 可用 `2,682,880` 字节；当前部署目录已移除。
...
- TEST1201–1262 的 DOM/CharacterData、HTML parser、detached wrapper、属性 facade、
...
选择并实现 next823：先从 compatibility corpus、源码、设备日志或截图固定一个新的、可
