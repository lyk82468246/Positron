...
最新 next779 设备证据及此前验证见“最新有效设备证据”；更早传输失败只
保留在 Git 历史和 `docs/history/`，不作为通过依据。

- 当前代码 next779 在 mutation Ex6 的 existing-element `append()`/`prepend()` 之后补齐
  `before()`/`after()`；Core `PCore_NodeInsertElementChildAtById` 按未过滤 `childNodes`
  位置支持混合文本中的同父重排和跨父迁移。Browser 保留 wrapper/snapshot 语义，错误
  child、结构 token、detached、越界和多参数均 fail closed；TEST1220 已覆盖该纵切。
...
- `TEST_MAX_NUMBER` 已为 1220。tracked `test_host/test_host.ini` 仍是窄 smoke：
  `auto=1`、`javascript=0`、选择 `13,20,27,56,58,62,64-67,73,75,1217-1220,999`；nightly/device
...
- Browser/Core 的表单 owner、validation、submission、dialog、reset、`requestSubmit`、direct
  ...
  source-selection 生命周期已形成有界合同；TEST1170–1220 的逐项夹具、边界和错误回退
  ...
  语义在 Core/Browser 而非宿主。TEST1209–1220 还覆盖 Ex4/Ex5/Ex6/Ex2/Ex3 callback、
...
- 当前 fixed-buffer 数值转换、原子部署和 RAPI 日志恢复已由 `formal10` 正式门验证；`next766`
  ...
  `TEST1218,999` 门、next778 的 `TEST1219,999` 门和 next779 的 `TEST1220,999` 门也已通过；不得把
  测试宿主扩展当作产品语义实现。
...
- `TEST_MAX_NUMBER`：1220。
- tracked `test_host/test_host.ini`：`auto=1`、`javascript=0`，选择 `13,20,27,56,58,62,64-67,73,75,1217-1220,999`。
- tracked INI 是窄 smoke，不是全量目录；nightly 打包脚本从源码 dispatch 动态生成全量自动清单。
- 设备连接必须先由用户在 WMDC/Device Emulator GUI 手动完成；RAPI gate 只使用当前唯一会话。

## 最新有效设备证据

最新自动证据是 `20260911-004625-next779`：正式 Debug ARMV4I，选择 `1220,999`，
2/2、唯一 PASS、零 ERROR/FAIL，完整日志已回收，目标卷和内部 object-store 双空间
预检通过，当前部署在完整日志回收后已清理；运行前转储清单为 0，运行后仍为 0，
`crash_check=PASS`、`new_crash_dump_count=0`。TEST1220 证明 existing-element
`before()`/`after()` 在混合 Text/element 子节点中完成同父重排与跨父迁移，保持 wrapper
identity、旧 snapshot 和父级文本，并对 self、detached、Text 和多参数请求 fail closed。
...
- TEST1219–1220 是离线的 Core/Browser existing-element Ex6 夹具，覆盖
  `append()`/`prepend()` 的末尾/零位插入以及 `before()`/`after()` 的同父重排、跨父迁移、
  mixed Text/element 位置、wrapper identity、旧 snapshot、父级 textContent 和同节点
  no-op；自动门已验证 Ex6 callback 与 Core 位置 primitive 及 detached、Text、越界、多参数
  的 fail-closed 边界。该路径暂无新增立即人工风险；DocumentFragment、Comment/CDATA 插入、
