...
- 快速配置：`test_host.exe` 同目录的 `test_host.ini` 使用 `tests=32,33`、`tests=1-5 7b` 一类语法。读取成功后 Yes 只跑这些编号，No 回到原四组路由；缺失/无效不会静默改变测试范围。TEST23 不可选。`stage.bat` 会复制仓库默认配置，当前为 `tests=33`。
...
- GDI Render：TEST 12、14、17、19、20、26-35，离线窗口渲染、WM Imaging 位图、SVG path/cache/fallback/fill-rule、CSS background-image、原生 SVG text、缓存 SVG gradient/text、线性渐变坐标、中心径向渐变及缓存正式链；TEST26-34 已真机通过，TEST35 待设备。
...
5. TEST30-34 已于 2026-07-13 真机通过：CSS 背景定位/平铺/复用、libsvgtiny 文本元数据经 WM GDI 绘制及 path/text 顺序均成立；TEST32 原三角接缝已消除；TEST33 确认线性坐标与变换；TEST34 确认径向椭圆、圆和平移变换。TEST35 已构建待设备，用于验证径向 SVG 的文档缓存/replaced-box/NetSurf redraw 正式链；复杂 shaping、`textPath`、逐字定位、任意 shear、径向焦点/扩散/透明 stop 尚未实现。
...
7. 图片/SVG：TEST20 四格式缓存 `<img>`、TEST25-34 的 SVG parse/draw/cache/fallback/fill-rule/CSS background-image/basic text/连续线性渐变、坐标变换与中心径向渐变，以及 TEST13 网络相对 SVG 链均已真机确认。TEST35 径向缓存正式链已完成 ARM 构建待设备；复杂文本、径向焦点/扩散/透明 stop、background-size、多层背景与异步资源事务仍是显式缺口。
