...
- 分支：`main`。next707 的 Browser selector 语义、TEST1152 夹具和职责文档已完成；
  tracked 范围只涉及 `positron_browser`、`test_host` 和对应文档，未把 `tmp/`
  中的本地证据纳入版本控制。
- `TEST_MAX_NUMBER` 已为 1152。tracked `test_host/test_host.ini` 仍是窄 smoke：
...
- next707 已完成 Browser selector 列表/组合器纵向能力；TEST1152、`1152,999` 定向
  门和 `1151-1152,999` 相邻回归门均已通过。当前唯一下一步是 next708：重新检查
  compatibility corpus、源码、设备日志和截图，固定一个新的用户可见缺口，再选择
  一个边界清楚的公共 DLL 纵向能力。不要仅为增加编号拆分提交，也不要把页面语义
  放回 `test_host`。
...
当前最新产品门为 next707 的 Browser selector 列表/组合器与 next706 autofocus 相邻回归：
...
- 独立定向目录：`tmp/device-runs/20260901-204404-next707-selector-mem/`；动态选择
  `1152,999`，2 项；2/2 通过，确认把 Browser bootstrap 堆上限调整到 678 KiB 后，
  新 selector 语义可稳定启动。
...
- 静态验证：`python scripts/test_c89ize.py`、正式 Debug ARMV4I 构建和
  `python scripts/audit_repo.py` 均通过；审计确认 selector 实现仍在
  `positron_browser`、宿主边界门通过，文档结构与本地链接无新增问题。Browser 会话
  的有界 heap ceiling 已从 662 KiB 调整为 678 KiB，以容纳新增 bootstrap。
...
- TEST1117–TEST1152 都是离线自动夹具，没有新增必须立即人工复核的崩溃或数据风险；它们的真实视觉、触摸、旋转、SIP/IME、picker 和不同 DPI 呈现继续进入人工累计清单。自动结果不替代真实网络恢复、OEM 控件或逐资源视觉验收。
...
- TEST1152 是离线的 Browser selector 语义夹具，没有新增必须立即人工复核的崩溃或数据风险；真实页面的复杂 CSS selector、伪类/伪元素、属性操作符、namespace、shadow DOM、布局视觉和不同 DPI 仍属于宿主集成观察，自动门只证明有界列表/组合器、属性值分隔、查询顺序和非法输入 fail-closed 合同。
...
- Browser selector 组合器目前只覆盖简单 compound selector 的顶层列表、后代/子代/兄弟关系；TEST1152 已验证 64 步上限和非法输入回退，但完整 CSS Selectors 语法、伪类/伪元素、属性操作符、namespace 和 shadow DOM 仍未实现。新增 bootstrap 也使浏览器脚本有界 heap ceiling 提升到 678 KiB，复杂页面仍可能触及资源预算。
...
next707 已在 Browser bootstrap 中补齐有界 selector 列表和关系组合器：TEST1152、
`1152,999` 定向设备门和 `1151-1152,999` 相邻回归验证了查询一致性、属性值逗号保护、
非法输入 fail-closed 和既有 autofocus 路径。下一步先从 compatibility corpus、源码、
日志或截图固定另一个真实缺口，再选择一个边界清楚的离线 fixture 或稳定哨兵。
...
- handoff 覆盖为 next707 快照，ROADMAP 只保留当前尚未完成的纵向能力。
