...
7. 图片/SVG：TEST20 四格式缓存 `<img>`、TEST25-37 的 SVG 正式链，以及 TEST13 网络相对 SVG 链均已真机确认。2026-07-15 又把 WM 位图 create/info/draw/free 正式移入 `positron_image.dll`，输入由 DLL 复制、NetSurf 重绘复用 retained handle、旧 `PCore_Image*` 兼容转发；next45 的 TEST19/20/26/27 与 TEST13 已确认迁移无回归。ABI 1.0 查询和只导入图片 DLL/COREDLL 的独立 WM 示例已通过 VS2008 ARM 与 PE 审计，next46 待真机确认。复杂文本、径向焦点、spread method、background-size 和多层背景仍是显式缺口。
