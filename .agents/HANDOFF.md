...
- 最新已验证产品基线：next325（本批采用定向门；最近一次完整自动基线仍为 next255）。
...
- TEST293/999、C89、审计和正式构建均保持通过；下一次启用 JavaScript 的相关回归采用
