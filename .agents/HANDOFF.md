...
更新时间：2026-08-08
当前分支：`main`  
当前候选：next155 已完成 C89、仓库审计和 VS2008 ARMV4I Debug 增量构建，设备验收待运行；
新增单个 BMP `WM_CHAR` 的 UTF-8 `keypress`/EDIT `beforeinput.data` 桥，TEST121 待设备
确认。代理对、IME/composition 未实现，默认 `javascript=0` 与 TEST13 路径不变。
...
的网络路径。next155 的 BMP 字符桥仍不应在设备日志通过前写成已完成能力。
...
`keypress` 和 EDIT `beforeinput.data` 桥，TEST121 使用 `→`/`★` 检查
`keyCode/charCode`、target/bubble、UTF-8 key 与取消 SELECT 默认动作。C89、仓库审计、
ARMV4I 增量构建已通过，待 staging 与轮换分辨率/DPI 的设备验收；代理对、IME/composition
和完整 Unicode 输入不在范围内。
