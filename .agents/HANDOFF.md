...
- next462–481 已完成第四组 20 个完整的浏览器 JavaScript 产品子功能：TextEncoder.encodeInto、
  TextDecoder 选项快照、同步 Request/Response JSON、Blob Request clone、Headers getSetCookie、
  Storage named properties/toJSON、classList/style iterator、toggleAttribute、节点 ownership、
  window aliases/defaultView/open/close 边界、Storage/HashChange/PopState/Error/Progress/Close
  事件构造器、MessagePort 生命周期与 messageerror、同 session BroadcastChannel、同步快照
  PerformanceObserver，以及 AbortSignal.abort/throwIfAborted。对应自动测试为 TEST429–448，
  实现仍位于 `positron_browser.dll`，`test_host.exe` 只提供 fixture、callback 注册和断言。
- next462–481 定向门 `TEST429-448,999` 通过 21/21，累计门 `TEST369-448,999` 通过 81/81；
  两次最终门均为零 ERROR、零 FAIL、唯一 TESTBENCH PASS、`test13_route_ok=True`。最终证据分别位于
  `tmp/device-runs/20260821-103115-next462-481-final-r2/` 和
...
- next522–541 已完成第七组 20 个完整的浏览器 JavaScript 产品子功能：受宿主显式
  `__pcoreRunMicrotasks()` 驱动的 bounded Promise 构造器与微任务时序、then/catch 链、
  rejection recovery、thenable assimilation、executor/finally、`resolve`/`reject`、
  `all`/`race`/`allSettled`/`any`，以及构造器错误、AggregateError-like rejection、64 项
  handler/input bound 和 Promise tag。实现位于 `positron_browser.dll`，`test_host.exe` 只提供
  fixture、pump 和断言；没有新增公共 C ABI。
- next522–541 定向设备门 `TEST522-541,999` 在
  `tmp/device-runs/20260821-120238-next522-541-promise/` 通过 21/21；相邻回归门
  `TEST389,390-448,482-541,999` 在 `tmp/device-runs/20260821-120337-next522-541-regression/`
  通过 121/121。两次均为零 ERROR、零 FAIL、唯一 TESTBENCH PASS、`test13_route_ok=True`。
  本批没有视觉、真实触摸、SIP、旋转、系统 picker 或网络失败，因此不新增人工页面验收。
- 本批 bootstrap 现在由公共入口按顺序评估八个 IIFE，仍共享同一 `positron_script.dll` context；
...
- next562–581 已完成第九组 20 个完整的浏览器 JavaScript 产品子功能：`positron_core.dll` 新增
  按 DOM id 的 attribute count/name/value relation；`positron_browser.dll` 在 bootstrap 中提供
  `getAttributeNames()`、`hasAttributes()`、受限 `NamedNodeMap`（`length`、`item()`、
  `getNamedItem()`、`setNamedItem()`、`removeNamedItem()`、8 个 indexed slots 和 iterator）与
  `Attr` 的 metadata、live value/nodeValue、owner/identity 以及跨 owner fail-closed 保护。
  `test_host.exe` 只提供 fixture、callback adapter 和 TEST562–581 断言。
- `next562–581` 定向设备门 `TEST562-581,999` 在
  `tmp/device-runs/20260821-142414-next562-581-attr-owner-guard/` 通过 21/21；相邻回归门
  `TEST389,390-448,482-581,999` 在
  `tmp/device-runs/20260821-142515-next562-581-regression-final/` 通过 161/161。两次均为零
  `ERROR`、零 `FAIL`、唯一 `TESTBENCH PASS`、`test13_route_ok=True`。
- 本批 bootstrap 现在由公共入口按顺序评估十个 IIFE。为容纳新增属性集合并保持既有
  TEST540 Promise 边界，浏览器 session 使用显式 576 KiB heap ceiling；独立
  `positron_script.dll` 的 512 KiB 默认值不变。NamedNodeMap 的索引槽位固定为 0–7，关系查询
  仍是同步、ID-addressable、内存内 snapshot；不提供 namespace API、通用 DOM mutation、
  节点创建、live collection、layout 或 native control 语义。
...
- next584 作为一个单一批次在既有 DOM snapshot 上补齐了有界集合协议：`childNodes`、`children`、
  `form.elements` 和元素作用域 `querySelectorAll()` 结果现在提供 `forEach()`、`keys()`、
  `values()`、`entries()`、可复用的默认迭代器和 `Symbol.toStringTag`；`children` 与
  `form.elements` 保留 `namedItem()`，元素作用域查询返回 `NodeList` 语义，不新增 core ABI。
  `test_host.exe` 只提供 fixture、adapter 和 `TEST622–641` 断言。
- `TEST622-641,999` 在 `tmp/device-runs/20260821-160529-next584/` 通过 21/21；相邻回归
  `TEST389,390-448,482-641,999` 在
  `tmp/device-runs/20260821-160635-next584-regression/` 通过 221/221。两次均为零
  `ERROR`、零 `FAIL`、唯一 `TESTBENCH PASS`、`test13_route_ok=True`；本批只涉及同步、只读、
  session-scoped 集合快照，不涉及视觉、真实触摸、SIP、旋转、系统 picker 或网络失败，
  因此不新增人工页面验收；tracked `test_host.ini` 仍为 `javascript=0`。

- next585 作为一个单一批次在既有 relation bridge 上增加了文档结构入口：
  `positron_core.dll` 为没有 HTML `id` 的 document root、直接 `head` 和直接 `body` 提供三个
  保留结构 token；`positron_browser.dll` 将其暴露为稳定的 `documentElement`/`head`/`body`
  wrapper，并接通 document root selector、parent/child/sibling、children/childNodes、
  identity/root/position/contains 和集合协议。真实 id 查找优先于 token fallback，结构节点不
  伪造 `id`，通用 DOM 创建/mutation/live collection/复杂 selector 仍未实现；`test_host.exe`
  只提供 fixture、adapter 和 `TEST642–661` 断言。为保持结构入口后的 document order 语义，
  既有 TEST549 的 root/form 位置断言从缺失 root parent 时的 `33` 更新为同一 body snapshot
  中的顺序值 `4`。
- `TEST642-661,999` 在 `tmp/device-runs/20260821-172711-next585/` 通过 21/21；
  `TEST549,642-661,999` 的兼容重跑在 `tmp/device-runs/20260821-173444-next585-r2/`
  通过 22/22。最终相邻回归 `TEST389,390-448,482-661,999` 在
  `tmp/device-runs/20260821-175025-next585-regression-r3/` 通过 241/241；三次最终门均为
  零 ERROR、零 FAIL、唯一 TESTBENCH PASS、`test13_route_ok=True`。
...
- next587 作为一个单一批次在 browser 层为 document、DocumentType、HTML element、
  CharacterData 和 Attr 补齐了受控 `baseURI`、HTML namespace、`prefix`、
  `lookupNamespaceURI()` 与 `isDefaultNamespace()` 元数据；baseURI 随当前 session URL 更新，
  namespace 只承诺 HTML/XML 两个已知值，未知 prefix 和非 HTML 节点 fail closed。没有新增
  core ABI、namespace parser、节点创建或 mutation；`test_host.exe` 只提供 URL fixture、adapter
  和 `TEST682–701` 断言。
- `TEST682-701,999` 在 `tmp/device-runs/20260822-142806-next587/` 通过 21/21；兼容门首尝
  在 TEST678 遇到一次设备 JavaScript timeout，未放宽预算或断言，原配置重跑在
  `tmp/device-runs/20260822-143147-next587-compat-r2/` 通过 62/62；最终相邻回归
  `TEST389,390-448,482-701,999` 在
  `tmp/device-runs/20260822-143521-next587-regression/` 通过 281/281。最终三次门均为零
  ERROR、零 FAIL、唯一 TESTBENCH PASS、`test13_route_ok=True`。本批只涉及同步脚本
  Node metadata，不新增人工页面验收；tracked `test_host.ini` 仍为 `javascript=0`。

- next588 作为一个单一批次在 browser 层完成了受控的
  `getElementsByTagName()`/`getElementsByClassName()` 查询族：document 和 HTML element
  wrapper 返回按深度优先文档顺序排列的 HTMLCollection snapshot，支持 tag 大小写归一、`*`、
  多 class token、`item()`/`namedItem()`、`forEach()`/`keys()`/`values()`/`entries()`、默认
  iterator 和 `Symbol.toStringTag`；元素查询排除 owner，document 查询包含 structural
  `documentElement`。空白/未知输入 fail closed，集合不 live、不承担通用 selector 或 mutation，
  也没有新增 core ABI；`test_host.exe` 只提供 fixture、adapter 和 `TEST702–721` 断言。
- `TEST702-721,999` 在 `tmp/device-runs/20260822-152000-next588-final/` 通过 21/21；
  `TEST549,642-721,999` 兼容门在 `tmp/device-runs/20260822-152109-next588-compat-final/`
  通过 82/82；相邻回归 `TEST389,390-448,482-721,999` 在
  `tmp/device-runs/20260822-152431-next588-regression-final/` 通过 301/301。三次均为零
  ERROR、零 FAIL、唯一 TESTBENCH PASS、`test13_route_ok=True`。本批只涉及同步脚本 API/DOM
  snapshot，不涉及视觉、触摸、SIP、系统 picker、旋转或网络失败，因此不新增人工页面验收。
- 新增 collection bootstrap 后，576 KiB browser session ceiling 在 TEST540 的既有 Promise
  boundary 上稳定触发内存上限；没有放宽断言，也没有修改独立脚本 context。browser session
  ceiling 已明确提高到 608 KiB，`positron_script` 默认堆仍为 512 KiB；
...
- next590 作为一个单一批次在 browser 层增加了 document named collection 纵切：
  `document.getElementsByName()` 返回按 DFS 顺序排列的 NodeList snapshot，严格匹配显式
  `name` 值；`document.forms`、`document.images` 和 `document.scripts` 提供相应的静态
  HTMLCollection projection，并复用既有 `item()`/`namedItem()`、迭代协议和 wrapper identity。
  这些 getter 只覆盖当前 ID-addressable bounded snapshot，不提供 live 更新、节点创建、通用
  mutation 或新的 core ABI；`test_host.exe` 只提供 fixture、adapter 和 `TEST742–761` 断言。
- `TEST742-761,999` 在 `tmp/device-runs/20260822-163511-next590-r1/` 通过 21/21；兼容门
...
- next591 作为一个单一批次在 browser 层增加了 hyperlink collection 纵切：
  `document.links` 按 DFS 顺序收集显式 `href` 的 `a`/`area`，`document.anchors` 按 DFS 顺序
  收集显式 `name` 的 `a`，两者均返回静态 HTMLCollection，并复用既有
  `item()`/`namedItem()`、迭代协议和 wrapper identity。属性增删只影响后续快照，不提供 live
  更新、通用 named properties、节点创建、mutation 或新的 core ABI；`test_host.exe` 只提供
  fixture、adapter 和 `TEST762–781` 断言。
- `TEST762-781,999` 在 `tmp/device-runs/20260822-171429-next591-r1/` 通过 21/21；兼容门
  ...
  ERROR、零 FAIL、唯一 TESTBENCH PASS、`test13_route_ok=True`。本批未跑旧的 341 项全回归，
  但保留了核心事件、TEST540 内存边界、TEST549 和 next642–781 风险区间；本批只涉及同步脚本
...
Observer option validation、MessagePort auto-start 和 Blob/File JSON metadata；本轮又增加 bounded
...
快照补齐 `forEach()`、`keys()`、`values()`、`entries()`、默认迭代器和集合类型标识，仍不提供
live collection 或通用 mutation。next585 又为没有 HTML `id` 的 document root、直接 `head`/
`body` 引入三个保留结构 token，并在 browser 层提供稳定的 `documentElement`、`head`、`body`
wrapper；既有关系、集合、selector、Node identity/root/position/contains 视图因此可以从
...
Node 的 baseURI 和有限 namespace metadata 统一到这些 wrapper：当前 URL 由 session location
...
next590 又在同一 browser-owned traversal 上增加 document named collection projection：
`getElementsByName()` 精确匹配显式 `name` 并返回 NodeList snapshot，`forms`/`images`/`scripts`
getter 返回对应 HTMLCollection snapshot；它们共享 DFS 顺序、`item()`/`namedItem()`、迭代器和
稳定 wrapper identity，但不提供 live collection、通用 DOM mutation 或 core ABI。
next591 又在同一 traversal 上增加 `document.links` 与 `document.anchors`：links 只收集显式
...
`next593` 在同一 browser-owned attribute snapshot 上增加只读的 namespace-aware attribute
projection：`getAttributeNS()`、`hasAttributeNS()`、`getAttributeNodeNS()`，以及 Attr 的
`namespaceURI`/`prefix`/`localName` 元数据。null 与空 namespace 等价于无 namespace；已知的
`xml`/`xmlns` 前缀映射到 XML/XMLNS namespace，未知前缀和未知 namespace fail closed；Attr
节点仍复用同 owner 的 live value/nodeValue wrapper。该批不提供 `setAttributeNS()`、
`removeAttributeNS()`、XML/SVG parser、prefix mutation、节点创建、live collection 或新的
core ABI；`test_host.exe` 只提供 fixture、adapter 和 `TEST802–821` 断言。最终定向门
`TEST802-821,999` 在 `tmp/device-runs/20260822-201726-next593-r2/` 通过 21/21，兼容门
`TEST549,642-821,999` 在 `tmp/device-runs/20260822-201838-next593-compat-r2/` 通过 182/182，
缩减回归 `TEST389,390-448,540,549,642-821,999` 在
`tmp/device-runs/20260822-202712-next593-regression-r2/` 通过 243/243；三次均为零
ERROR/FAIL 且 `TESTBENCH PASS` 唯一。本批只涉及同步脚本 API/DOM snapshot，不涉及视觉、触摸、
SIP、系统 picker、旋转或网络失败，因此不新增人工页面验收。

`next594` 在同一 browser-owned `NamedNodeMap` 上增加只读的
...
语义；保留的 map 会观察后续属性增删和值更新。该批不提供 `setNamedItemNS()`、
`removeNamedItemNS()`、XML/SVG parser、namespace mutation、节点创建、live collection 或新的
core ABI；`test_host.exe` 只提供 fixture、adapter 和 `TEST822–841` 断言。定向门
`TEST822-841,999` 在 `tmp/device-runs/20260822-204905-next594-r1/` 通过 21/21，缩减回归
`TEST389,390-448,540,549,642-841,999` 在
`tmp/device-runs/20260822-205012-next594-regression-r1/` 通过 263/263；两次均为零
ERROR/FAIL 且 `TESTBENCH PASS` 唯一。本批只涉及同步脚本 API/DOM snapshot，不涉及视觉、触摸、
SIP、系统 picker、旋转或网络失败，因此不新增人工页面验收；兼容子集沿用 next593 的
`TEST549,642-821,999` 证据，未重复同一风险区间。

`next595` 在同一 browser-owned namespace metadata 上增加只读的 `lookupPrefix(namespace)`。
document、DocumentType、HTML element、CharacterData 和 Attr wrapper 都提供该方法：已知 XML
namespace 返回 `xml`；XMLNS namespace 只在对应 `xmlns:*` Attr 上返回 `xmlns`；HTML default
namespace、null/空值、未知 namespace 和未知 Attr prefix 均 fail closed。参数接受有限的 String
coercion，但不执行 namespace declaration/parser 解析，也不新增 `positron_core.dll` ABI、prefix
mutation、节点创建或 live collection。`test_host.exe` 只提供 fixture、adapter 和 `TEST842–861`
断言。定向门 `TEST842-861,999` 在
...
`next599` 在既有 Attr leaf wrapper 上补齐 detached-node relation 语义：`isConnected` 固定为 false，
`getRootNode()` 返回 Attr 自身且忽略 bounded `composed` 选项，`contains()` 只对自身返回 true；
`compareDocumentPosition()` 对自身返回 0，对其他 Attr、owner element、null 或非法对象统一返回
`33`（`DISCONNECTED|IMPLEMENTATION_SPECIFIC`）。该批不把 ownerElement 伪造成 parent、不扩展
...
`next601` 为既有 `document.doctype` snapshot 补齐只读 `publicId`、`systemId` 和
`internalSubset` 元数据；HTML doctype 默认值分别为空字符串、空字符串和 `null`，并保持
不可变、稳定 branding、document 关系和 namespace/baseURI 语义。`TEST962-981,999` 首次尝试的
TEST968 仅因测试期望多写一个分隔符失败，修正断言后在
`tmp/device-runs/20260823-115525-next601/` 通过 21/21；namespace/Attr/child-wrapper/doctype
缩减回归 `TEST802-981,999` 在
`tmp/device-runs/20260823-115645-next601-regression/` 通过 181/181。两次最终运行均为零
ERROR/FAIL 且 `TESTBENCH PASS` 唯一。本批只涉及同步脚本 API/DOM snapshot，不涉及视觉、触摸、
SIP、系统 picker、旋转或网络失败，因此不新增人工页面验收；tracked `test_host.ini` 仍保持
`javascript=0`。

`next602` 在既有 `document.doctype` snapshot 上增加了独立、稳定、冻结的空 `entities` 与
`notations` `NamedNodeMap` 视图。两者的 `length` 为 0，固定 indexed slots 和 named/namespace
lookup 均返回空结果，iterator 立即结束，mutation 方法 fail closed；它们带有
`NamedNodeMap` branding，但不解析 DTD/实体，也不创建节点或扩展 `positron_core.dll` ABI。
实现通过 lazy helper 复用既有 `m10(null)`，避免复制 bootstrap 实现和增加脚本堆压力。
`TEST982-998,999` 在 `tmp/device-runs/20260823-122704-next602/` 通过 18/18，
`TEST802-998,999` 在 `tmp/device-runs/20260823-122827-next602-regression/` 通过 198/198；
两次最终运行均为零 ERROR/FAIL、唯一 `TESTBENCH PASS` 且 `test13_route_ok=True`。首次长回归曾在
...
`next604` 为静态 `HTMLCollection` snapshot 增加了只读、不可枚举的 `id`/`name` 直达属性；
属性值复用 snapshot 中的 element wrapper，`item()`、`namedItem()`、数字索引、集合 iterator
和 reserved member 保持稳定，`NodeList` 不获得这组 named projection。属性只在 snapshot 创建时
定义，不承诺 live 更新；未知名称、空 collection、赋值和删除均 fail closed。实现仍在
`positron_browser.dll`，不扩展 core ABI、节点 mutation 或 live collection。
`TEST1018-1035,999` 在 `tmp/device-runs/20260823-134449-next604-r2/` 通过 19/19，
缩减回归 `TEST802-998,1000-1035,999` 在
`tmp/device-runs/20260823-134557-next604-regression/` 通过 234/234；两次最终运行零
...
- next328 在同一 bridge 上增加 `HTMLElement.lang` raw 反射；TEST295 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不涉及语言解析、本地化或
  人工页面验收。`TEST295/999` 与最近 `TEST264-295/999` 均已通过。
- next329 在同一 bridge 上增加 `HTMLElement.dir` raw 反射；TEST296 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不涉及 CSS 方向布局或人工
  页面验收。`TEST296/999` 与最近 `TEST264-296/999` 均已通过。
- next330 在同一 bridge 上增加 `HTMLElement.hidden` 布尔反射；TEST297 自动覆盖初始值、
  attribute 设置、property true/false、移除恢复和布尔 getter，不涉及隐藏布局算法、视觉
  或人工页面验收。`TEST297/999` 与最近 `TEST264-297/999` 均已通过。
- next331 在同一 bridge 上增加 `HTMLElement.accessKey` raw 反射；TEST298 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不触发 WM 快捷键或焦点副作用。
  `TEST298/999` 与最近 `TEST264-298/999` 均已通过。
- next332 在同一 bridge 上增加 `HTMLElement.role` raw 反射；TEST299 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现辅助技术树或语义计算。
  `TEST299/999` 与最近 `TEST264-299/999` 均已通过。
- next333 在同一 bridge 上增加 `HTMLElement.ariaLabel` ↔ `aria-label` raw 反射；TEST300 自动
  覆盖初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 ARIA 语义
  树或辅助技术计算。`TEST300/999` 与最近 `TEST264-300/999` 重试均已通过；首次近期门的
  既有 TEST299 bootstrap timeout 不作为基线。
- next334 在同一 bridge 上增加 `HTMLElement.contentEditable` raw 反射；TEST301 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不改变 layout、编辑控件或
  native IME 行为。`TEST301/999` 与最近 `TEST264-301/999` 均已通过。
...
- next337 在同一 bridge 上增加 `HTMLInputElement.accept` raw 反射；TEST304 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现文件类型解析、过滤或
  系统 picker 行为。`TEST304/999` 与最近 `TEST264-304/999` 均已通过。
- next338 在同一 bridge 上增加 `HTMLInputElement.capture` raw 反射；TEST305 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现摄像头/麦克风捕获、文件
  类型过滤或系统 picker 行为。`TEST305/999` 与最近 `TEST264-305/999` 均已通过。
- next339 在同一 bridge 上增加 `HTMLInputElement.dirname` raw 反射；TEST306 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不改变提交方向字段、编码或
  其他表单默认行为。`TEST306/999` 与最近 `TEST264-306/999` 均已通过。
- next340 在同一 bridge 上增加 `HTMLInputElement.list` raw 反射；TEST307 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 datalist 解析、建议项或
  自动完成行为。`TEST307/999` 与重试后的最近 `TEST264-307/999` 均已通过；首次近期门的
  既有 TEST296 bootstrap timeout 不作为基线。
- next341 在同一 bridge 上增加 `HTMLTextAreaElement.wrap` raw 反射；TEST308 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现软/硬换行布局或提交编码
  差异。`TEST308/999` 与最近 `TEST264-308/999` 均已通过。
- next342 在同一 bridge 上增加 `HTMLElement.htmlFor` ↔ `for` raw 反射；TEST309 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回。一次追加 `className` 的候选在
  bootstrap 阶段因已有不可重定义描述符报告 `TypeError: not configurable`，已撤回并记录在
  `FAILED_EXPERIMENTS.md`；不得把该失败当作环境噪声。`TEST309/999` 与最近
  `TEST264-309/999` 均已通过。
- next343 在同一 bridge 上增加 `HTMLElement.slot` ↔ `slot` raw 反射；TEST310 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 Shadow DOM slot 分配。
  `TEST310/999` 与最近 `TEST264-310/999` 均已通过。
- next344 在同一 bridge 上增加 `HTMLElement.itemId` ↔ `itemid` raw 反射；TEST311 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 microdata 解析或语义树。
  `TEST311/999` 与最近 `TEST264-311/999` 均已通过。
- next345 在同一 bridge 上增加 `HTMLElement.itemProp` ↔ `itemprop` raw 反射；TEST312 自动覆盖
  初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 microdata token
  解析或语义树。`TEST312/999` 与最近 `TEST264-312/999` 均已通过。
- next346 在同一 bridge 上增加 `HTMLElement.itemRef` ↔ `itemref` raw 反射；TEST313 自动覆盖
  初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 microdata 引用
  解析或语义树。`TEST313/999` 与最近 `TEST264-313/999` 均已通过。
- next347 在同一 bridge 上增加 `HTMLElement.itemScope` ↔ `itemscope` 布尔反射；TEST314 自动
  覆盖初始值、attribute 设置、property true/false、移除恢复和布尔 getter，不实现 microdata
  item 解析或语义树。`TEST314/999` 与最近 `TEST264-314/999` 均已通过。
- next348 在同一 bridge 上增加 `HTMLElement.itemType` ↔ `itemtype` raw 反射；TEST315 自动覆盖
  初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 microdata
  vocabulary 解析或语义树。`TEST315/999` 与最近 `TEST264-315/999` 均已通过。
- next349 在同一 bridge 上增加 `HTMLElement.nonce` ↔ `nonce` raw 反射；TEST316 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 CSP、安全策略或脚本
  执行语义。首个 `<script>` 夹具没有产生 probe 结果，已改为普通 `<div>` 后重跑；
  `TEST316/999` 与最近 `TEST264-316/999` 均已通过。
- next350 在同一 bridge 上增加 `HTMLElement.part` ↔ `part` raw 反射；TEST317 自动覆盖初始值、
  `setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 Shadow DOM 部件导出、
  CSS 选择器语义或完整 HTMLElement Web IDL。`TEST317/999` 与最近 `TEST264-317/999` 均已通过。
- next351 在同一 bridge 上增加 `HTMLElement.exportParts` ↔ `exportparts` raw 反射；TEST318 自动
  覆盖初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 Shadow DOM
  部件导出算法或完整 HTMLElement Web IDL。`TEST318/999` 与最近 `TEST264-318/999` 均已通过。
- next352 在同一 bridge 上增加 `HTMLElement.inert` 布尔反射；TEST319 自动覆盖初始存在性、
  attribute 设置、property true/false、移除恢复和布尔 getter，不实现焦点、键盘、无障碍树或
  完整 HTMLElement Web IDL。`TEST319/999` 与最近 `TEST264-319/999` 均已通过。
- next353 在同一 bridge 上增加 `HTMLElement.popover` ↔ `popover` raw 反射；TEST320 自动覆盖
  初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现 popover 显示、
  隐藏、焦点管理、top-layer 或完整 HTMLElement Web IDL。`TEST320/999` 与最近
  `TEST264-320/999` 均已通过。
- next354 在同一 bridge 上增加 `HTMLElement.autofocus` 布尔反射；TEST321 自动覆盖初始存在性、
  attribute 设置、property true/false、移除恢复和布尔 getter，不实现焦点调度或窗口激活。
  最近门首尝在既有 TEST266 处出现 JavaScript bootstrap timeout，原选择重跑后
  `TEST321/999` 与 `TEST264-321/999` 均已通过。
- next355 在同一 bridge 上增加 `HTMLInputElement.enterKeyHint` ↔ `enterkeyhint` raw 反射；
  TEST322 自动覆盖初始值、`setAttribute`、property setter、移除恢复和 UTF-8 DOM 读回，不实现
  SIP、键盘布局或输入法策略。`TEST322/999` 与最近 `TEST264-322/999` 均已通过。
- next356 在同一 bridge 上增加 `HTMLInputElement.virtualKeyboardPolicy` ↔
  `virtualkeyboardpolicy` raw 反射；TEST323 自动覆盖初始值、`setAttribute`、property setter、
  移除恢复和 UTF-8 DOM 读回，不实现 SIP 或虚拟键盘策略执行。`TEST323/999` 与最近
  `TEST264-323/999` 均已通过。
- next357 在同一 bridge 上增加 `HTMLInputElement.webkitDirectory` 布尔反射；TEST324 自动
  覆盖初始存在性、attribute 设置、property true/false、移除恢复和布尔 getter，不触发目录
  picker 或实现目录选择语义。`TEST324/999` 与最近 `TEST264-324/999` 均已通过。
- next358 在同一 bridge 上为 `HTMLInputElement.size` 接入有限整数反射；TEST325 自动覆盖
  初始/attribute/property 整数往返、malformed 回落和移除恢复，不声明默认 20、控件宽度、
  范围钳制或原生输入 UI 语义。`TEST325/999` 与最近 `TEST264-325/999` 均已通过。
- next359 在同一 bridge 上为 `HTMLTextAreaElement.cols` 接入有限整数反射；TEST326 自动覆盖
  初始/attribute/property 整数往返、malformed 回落和移除恢复，不声明 textarea 布局宽度或
  完整 Web IDL 语义。`TEST326/999` 与最近 `TEST264-326/999` 均已通过。
- next360 在同一 bridge 上为 `HTMLTextAreaElement.rows` 接入有限整数反射；TEST327 自动覆盖
  初始/attribute/property 整数往返、malformed 回落和移除恢复，不声明 textarea 布局高度或
  完整 Web IDL 语义。`TEST327/999` 与最近 `TEST264-327/999` 均已通过。
...
- next324 自动候选证据：`TEST292/999` 2/2 位于
  ...
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next325 自动候选证据：`TEST293/999` 2/2 位于
  `tmp/device-runs/20260820-145244-next325-reset-metadata-js/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近回归首尝在既有 TEST266 bootstrap timeout 处仅完成
  2/31，未作为基线；重试后 `TEST264-293/999` 31/31 位于
  `tmp/device-runs/20260820-145332-next325-reset-metadata-recent-js-retry/`，零 ERROR/FAIL，tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
...
- next327 自动候选证据：`TEST294/999` 2/2 位于
  ...
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next328 自动候选证据：`TEST295/999` 2/2 位于
  `tmp/device-runs/20260820-153036-next328-lang-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-295/999` 33/33 位于
  `tmp/device-runs/20260820-153155-next328-lang-reflection-recent/`；tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next329 自动候选证据：`TEST296/999` 2/2 位于
  `tmp/device-runs/20260820-154035-next329-dir-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-296/999` 34/34 位于
  `tmp/device-runs/20260820-154115-next329-dir-reflection-recent/`；tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next330 自动候选证据：`TEST297/999` 2/2 位于
  `tmp/device-runs/20260820-155105-next330-hidden-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-297/999` 35/35 位于
  `tmp/device-runs/20260820-155138-next330-hidden-reflection-recent/`；tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next331 自动候选证据：`TEST298/999` 2/2 位于
  `tmp/device-runs/20260820-155713-next331-accesskey-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-298/999` 36/36 位于
  `tmp/device-runs/20260820-155741-next331-accesskey-reflection-recent/`；tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next332 自动候选证据：`TEST299/999` 2/2 位于
  `tmp/device-runs/20260820-160251-next332-role-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-299/999` 37/37 位于
  `tmp/device-runs/20260820-160315-next332-role-reflection-recent/`；tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next333 自动候选证据：`TEST300/999` 2/2 位于
  `tmp/device-runs/20260820-160826-next333-aria-label-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。独立 `TEST299-300/999` 3/3 位于
  `tmp/device-runs/20260820-161029-next333-aria-label-focus-retry/`；近期链首次在 TEST299
  的环境 timeout 处 35/38，未作为基线；重试 `TEST264-300/999` 38/38 位于
  `tmp/device-runs/20260820-161116-next333-aria-label-reflection-recent-retry/`，tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next334 自动候选证据：`TEST301/999` 2/2 位于
  `tmp/device-runs/20260820-161630-next334-contenteditable-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-301/999` 39/39 位于
  `tmp/device-runs/20260820-161711-next334-contenteditable-reflection-recent/`；tracked
  `test_host/test_host.ini` 已恢复默认 `javascript=0`。
- next335 自动候选证据：`TEST302/999` 2/2 位于
  `tmp/device-runs/20260820-162725-next335-draggable-reflection/`；零 ERROR/FAIL、唯一
...
- next336 自动候选证据：`TEST303/999` 2/2 位于
  ...
  已恢复默认 `javascript=0`。
- next337 自动候选证据：`TEST304/999` 2/2 位于
  `tmp/device-runs/20260820-164631-next337-input-accept-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-304/999` 42/42 位于
  `tmp/device-runs/20260820-164656-next337-input-accept-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next338 自动候选证据：`TEST305/999` 2/2 位于
  `tmp/device-runs/20260820-165204-next338-input-capture-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-305/999` 43/43 位于
  `tmp/device-runs/20260820-165256-next338-input-capture-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next339 自动候选证据：`TEST306/999` 2/2 位于
  `tmp/device-runs/20260820-165825-next339-input-dirname-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-306/999` 44/44 位于
  `tmp/device-runs/20260820-165845-next339-input-dirname-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next340 自动候选证据：`TEST307/999` 2/2 位于
  `tmp/device-runs/20260820-170256-next340-input-list-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。近期链首次在既有 TEST296 的 DOM bootstrap timeout
  处停止（`tmp/device-runs/20260820-170320-next340-input-list-reflection-recent/`），未作为
  基线；重试 `TEST264-307/999` 45/45 位于
  `tmp/device-runs/20260820-170415-next340-input-list-reflection-recent-retry/`，零
  ERROR/FAIL、唯一 TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini`
  已恢复默认 `javascript=0`。
- next341 自动候选证据：`TEST308/999` 2/2 位于
  `tmp/device-runs/20260820-170809-next341-textarea-wrap-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-308/999` 46/46 位于
  `tmp/device-runs/20260820-170830-next341-textarea-wrap-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next342 的失败候选证据位于
  `tmp/device-runs/20260820-193424-next342-class-name-reflection/`，bootstrap 明确报告
  `TypeError: not configurable`，未作为基线；随后 htmlFor 候选 `TEST309/999` 2/2 位于
  `tmp/device-runs/20260820-193609-next342-html-for-reflection-retry/`，零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-309/999` 47/47 位于
  `tmp/device-runs/20260820-193636-next342-html-for-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next343 自动候选证据：`TEST310/999` 2/2 位于
  `tmp/device-runs/20260820-194058-next343-slot-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-310/999` 48/48 位于
  `tmp/device-runs/20260820-194117-next343-slot-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next344 自动候选证据：`TEST311/999` 2/2 位于
  `tmp/device-runs/20260820-194457-next344-itemid-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-311/999` 49/49 位于
  `tmp/device-runs/20260820-194515-next344-itemid-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next345 自动候选证据：`TEST312/999` 2/2 位于
  `tmp/device-runs/20260820-194831-next345-itemprop-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-312/999` 50/50 位于
  `tmp/device-runs/20260820-194850-next345-itemprop-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next346 自动候选证据：`TEST313/999` 2/2 位于
  `tmp/device-runs/20260820-195216-next346-itemref-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-313/999` 51/51 位于
  `tmp/device-runs/20260820-195233-next346-itemref-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next347 自动候选证据：`TEST314/999` 2/2 位于
  `tmp/device-runs/20260820-200253-next347-itemscope-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-314/999` 52/52 位于
  `tmp/device-runs/20260820-200311-next347-itemscope-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next348 自动候选证据：`TEST315/999` 2/2 位于
  `tmp/device-runs/20260820-200637-next348-itemtype-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-315/999` 53/53 位于
  `tmp/device-runs/20260820-200656-next348-itemtype-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next349 自动候选证据：`TEST316/999` 2/2 位于
  `tmp/device-runs/20260820-201118-next349-nonce-reflection-retry/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-316/999` 54/54 位于
  `tmp/device-runs/20260820-201314-next349-nonce-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。`tmp/device-runs/20260820-201025-next349-nonce-reflection/`
  是已撤回的 `<script>` 夹具失败，不作为基线；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next350 自动候选证据：`TEST317/999` 2/2 位于
  `tmp/device-runs/20260820-202104-next350-part-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-317/999` 55/55 位于
  `tmp/device-runs/20260820-202120-next350-part-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next351 自动候选证据：`TEST318/999` 2/2 位于
  `tmp/device-runs/20260820-202451-next351-exportparts-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-318/999` 56/56 位于
  `tmp/device-runs/20260820-202506-next351-exportparts-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next352 自动候选证据：`TEST319/999` 2/2 位于
  `tmp/device-runs/20260820-202802-next352-inert-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-319/999` 57/57 位于
  `tmp/device-runs/20260820-202819-next352-inert-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next353 自动候选证据：`TEST320/999` 2/2 位于
  `tmp/device-runs/20260820-203134-next353-popover-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-320/999` 58/58 位于
  `tmp/device-runs/20260820-203148-next353-popover-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next354 自动候选证据：`TEST321/999` 2/2 位于
  `tmp/device-runs/20260820-203450-next354-autofocus-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。首尝近期门在
  `tmp/device-runs/20260820-203506-next354-autofocus-reflection-recent/` 的既有 TEST266
  bootstrap timeout 处停止，不作为基线；原选择重跑后的 `TEST264-321/999` 59/59 位于
  `tmp/device-runs/20260820-203544-next354-autofocus-reflection-recent-retry/`；零
  ERROR/FAIL、唯一 TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini`
  已恢复默认 `javascript=0`。
- next355 自动候选证据：`TEST322/999` 2/2 位于
  `tmp/device-runs/20260820-203851-next355-enterkeyhint-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-322/999` 60/60 位于
  `tmp/device-runs/20260820-203905-next355-enterkeyhint-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next356 自动候选证据：`TEST323/999` 2/2 位于
  `tmp/device-runs/20260820-204215-next356-virtualkeyboardpolicy-reflection/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-323/999` 61/61 位于
  `tmp/device-runs/20260820-204230-next356-virtualkeyboardpolicy-reflection-recent/`；零
  ERROR/FAIL、唯一 TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini`
  已恢复默认 `javascript=0`。
- next357 自动候选证据：`TEST324/999` 2/2 位于
  `tmp/device-runs/20260820-204543-next357-webkitdirectory-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-324/999` 62/62 位于
  `tmp/device-runs/20260820-204558-next357-webkitdirectory-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next358 自动候选证据：`TEST325/999` 2/2 位于
  `tmp/device-runs/20260820-204956-next358-input-size-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-325/999` 63/63 位于
  `tmp/device-runs/20260820-205011-next358-input-size-reflection-recent/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next359 自动候选证据：`TEST326/999` 2/2 位于
  `tmp/device-runs/20260820-205355-next359-textarea-cols-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-326/999` 64/64 位于
  `tmp/device-runs/20260820-205412-next359-textarea-cols-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next360 自动候选证据：`TEST327/999` 2/2 位于
  `tmp/device-runs/20260820-205904-next360-textarea-rows-reflection/`；零 ERROR/FAIL、唯一
  TESTBENCH PASS，`test13_route_ok=True`。最近 `TEST264-327/999` 65/65 位于
  `tmp/device-runs/20260820-205924-next360-textarea-rows-reflection-recent/`；零 ERROR/FAIL、
  唯一 TESTBENCH PASS，`test13_route_ok=True`；tracked `test_host/test_host.ini` 已恢复默认
  `javascript=0`。
- next361 自动候选证据：`TEST328/999` 2/2 位于
  `tmp/device-runs/20260820-210056-next361-open-reflection/`；零 ERROR/FAIL、唯一 TESTBENCH
  PASS，`test13_route_ok=True`。最近 `TEST264-328/999` 66/66 位于
