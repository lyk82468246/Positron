...
更新时间：2026-08-08
当前分支：`main`  
最新设备基线：next146 已在 `screen=240x320 dpi=96` 下完成默认 testbench，
TEST13/20/27/43/44/56/58-77/80-112 全部通过并记录 `TESTBENCH PASS`。该批在默认关闭的
...
**当前构建候选（next147）**：显式 `javascript=1` 页面新增最小
...
可信 click、取消、重新布局和 listener 清理；C89 与 ARMV4I 构建已通过，设备验收待进行。
默认 `javascript=0` 与 TEST13 路径不变，键盘/焦点/输入事件仍未实现。
...
- 快速配置：`test_host.exe` 同目录的 `test_host.ini` 使用 `tests=13,20,27,43,44,56,58-77,80-113`；next146 已在 `screen=240x320 dpi=96` 下完成整批设备验收，next147 的 TEST113 待设备验收。`javascript=0` 是默认产品门，只有显式改为 `1` 才执行初次加载的 classic inline/external scripts，并保留页面 context 与最小 event listener；未成功抓取或不支持类型的 external 会跳过。TEST79/float 候选已撤回。自动日志会在开头写入 screen/DPI；若 TEST20 的 48 CSS px 被换算成异常物理尺寸，先记录设备指标，不要放宽断言。也支持 `tests=1-5 7b` 一类语法。`auto=1` 时不弹 Yes/No/OK，窗口首帧后自动关闭，TEST13 自动跑 example.com → IANA Example Domains → Reserved Domains，并把每个原始结果和逐页遥测覆盖写入同目录 `test_host.log`；`auto=0` 保留 Yes/No 与原四组路由。自动首帧冒烟不替代新视觉能力的人工截图验收。缺失/无效配置不会静默改变测试范围，TEST23/78/79 不可选。
