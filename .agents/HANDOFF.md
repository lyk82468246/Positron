...
更新时间：2026-07-30
当前分支：`main`  
当前设备基线：next99，配置的 TEST13/20/27/43/44/56/58-68 已由无人值守设备日志确认全部 PASS。该版本加入 NetSurf button gadget、successful-controls 编码与 WM GET/URL-encoded POST 导航；TEST13 的 example.com、IANA help 和 reserved 深链也完整通过。multipart/file、reset 恢复、Enter 隐式提交、label 激活和完整事件系统尚未实现。**next78 仍是已撤回的失败实验，不得使用**。
...
- 快速配置：`test_host.exe` 同目录的 `test_host.ini` 当前使用 `tests=13,20,27,43,44,56,58-66`；也支持 `tests=1-5 7b` 一类语法。`auto=1` 时不弹 Yes/No/OK，窗口首帧后自动关闭，TEST13 自动跑 example.com → IANA Example Domains → Reserved Domains，并把每个原始结果和逐页遥测覆盖写入同目录 `test_host.log`；`auto=0` 保留 Yes/No 与原四组路由。自动首帧冒烟不替代新视觉能力的人工截图验收。缺失/无效配置不会静默改变测试范围，TEST23 不可选。
...
- GDI Render：TEST 12、14、17、19、20、26-37、39、46-58、62-68，离线窗口渲染、WM Imaging 位图、SVG path/cache/fallback/fill-rule、CSS background-image、原生 GDI text、线性/径向渐变、同文档及重叠文档缓存复用、table/list、HTML inline author CSS、checkbox/radio、WM 原生输入/select、button 与基础提交；TEST65-68 已验收。
...
> 2026-07-30 优先级调整：先解决“有没有”，再解决已有小范围“好不好”。checkbox/radio、单行 text/password、textarea、单选 select、button 与基础 GET/URL-encoded POST 已推进到 next99 设备自动化基线；之后再补 multipart/reset/事件状态和重大布局缺口。已有 NetSurf Duktape backend 的 JavaScript 最小纵切提前到中期。首屏 SVG 冷解析、抗锯齿、渐变高级参数、视觉微调和全面性能优化后置，除非它们造成崩溃、数据错误或阻塞基本操作。
