...
更新时间：2026-07-10
当前分支：`main`  
当前最新提交：请以 `git log --oneline -5` 为准；Codex 接手后已刷新文档，接入 M5f border、CSS selector 补强、`<img>` fallback/fetch 与文档级图片字节缓存，并把 TEST 11 扩展为 margin-collapse 正反样例。
...
- TEST 11 原有 `body.y=8` / `p.y=24` 是旧手写布局器预期；NetSurf 折叠结果为 `body.y=p.y=16`。当前源码新增 `padding-top:1px` 阻断组，必须同时得到 `(16,16)` 与 `(8,25)` 才通过；待 WM6 复编确认。
...
1. ENGINE 回归：TEST 11 应显示 collapse `body/p y=16`、padding barrier `body y=8, p y=25`；新版 TEST 18 应显示 first/second 都为 `2/2` 且 fetch calls 为 2。
2. 图片/SVG：缓存去重复编通过后，把缓存字节交给 WM Imaging API 解码，再接 NetSurf bitmap / `plot_bitmap`；当前 SVG logo/PNG/JPEG 仍不会真实显示。
