...
更新时间：2026-08-08
当前分支：`main`  
当前候选：next156 已完成 C89、仓库审计、VS2008 ARMV4I Debug 增量构建和 staging，设备
验收待运行；修正 next155 事件回调 JSON 过滤器误删合法 UTF-8 的问题，新增单个 BMP
`WM_CHAR` 的 UTF-8 `keypress`/EDIT `beforeinput.data` 桥，TEST121 待设备确认。代理对、
IME/composition 未实现，默认 `javascript=0` 与 TEST13 路径不变。
...
next156 修复，不能把 next155 失败包写成设备基线。
...
C89、仓库审计、ARMV4I 增量构建和 `C:\WMShare\Positron-next156` staging 已通过，待
轮换分辨率/DPI 的设备验收；代理对、IME/composition 和完整 Unicode 输入不在范围内。
