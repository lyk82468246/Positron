...
2. 图片/SVG：TEST 19 已确认 2x2 BMP（red/green + blue/yellow）可由 WM Imaging `IImage::Draw` 绘制；下一步把缓存字节接回 NetSurf bitmap / `plot_bitmap`。当前 SVG logo/PNG/JPEG 仍不会通过 `<img>` 真实显示。
