...
7. 图片/SVG：TEST20 四格式缓存 `<img>`、TEST25-37 的 SVG 正式链，以及 TEST13 网络相对 SVG 链均已真机确认。公共 `positron_image.dll` 已覆盖 retained WM 位图/SVG、旧 core 转发、PNG/JPEG 编码与静态 libjpeg-turbo 4:4:4；next49 确认 JPEG 颜色、行方向与采样正确。next50 截图确认 ABI 1.3 的 padded BGR24、straight-alpha BGRA32、RGB/alpha PNG、JPEG 与 SVG 六项视觉正确，但示例标题栏 X 采用 WM 智能最小化并残留后台。ABI 1.4 现增加 WM Imaging BMP/GIF 编码启动自检，并显式处理 `WM_CLOSE` 真退出；next51 验收时既要看到原六项，也要在点 X 后确认进程消失。按用户要求不增加左右软键。复杂文本、径向焦点、spread method、background-size 和多层背景仍是显式缺口。
