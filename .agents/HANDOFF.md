...
更新时间：2026-08-08
当前分支：`main`  
最新设备基线：next151 已在 `screen=320x320 dpi=128` 下完成默认 testbench，
TEST13/20/27/43/44/56/58-77/80-117 全部通过并记录 `TESTBENCH PASS`。该批在默认关闭的
...
当前交接基线：next151 已完成统一 script sequence ABI、external resource worker round、
...
的网络路径；next151 的 beforeinput 桥和 TEST117 也已通过设备验收。

**当前开发候选（next152，设备待验收）**：原生 `COMBOBOX/LISTBOX` 的 WM 键盘消息已
加入宿主侧子类桥，TEST118 同时覆盖公开 SELECT 键盘事件 ABI 与真实 `WM_KEYDOWN/UP`
入口。C89、仓库审计和 VS2008 ARMV4I 增量构建已通过，但尚未 staging/设备验收；在
收到 next152 日志前，next151 仍是可交接设备基线。默认 `javascript=0` 和 TEST13
网络路径不变。
...
**next152 本地候选（2026-08-08，设备待验收）**：`pcore_native_select` 保存原始窗口
过程并对原生 `COMBOBOX/LISTBOX` 子类化，先派发 `keydown/keyup`，未取消时调用原生
默认过程。TEST118 会在自动化窗口创建阶段发送 ArrowDown 的 WM 消息，并在窗口关闭
后核对 target/bubble 事件记录；这只验证 SELECT 键盘纵切，不扩展到 IME、`keypress`
...
- 快速配置：`test_host.exe` 同目录的 `test_host.ini` 使用 `tests=13,20,27,43,44,56,58-77,80-118`；next151 已在 `screen=320x320 dpi=128` 下完成整批设备验收，TEST117 已通过，TEST118 是当前设备待验收候选。`javascript=0` 是默认产品门，只有显式改为 `1` 才执行初次加载的 classic inline/external scripts，并保留页面 context、click listener、原生表单事件、next149 的 EDIT 键盘 bridge、next150 的 focusin/focusout bridge、next151 的 beforeinput bridge 和 next152 的 SELECT 键盘候选；未成功抓取或不支持类型的 external 会跳过。TEST79/float 候选已撤回。自动日志会在开头写入 screen/DPI；若 TEST20 的 48 CSS px 被换算成异常物理尺寸，先记录设备指标，不要放宽断言。也支持 `tests=1-5 7b` 一类语法。`auto=1` 时不弹 Yes/No/OK，窗口首帧后自动关闭，TEST13 自动跑 example.com → IANA Example Domains → Reserved Domains，并把每个原始结果和逐页遥测覆盖写入同目录 `test_host.log`；`auto=0` 保留 Yes/No 与原四组路由。自动首帧冒烟不替代新视觉能力的人工截图验收。缺失/无效配置不会静默改变测试范围，TEST23/78/79 不可选。
