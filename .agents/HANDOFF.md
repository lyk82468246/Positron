...
- 分支：`main`。next672–693 的过时导航、资源终态/重试、提交门、摘要观测、资源事务所有权、候选生命周期所有权、candidate/resource 提交组合、提交后清理观测、Browser-owned history viewport snapshot、page-level 横向 viewport、脚本滚动视口桥接、布局几何桥接、viewport resize 通知、动态 `matchMedia()` 更新、布局视口对应的 `visualViewport`、稳定的 `screen.orientation` 方向事件、`history.scrollRestoration` 宿主策略门、页面替换前的 cancelable `beforeunload` 门、统一脚本任务检查点以及初始/可见性 `pageshow` 生命周期组合均已实现；next694 又补齐了宿主驱动的顶层窗口 focus/blur 状态、`document.hasFocus()` 和去重事件分发。
...
- Browser script session 还提供由宿主驱动的顶层窗口焦点合同：`PBrowser_ScriptSessionDispatchWindowFocus` 归一化激活值，更新 `document.hasFocus()`，并在实际变化时按属性 handler、listener 顺序派发可信的非冒泡 focus/blur；重复值保持静默。参考宿主已把 `WM_ACTIVATE` 接到该入口，但 native 控件焦点、焦点矩形和 OEM/跨窗口策略仍由宿主负责。
...
- TEST1133 覆盖布局视口对应的 `visualViewport` 快照、page scroll 同步、visual/window resize/scroll 顺序、事件字段、监听器移除和重复通知去重。
- TEST1134 覆盖 `history.scrollRestoration` 的 Browser→宿主策略门：`manual` 保留当前 viewport，`auto` 恢复并 clamp Browser entry snapshot，非法参数不改变调用方输出。
- TEST1135 覆盖稳定 `screen.orientation` 对象、方向翻转的可信 `change` 事件、同方向/重复 resize 去重、监听器移除和非法参数保护。
- TEST1136 覆盖 `beforeunload` 的事件字段与调用顺序、`preventDefault()`、非空 `returnValue`、handler 返回字符串、监听器移除和空参数 fail-closed 保护；参考宿主没有确认对话框。
- TEST1137 覆盖统一脚本任务检查点的阶段选择和顺序、每阶段后的 microtask、frame timestamp、idle timeout、message limit、零 mask 和未知 mask 拒绝；参考宿主在 UI `WM_TIMER` 中调用同一 Browser API。
- TEST1138 覆盖初始 `complete` 后唯一的 `pageshow`、重复 complete 去重、hidden→visible 的 `visibilitychange`/`pagehide`/`pageshow` 顺序、重复可见性值静默和 `persisted == false`。
- TEST1139 覆盖 `document.hasFocus()`、window focus/blur 的状态变化、事件字段、属性 handler、listener、非零归一化和重复通知去重；参考宿主的 `WM_ACTIVATE` 接线与离线 Browser 契约共同验证。

### 当前测试入口

- `TEST_MAX_NUMBER`：1139。
...
当前最新产品门为 next694 的顶层窗口焦点生命周期组合：
...
上一条产品门为 next693 的初始/可见性页面生命周期组合：

- next693 定向目录：`tmp/device-runs/20260831-165553-next693-pageshow-adjacent-r4/`；
- 动态选择：`1135,1136,1137,1138,999`，5 项；5/5 通过，零 `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`。TEST1135–1137 保留 screen.orientation、beforeunload 和统一任务检查点相邻回归；TEST1138 验证初始 pageshow、重复 complete 去重、hidden→visible 的 visibilitychange/pagehide/pageshow 顺序、重复状态静默和 `persisted == false`；TEST999 提示音请求一次。
- 设备：640x480，dpi=192；使用当前 WMDC GUI 会话、正式 Debug ARMV4I 构建和同批 staging；RAPI gate 只复用已有 GUI 会话，不连接、选择、重置或杀死设备。
- 静态验证：`python scripts/test_c89ize.py`、正式 Debug ARMV4I 构建、设备门和 `python scripts/audit_repo.py` 均通过；r4 日志中的选择、完成数、错误数和唯一 `TESTBENCH PASS` 已复核。

上一条产品门为 next692 的统一脚本任务检查点与参考宿主消息循环接线：

- next692 定向目录：`tmp/device-runs/20260831-162111-next692-task-checkpoint-adjacent-r7/`；
- 动态选择：`1135,1136,1137,999`，4 项；4/4 通过，零 `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`。TEST1135–1136 保留 screen.orientation 与 beforeunload 相邻回归；TEST1137 验证 Browser 统一 checkpoint 的阶段选择、timer/frame/message/idle 顺序、每阶段 microtask、frame timestamp、idle timeout、message limit 和非法 mask；TEST999 提示音请求一次。
- 设备：640x480，dpi=192；使用当前 WMDC GUI 会话、正式 Debug Rebuild 后的 VS2008 ARMV4I 产物和同批 staging。隔离重试 `tmp/device-runs/20260831-155924-next692-task-checkpoint-isolated-r2/` 也以 2/2 通过；此前 bootstrap 瞬态失败仅保留为排障背景，不作为产品基线。
- 静态验证：`python scripts/test_c89ize.py`、正式 Debug Rebuild、设备门和 `python scripts/audit_repo.py` 均通过；r7 日志中的选择、完成数、错误数和唯一 `TESTBENCH PASS` 已复核。

上一条产品门为 next691 的页面替换前 `beforeunload` 取消门；最近一次稳定全量 checkpoint 仍为 next670：

- next691 定向目录：`tmp/device-runs/20260831-153449-next691-beforeunload-adjacent-r15/`；
- 动态选择：`1135,1136,999`，3 项；3/3 通过，零 `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`。TEST1135 保留 screen.orientation 相邻回归；TEST1136 验证事件字段、handler/listener 顺序、`preventDefault()`、非空 `returnValue`、handler 返回字符串、监听器移除和参数 fail-closed；TEST999 提示音请求一次。
- 设备：640x480，dpi=192；使用当前 WMDC GUI 会话、完整 Rebuild 后的 VS2008 ARMV4I Debug 构建和同批 staging。`beforeunload` 脚本在按需安装路径中运行通过，日志没有 prompt 或导航副作用。
- 静态验证：`python scripts/test_c89ize.py`、正式 Debug Rebuild 和设备门均通过；r15 设备日志中的选择、完成数、错误数和唯一 `TESTBENCH PASS` 已复核。此前相邻重跑的 bootstrap 瞬态超时没有写入产品基线。

前一条产品门为 next690 的 `screen.orientation` 方向事件与 viewport resize 相邻回归；最近一次稳定全量 checkpoint 仍为 next670：

- next690 定向目录：`tmp/device-runs/20260831-143153-next690-screen-orientation-r9/`；
- 动态选择：`1131,1132,1133,1134,1135,999`，6 项；6/6 通过，零 `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`。TEST1131–1134 保留 viewport、动态媒体查询、visualViewport 和 scroll restoration 相邻回归；TEST1135 验证稳定 orientation 对象、方向翻转 change 事件、媒体→orientation→visual/window 顺序、重复/同方向去重、监听器移除和非法 resize 保护；TEST999 提示音请求一次。
- 设备：640x480，dpi=192；使用当前 WMDC GUI 会话、完整 Rebuild 后的 VS2008 ARMV4I Debug 构建和同批 staging。早期重试中的 bootstrap 瞬态失败均未复现，最终 r9 日志完整通过。
- 静态验证：`python scripts/test_c89ize.py`、正式 Debug 构建、设备门和 `python scripts/audit_repo.py` 均通过；最终 r9 设备日志中的选择、完成数、错误数和唯一 `TESTBENCH PASS` 已复核。

上一条产品门为 next689 的 `history.scrollRestoration` 与 viewport snapshot restore 相邻回归；最近一次稳定全量 checkpoint 仍为 next670：

- next689 定向目录：`tmp/device-runs/20260831-135517-next689-scroll-restoration-r6/`；
- 动态选择：`1131,1132,1133,1134,999`，5 项；5/5 通过，零 `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`。TEST1131–1133 保留 viewport、动态媒体查询和 visualViewport 直接相邻回归；TEST1134 验证默认/显式 `history.scrollRestoration`、宿主自动 snapshot restore 的跳过/恢复、非法参数保护；TEST999 提示音请求一次。
- 设备：640x480，dpi=192；使用当前 WMDC GUI 会话、完整 Rebuild 后的 VS2008 ARMV4I Debug 构建和同批 staging。
...
上一条产品门为 next688 的布局视口 `visualViewport` 与 resize/scroll 相邻回归；最近一次稳定全量 checkpoint 仍为 next670：

- next688 定向目录：`tmp/device-runs/20260831-131517-next688-visual-viewport-r14/`；
- 动态选择：`1133,1132,1131,999`，4 项；4/4 通过，零 `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`。TEST1133 验证 `visualViewport` 快照、page scroll 同步、visual/window resize/scroll 顺序、事件字段、监听器移除和重复通知去重；TEST1132 验证动态媒体查询；TEST1131 验证 WM_SIZE 的 CSS viewport/DPR 与 window 事件；TEST999 提示音请求一次。
...
- TEST1117–TEST1139 都是离线自动夹具，没有新增必须立即人工复核的崩溃或数据风险；它们的 dialog、候选提交、资源 gate、cleanup、history、page viewport、单元素 geometry、resize metadata、动态媒体查询、visualViewport、screen.orientation、脚本任务检查点、页面可见性和窗口焦点生命周期的视觉、触摸、旋转与不同 DPI 呈现，继续与既有人工清单一起累计观察。自动结果不替代真实网络恢复、OEM 控件或逐资源视觉验收。
...
- 已建立固定、小型、可重复的离线 corpus 流程，但它们仍不能代表任意真实网站；TEST13 仍只是单一网络哨兵。TEST1119–TEST1139 已覆盖候选取消、Browser candidate admission/结果/终态、资源事务终态、有界 transport 重试、required/optional 提交门、candidate/resource 组合决策、清理快照、page-level viewport、脚本/宿主滚动同步、有限 `getBoundingClientRect()` 几何、viewport resize metadata、动态 `matchMedia()`、布局视口对应的 `visualViewport`、稳定的 `screen.orientation` 方向事件、页面替换前的 `beforeunload` 取消门、统一脚本任务检查点、初始 pageshow/可见性生命周期和宿主驱动的窗口焦点生命周期，但取消仍是协作式的，不能保证正在阻塞的 PHttp 调用立即返回；脚本队列仍依赖宿主主动调度，宿主尚未提供面向用户的逐资源 UI，也不能保证任意真实站点的 fallback 视觉、复杂布局几何、pinch zoom、精确逐元素归因或自定义 prompt 体验。
...
next694 已补齐宿主驱动的顶层窗口 focus/blur、`document.hasFocus()` 和重复状态去重，并由 TEST1139 与 r1 设备门验证。现在先从源码、日志或截图固定一个新的真实缺口，再选择离线 fixture 或稳定哨兵。实现必须把可复用语义放在正确的公共 DLL，宿主只做平台接线、调度、应用策略和断言；不要把互不相关的能力拆成只增加编号的提交，也不要在没有证据时扩大 ABI。现有 viewport、几何、媒体查询、visualViewport、稳定的 screen.orientation、scroll restoration、beforeunload 取消门、统一脚本任务检查点、初始/可见性 pageshow 生命周期和窗口焦点生命周期已有自动与设备证据；复杂布局、nested overflow、pinch zoom、transforms、平滑/惯性滚动、完整媒体查询语法、bfcache 和视觉差异仍是限制，不应在下一步中被误写成已支持。
...
- 自动断言覆盖该纵向能力的成功、失败/取消、资源清理和直接相邻旧路径，且不会削弱 next685–693 的布局 relation、`getBoundingClientRect()`、DPI 换算、history snapshot、宿主 clamp/apply、scroll restoration、beforeunload、脚本任务检查点或旧页保留契约；
...
- handoff 覆盖为 next694 快照，ROADMAP 只保留当前尚未完成的纵向能力。
