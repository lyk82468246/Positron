...
最新 next805 设备证据见“最新有效设备证据”；更早传输失败只保留在 Git 历史和
...
- 设备门复用 WMDC GUI 当前唯一 RAPI 会话；超时进程需在设备端正常结束。
  `tmp/` 中的本地证据未纳入版本控制。
- `TEST_MAX_NUMBER` 已为 1246。tracked `test_host/test_host.ini` 仍是窄 smoke：
  `auto=1`、`javascript=0`、选择 `13,20,27,56,58,62,64-67,73,75,1217-1246,999`；nightly/device
...
- DOM 结构 mutation 已覆盖 direct-element removal、文本/Comment/CDATA、normalize、clone/
  ...
  和 detached `Comment` creation/data/lifecycle/insertion。
...
- 当前基线覆盖表单 owner/validation/submission/reset/FormData、selector、滚动/几何、
  生命周期、焦点、图片资源和有界 CharacterData/DOM mutation；最近 next784–next804
  ...
  `insertAdjacentHTML`、单根 `outerHTML` replacement/removal、text-only
  `DocumentFragment` staging、Ex13 文本/fragment、Ex14 同父 mixed、Ex15 node-aware
  `Element.replaceChildren()`、next803 detached `Text` creation/insertion、next804 detached
  Element creation、属性/Text staging、Core 物化与 identity/lifecycle，以及 next805 detached
  Comment creation/data/lifecycle/insertion。
  稳定合同和逐测试
...
- `TEST_MAX_NUMBER`：1246。
- tracked `test_host/test_host.ini`：`auto=1`、`javascript=0`，选择 `13,20,27,56,58,62,64-67,73,75,1217-1246,999`。
- tracked INI 是窄 smoke，不是全量目录；nightly 打包脚本从源码 dispatch 动态生成全量自动清单。
- 设备连接必须先由用户在 WMDC/Device Emulator GUI 手动完成；RAPI gate 只使用当前唯一会话。

## 最新有效设备证据

`tmp/device-runs/20260914-215045-next805` 是当前基线：Debug ARMV4I，选择
`1244,1245,1246,999`，4/4 PASS，零 ERROR/FAIL；日志完整，双空间预检、完成后清理、
...
基于 compatibility corpus、源码或用户页面选择 next806 的一个产品缺口；next805 的 detached
`document.createComment()` 路径和 TEST1246 已完成并通过设备门。新批次仍须把可复用
