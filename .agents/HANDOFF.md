...
- `TEST_MAX_NUMBER` 已为 1191。tracked `test_host/test_host.ini` 仍是窄 smoke：
...
- Browser/Core 的表单 owner、validation、submission、dialog、reset、`requestSubmit`、
  ...
  合同，构造成功后还会同步派发 `formdata`；TEST1170–1191 是当前相邻夹具，其中
  ...
  form-associated owner/collection 关系与无效 owner 回退。
...
- 近期 Browser/Core 已形成 focus/autofocus、selector、validation、form-owner、reset、
  submission、FormData、fieldset/object/output collection 和生命周期组合；各能力边界与测试编号
...
- next737–740 已把宿主批准的 `:visited`、有界 `:scope`/`:default` selector 和
  ...
  TEST1179–1182 及相邻设备门均通过，Browser heap 上限仍为 730 KiB，慢设备的页面
...
- next749 在 Core/Browser 公共关系中接入 `<object>` 的祖先/显式 `form="id"` owner，按
  ...
  与 TEST999 已通过最新设备门。
- 当前唯一下一步是 next750：从 compatibility corpus、源码、设备日志或截图固定新的
...
- Browser 层提供有界 history、same-document state、script session、DOM/Event/input/navigation callbacks，以及 timer/microtask/lifecycle、native 控件事务、导航资源事务、候选生命周期/结果协调和 FormData snapshot bridge（含 Ex submitter 与 formdata 事件路径）。`select.options`、`selectedOptions`、`length`、`option.index`、`option.form`、fieldset 的 type/form/elements、object 的 form 以及 output 的 form/labels、`form.elements` 中的 fieldset/object/output enumeration 也在 Browser 中以有界、可寻址元素 bridge 提供。
...
- Core 的 form owner relation 对支持的 input、select、textarea、button、fieldset、object、output 解析
  最近祖先或显式 `form="id"` 目标；Browser 的 `Element.form` 与 `form.elements` 复用这条
  规则，后者从整棵文档按顺序返回跨树 form-associated 元素的有界 snapshot，空值/无效
  目标不回退祖先。fieldset/object/output 只进入 relation collection（以及 output 的 labels），
...
最新设备门证据为 next749 的 object form-association/collection；双空间预检/空间回收窄门
仍是部署安全基线：

- `tmp/device-runs/20260904-131641-next749/`；选择 `1191,999`，2/2 通过，零
  `ERROR`/`FAIL`，唯一 `TESTBENCH PASS`，`complete_log_retrieved=True`。Storage Card
  目标卷 `CeGetDiskFreeSpaceEx` 可用 65,673,953,280 字节，总计 511,101,108,224，
  payload 9,756,676、reserve 1,048,576、required 10,805,252；内部 object store
  `CeGetStoreInformation` 可用 8,751,104/32,942,080，cache reserve 65,536，目标与
...
- 静态验证：`python scripts/test_c89ize.py`、正式 Debug ARMV4I build、同批 staging、
  ...
  730 KiB，`PSCRIPT_MAX_NATIVE_FUNCTIONS` 为 28。TEST1191 日志确认 object owner/collection
  由公共 DLL 提供，且不进入 validation、successful-control 或 FormData。
...
Core/Browser form owner 目前覆盖 input、select、textarea、button、fieldset、object 和 output；这些元素
（包括显式 `form="id"` 的跨树元素）按文档顺序加入有界 `form.elements` snapshot。fieldset、
object 和 output 只属于 DOM relation enumeration，仍不进入 successful-control visitor、submission 或
FormData；output 的 labels、descendant-text `value`、独立 default override、只读 `type` 和
reset 恢复已覆盖；`img` 等其他 form-associated 元素尚未覆盖。validation、
...
association/collection 均已完成；next750 仍需从 compatibility corpus、源码、日志或截图固定一个真实产品缺口，再选择一个边界清楚的离线 fixture 或稳定哨兵。实现必须把可复用语义放在正确的公共 DLL，宿主只做平台接线、调度、应用策略和断言；
...
- next749 完成后 handoff 应覆盖为 next749 快照，ROADMAP 只保留当前尚未完成的纵向能力；
  TEST1191 的 object owner/collection 边界和设备证据应可由本文件与 `docs/TESTING.md` 复核。
