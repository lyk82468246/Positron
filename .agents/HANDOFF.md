...
- 最新已验证产品基线：next221。
- next221 批次包含 `test_host/main.c`、`test_host/test_host.ini`，以及已验证的
  `scripts/device_gate.bat`、`scripts/device_gate.ps1` 自动设备门。
- 本地设备证据位于 `tmp/device-runs/20260813-113421-next221/`；`tmp/` 不跟踪，干净 clone
...
- 配置：`TEST13/20/27/43/44/56/58-77/80-188/999`，共 136 项。
...
- 通道：脚本从唯一运行的 Device Emulator 读取 VMID，精确匹配既有 CoreCon 目标；没有
  启动、选择、Cradle 或重置设备。
- 结果：136 个选中测试 ID 全部有 `OK`，TEST13 overview/box detail 完整；零 `ERROR`、
  零 `FAIL`、远端退出码 0、最终唯一 `TESTBENCH PASS`。
- TEST13：example.com、IANA Example Domains、Reserved Domains 三段导航均 `completed=1`。
- 能力终点：绝对和根相对 URL 中多个完整 `%2E%2E` double-dot segment 的受控顺序折叠与
  同文档 fragment 导航；混合编码、字面父目录和不同 query/path 仍为反例。
...
- 当前 ini 为 1312 字节，选择
  `TEST13/20/27/43/44/56/58-77/80-188/999`，共 136 项；
- 上述完整设备日志和 TEST13 三段真实导航。

## 唯一下一步

整理绝对与根相对 URL 的受支持/拒绝矩阵，选择一个有真实页面价值且可独立验收的上游
同文档导航语义；不要顺便扩展其他 URL 拼写、DOM、布局或性能。

完成标准：

- 先形成绝对/根相对、完整/半编码、字面父目录、不同 query/path、origin 根边界矩阵；
- 选定语义有正例、反例、无 GET、state/length、hashchange 和历史行为断言；
