...
- contenteditable 的 OEM 硬键盘/自动重复、SIP/IME 候选词、滚动/旋转和不同 DPI 下的文本视觉仍属于可累计人工风险；1113 已在真实 WM EDIT 上验证无修饰鼠标拖选的连续范围/方向通知，1114 验证了 Shift/方向键、捕获丢失和焦点切换的有界通知收尾，1112 覆盖脚本 `selectionchange` 去重。
...
以已验证的离线 `contenteditable` 语料为基础，next667 唯一推进单元素编辑的下一个高价值缺口：评估受限 clipboard/cut/paste 事务与选区同步，仍不引入 Range/Selection 对象、富文本、designMode、跨文档 modal 或无关 CSS 扩张。next666 已把原生 EDIT 的 Shift/方向键扩展、捕获丢失和焦点切换收尾接到宿主；next665 的无修饰鼠标拖选和 next664 的 programmatic/native 去重 `selectionchange` 仍由 Browser 公共 DLL 统一拥有；WM/native EDIT 代理、`beforeinput` 取消和 Core 纯文本 mutation 均已有设备证据。
...
- next666 的 Browser 选区与 `selectionchange` 语义仍位于公共 DLL；`test_host` 只提供原生 HWND 的键盘/拖选 anchor、捕获与焦点收尾、同步 callback、UTF-16/CRLF 平台转换和 fixture。下一批不得把事件策略或 Range/Selection 模型复制回 `test_host`；
