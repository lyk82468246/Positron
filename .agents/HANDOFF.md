...
最新 next780 设备证据及此前验证见“最新有效设备证据”；更早传输失败只
保留在 Git 历史和 `docs/history/`，不作为通过依据。

- 当前代码 next780 在 mutation Ex6 的 existing-element `append()`/`prepend()`/`before()`/
  `after()` 之后补齐单值 primitive 相对文本插入：Browser 复用既有 write Ex6
  `insert_text_child`，在目标 direct-parent 的未过滤 `childNodes` 位置创建 Text；Core
  继续拥有结构语义。TEST1220/1221 覆盖 element 与字符串化 primitive 两条路径，保留
  wrapper/snapshot，错误 child、对象/节点、detached、越界和多参数均 fail closed。
- 设备门每次远端启动使用唯一 executable basename，复用 WMDC GUI 当前唯一 RAPI 会话；
  超时进程需在设备端正常结束。`tmp/` 中的本地证据未纳入版本控制。
- `TEST_MAX_NUMBER` 已为 1221。tracked `test_host/test_host.ini` 仍是窄 smoke：
  `auto=1`、`javascript=0`、选择 `13,20,27,56,58,62,64-67,73,75,1217-1221,999`；nightly/device
...
- Browser/Core 的表单 owner、validation、submission、dialog、reset、`requestSubmit`、direct
  ...
  source-selection 生命周期已形成有界合同；TEST1170–1221 的逐项夹具、边界和错误回退
  统一见 [`docs/TESTING.md`](../docs/TESTING.md)。最近的 TEST1199–1221 还覆盖 source
  ...
  语义在 Core/Browser 而非宿主。TEST1209–1221 还覆盖 Ex4/Ex5/Ex6/Ex2/Ex3 callback、
  ...
  `before()`/`after()` 相对与 primitive 文本插入。
...
- 当前 fixed-buffer 数值转换、原子部署和 RAPI 日志恢复已由 `formal10` 正式门验证；`next766`
  ...
  `TEST1218,999` 门、next778 的 `TEST1219,999` 门、next779 的 `TEST1220,999` 门和
  next780 的 `TEST1221,999` 门也已通过；不得把
  测试宿主扩展当作产品语义实现。
...
- `TEST_MAX_NUMBER`：1221。
- tracked `test_host/test_host.ini`：`auto=1`、`javascript=0`，选择 `13,20,27,56,58,62,64-67,73,75,1217-1221,999`。
- tracked INI 是窄 smoke，不是全量目录；nightly 打包脚本从源码 dispatch 动态生成全量自动清单。
- 设备连接必须先由用户在 WMDC/Device Emulator GUI 手动完成；RAPI gate 只使用当前唯一会话。

## 最新有效设备证据

最新自动证据是 `20260911-014449-next780-text-final2`：正式 Debug ARMV4I，选择 `1221,999`，
2/2、唯一 PASS、零 ERROR/FAIL，完整日志已回收，目标卷和内部 object-store 双空间
预检通过，部署在日志回收后已清理；运行前后转储清单均为 0，
`crash_check=PASS`、`new_crash_dump_count=0`。TEST1221 证明 `before()`/`after()` 在混合
Text/element 子节点中插入一个字符串化 primitive，保持目标 wrapper、旧 snapshot 和父级文本，
并对对象、Text、detached clone、多参数和零参数边界 fail closed。此前 `20260911-004625-next779`
的 `1220,999` 门证明 existing-element 相对重排与跨父迁移。
...
- TEST1219–1221 是离线的 Core/Browser existing-element Ex6 夹具，覆盖
  `append()`/`prepend()` 的末尾/零位插入、`before()`/`after()` 的同父重排/跨父迁移，以及
  relative text 的字符串化 primitive；自动门已验证 mixed Text/element 位置、wrapper identity、
  旧 snapshot、父级 textContent、同节点 no-op 与 detached、对象/节点、越界、多参数的
  fail-closed 边界。该路径暂无新增立即人工风险；DocumentFragment、Comment/CDATA 插入、
  通用节点 mutation、完整 live collection 和 native/视觉行为仍未实现，逐项合同见
