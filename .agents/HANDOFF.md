...
- GDI Render：TEST 12、14、17、19、20、26-34，离线窗口渲染、WM Imaging 位图、SVG path/cache/fallback/fill-rule、CSS background-image、原生 SVG text、缓存 SVG gradient/text、线性渐变坐标与中心径向渐变；TEST26-33 已真机通过，TEST34 待设备。
...
5. TEST30-33 已于 2026-07-13 真机通过：CSS 背景定位/平铺/复用、libsvgtiny 文本元数据经 WM GDI 绘制及 path/text 顺序均成立；TEST32 原三角接缝已由结构化参数 -> NanoSVG 单路径连续填充消除；TEST33 截图确认线性渐变的两种坐标系和 gradientTransform。TEST34 已构建待设备，用于验证中心径向渐变；复杂 shaping、`textPath`、逐字定位、任意 shear、径向焦点/扩散/透明 stop 尚未实现。
...
7. 图片/SVG：TEST20 四格式缓存 `<img>`、TEST25-33 的 SVG parse/draw/cache/fallback/fill-rule/CSS background-image/basic text/连续线性渐变及坐标变换，以及 TEST13 网络相对 SVG 链均已真机确认。TEST34 中心径向渐变已完成 ARM 构建待设备；复杂文本、径向焦点/扩散/透明 stop、background-size、多层背景与异步资源事务仍是显式缺口。
