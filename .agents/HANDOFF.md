...
2. 图片/SVG：先跑 TEST 20，预期 96x72 有边框的 2x2 BMP（red/green + blue/yellow）显示且无 fallback text；之后将同一 fetch 流程接入 Browse host。当前 SVG logo/PNG/JPEG 仍不会通过 `<img>` 真实显示。
