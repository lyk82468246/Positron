...
最新 `next769` 设备证据、此前 `next768`/`next767`/`formal10` 证据及失败实验均在“最新有效设备证据”段落说明；更早的传输失败记录只保留在 Git 历史和 `docs/history/`，不作为产品通过依据。

- 当前代码 next769 延续 next761–768 的 Core child-data 与 Browser CharacterData bridge，
  保持 Ex3 `Text.splitText()`、关系 50 `wholeText`、Ex4 `replaceWholeText()` 和 Ex5
  `Node.normalize()` 的有界 ABI/失效合同；Browser 新增 `Node.cloneNode(deep)` detached
  snapshot。产品语义仍在 Core/Browser，`test_host` 只接 callback、fixture 和断言；错误
  child、结构边界、失效 wrapper 与超预算均 fail closed。
- 设备门每次远端启动使用唯一 executable basename，复用 WMDC GUI 当前唯一 RAPI 会话；
  超时进程需在设备端正常结束。`tmp/` 中的本地证据未纳入版本控制。
- `TEST_MAX_NUMBER` 已为 1211。tracked `test_host/test_host.ini` 仍是窄 smoke：
...
- Browser/Core 的表单 owner、validation、submission、dialog、reset、`requestSubmit`、direct
  ...
  source-selection 生命周期已形成有界合同；TEST1170–1211 的逐项夹具、边界和错误回退
  统一见 [`docs/TESTING.md`](../docs/TESTING.md)。最近的 TEST1199–1211 还覆盖 source
  ...
  `replaceWholeText()` 结构合同、`Node.normalize()` 递归/快照和 `Node.cloneNode()`
  detached snapshot 合同，均保持产品语义在 Core/Browser 而非宿主。TEST1209–1211 还
  覆盖 Ex4/Ex5 callback、相邻 Text 合并、目标/首个非空 wrapper 保持身份、克隆属性和
  独立数据。
...
- 当前基线已包含表单 owner/validation/submission/reset/FormData、selector、滚动/几何、
  ...
  next767 再补齐 `Text.replaceWholeText()` 的 Ex4/Core 有界相邻段合并；next768 再补齐
  `Node.normalize()` 的 Ex5/Core direct-child 整理、可寻址 wrapper 递归和新旧 snapshot
  reconciliation；next769 再补齐 Browser-owned `Node.cloneNode(deep)` 的有界 detached
  snapshot，覆盖属性、子树、parent links 和克隆独立性；
  稳定合同和逐测试说明以
...
- 当前 fixed-buffer 数值转换、原子部署和 RAPI 日志恢复已由 `formal10` 正式门验证；`next766`
  的 wholeText、`next767` 的 replaceWholeText、`next768` 的 Node.normalize 和 `next769`
  的 Node.cloneNode 纵切均已有正式设备门证据。下一步应从 compatibility corpus、源码、
  设备日志或截图固定一个新的真实产品缺口，选择 next770 的下一条公共 DLL 纵向能力。不得
  把测试宿主扩展当作产品语义实现。
...
- `textContent` 与非编辑元素的 `innerText` setter 通过 Browser 的既有 text callback
  ...
  wrapper 与旧 snapshot。宿主负责输入/事件策略和重新 style/layout/paint；通用节点插入、
  reparent、独立文本节点删除、MutationObserver 与完整 live collection 仍未实现。
...
最新正式证据是 `20260908-203254-next769-node-clone`：正式 Debug ARMV4I、
当前 GUI 连接的唯一 WMDC 目标、全新外部目录，定向运行 `TEST1210,1211,999`。
3/3 测试通过，只有一个 `TESTBENCH PASS`，零 `ERROR`/`FAIL`，完整日志已回收，
`current_cleanup=removed_after_complete_log`，空间预检和内部 object-store 检查均通过。
它确认 Node.normalize 回归与 `Node.cloneNode()` 的浅/深 detached snapshot、属性/子树、
parent links、独立数据、当前 32 KiB RAPI 写块、日志回收和完成后清理路径。
...
next769 已完成 Browser-owned `Node.cloneNode(deep)` 的有界 detached snapshot：浅/深
克隆复制 element 属性、子节点顺序、parent links 和独立数据，限制为 64 个 direct children
与 256 个节点，不改变 Core 文档、layout 或事件 listener。TEST1210–1211、TEST999 的
正式 Debug ARMV4I 设备门已通过，日志完整回收且远端目录已清理。

next770 的唯一下一步是先从 compatibility corpus、源码、设备日志或用户新页面固定一个
新的、可复现的产品缺口，再选择对应公共 DLL 的完整纵向能力。通用节点插入、reparent、
独立文本删除、Range/Selection、完整 live collection、MutationObserver、完整滚动容器
