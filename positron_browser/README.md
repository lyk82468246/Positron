...
document、DOM、navigation、event、input、keyboard、focus、EDIT change/post-change input、click、programmatic `HTMLElement.click()`、`HTMLElement.disabled`、`checkValidity()`、`willValidate`、`validity` 查询、`required`、`readOnly`、`multiple`、`noValidate`、`formNoValidate`、`min`/`max`/`step`、submit/reset、invalid、file-input、checkbox/radio input/change 和 SELECT input/change 适配。这些表单属性通过既有 attribute callback bridge 实现；validation query 通过独立的 size-tagged callback 获取 core 的控件状态；对 file input，programmatic click 只负责 typed click 分发；系统 picker、文件系统权限和窗口生命周期仍由宿主 GUI 拥有。
...
- `PBrowser_ScriptSession*`：创建/销毁 PScript context、求值、JSON global、bootstrap、
  ...
  `HTMLElement.click()`、`HTMLElement.disabled`、`checkValidity()`/`willValidate`/`validity` 查询、约束相关 `required`/`readOnly`/`multiple`/`noValidate`/
...
按 DOM id 返回控制候选的 `valid`、`will_validate` 和 flags；它是查询边界，不实现 form
`checkValidity()`、`validationMessage`、invalid 事件触发或 native invalid UI。产品层只管理
