...
按 `childNodes` 索引物化到 live Element。wrapper 最多 64 个属性和 64 个 Text child，
`childNodes`、首尾 child、`hasChildNodes()` 跟随 staging。
...
属性/Text 且与源隔离，连接前须有唯一 id。结构、嵌套、无 id 等非法输入 fail closed。
