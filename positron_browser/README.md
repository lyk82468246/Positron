...
- `File`/`Blob` 是 session 内的有界内存对象，只接受受限文本/字节序列并暴露名称、类型和大小；Browser 不替宿主读 picker 路径，也不把脚本 `FormData` pairs 转成 Core multipart handle。要发送由 Core DOM successful-control snapshot 产生的 multipart body，应用仍直接调用 `PCore_FormDataEncode()` 并提供同步 file read/free callback；脚本对象到 wire body 的完整桥接尚未实现；
