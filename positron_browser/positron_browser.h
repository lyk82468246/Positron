...
} PBrowserScriptDomWriteCallbacksEx4;

/* Extended Text mutation table. Ex4 remains ABI-fixed for existing hosts;
 * Ex5 appends the bounded Node.normalize() adapter. The callback normalizes
 * the addressed element's direct child list; Browser may invoke it for
 * addressed descendants in a controlled order. element_id is borrowed UTF-8
 * for the synchronous call. normalize_child_text returns >0 after a
 * successful call (including a no-op), 0 when the element is unavailable and
 * <0 on adapter failure. The host must re-query and restyle/layout/paint
 * after a successful mutation. */
typedef int (*PBrowserScriptNormalizeChildTextFn)(void *pw,
        const char *element_id);
typedef struct PBrowserScriptDomWriteCallbacksEx5 {
    unsigned long size;
    void *pw;
    PBrowserScriptSetTextFn set_text;
    PBrowserScriptSetTextChildFn set_child_text;
...
/* Extended HTML write table. Ex7 remains ABI-fixed for the existing bounded
 * text/CharacterData insertion surface; Ex8 appends a parser-backed
 * Element.innerHTML setter. `id` and `html` are borrowed UTF-8 for the
 * synchronous call. The host keeps the addressed element's identity, replaces
 * only its child list, and must schedule a fresh style/layout/paint pass after
 * success. No outerHTML setter, script execution, resource fetch or DOM
 * event dispatch is implied by this callback. */
typedef int (*PBrowserScriptSetInnerHTMLFn)(void *pw, const char *id,
        const char *html);
typedef struct PBrowserScriptDomWriteCallbacksEx8 {
    unsigned long size;
    void *pw;
    PBrowserScriptSetTextFn set_text;
    PBrowserScriptSetTextChildFn set_child_text;
...
/* Extended HTML write table. Ex9 remains ABI-fixed; Ex10 appends the
 * bounded Element.outerHTML setter. `id` and `html` are borrowed UTF-8 for
 * the synchronous call. The Core adapter accepts one Element root or an
 * empty string (remove), preserves the parent position on success, and must
 * schedule a fresh style/layout/paint pass. Top-level text/multiple roots,
 * script execution, resource fetches, mutation events and DocumentFragment
 * exposure remain outside this callback. */
typedef int (*PBrowserScriptSetOuterHTMLFn)(void *pw, const char *id,
        const char *html);
typedef struct PBrowserScriptDomWriteCallbacksEx10 {
    unsigned long size;
    void *pw;
    PBrowserScriptSetTextFn set_text;
    PBrowserScriptSetTextChildFn set_child_text;
...
/* Extended DOM mutation table. The original table remains ABI-fixed; Ex2
 * appends a bounded direct Text-child removal adapter while reusing the same
 * `__pcoreRemoveChild` native JSON slot. `parent_id` is borrowed UTF-8 and
 * `child_index` is the unfiltered childNodes index for the synchronous call.
 * remove_text_child returns >0 after removal, 0 when the parent/index/type is
 * unavailable and <0 on adapter failure. The host must re-query and schedule
 * style/layout/paint after success. Element children, document structure,
 * reparenting and insertion remain outside this callback. */
typedef int (*PBrowserScriptRemoveTextChildFn)(void *pw,
        const char *parent_id, unsigned int child_index);
typedef struct PBrowserScriptDomMutationCallbacksEx2 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
} PBrowserScriptDomMutationCallbacksEx2;

/* Extended DOM mutation table. Ex2 remains ABI-fixed for existing hosts;
 * Ex3 appends a bounded direct CharacterData-child removal adapter while
 * preserving both older callbacks. `node_type` is the DOM value 3 (Text),
 * 4 (CDATA) or 8 (Comment), and `child_index` is the unfiltered childNodes
 * index. The Browser emits the Text operation through remove_text_child and
 * dispatches Comment/CDATA through remove_character_data_child. The
 * callback returns >0 after removal, 0 when the parent/index/type is
 * unavailable and <0 on adapter failure. The host must re-query and
 * schedule style/layout/paint after success. Element children, document
 * structure, reparenting and insertion remain outside this callback. */
typedef int (*PBrowserScriptRemoveCharacterDataChildFn)(void *pw,
        const char *parent_id, unsigned int child_index,
        unsigned int node_type);
typedef struct PBrowserScriptDomMutationCallbacksEx3 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
    PBrowserScriptRemoveCharacterDataChildFn remove_character_data_child;
} PBrowserScriptDomMutationCallbacksEx3;

/* Extended DOM mutation table. Ex3 remains ABI-fixed for existing hosts;
 * Ex4 appends a bounded existing-element insertion adapter while preserving
 * all three removal callbacks. `parent_id` and `child_id` are borrowed UTF-8
 * element ids. `reference_child_id` is NULL for append, otherwise it names a
 * direct element child of `parent_id`. insert_child returns >0 after the
 * existing element was inserted (including a same-node no-op), 0 when the
 * relation or hierarchy is unavailable and <0 on adapter failure. The host
 * must re-query and schedule style/layout/paint after success. Text nodes,
 * document structure tokens, fragments and event dispatch remain outside
 * this bounded callback. */
typedef int (*PBrowserScriptInsertChildFn)(void *pw,
        const char *parent_id, const char *child_id,
        const char *reference_child_id);
typedef struct PBrowserScriptDomMutationCallbacksEx4 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
} PBrowserScriptDomMutationCallbacksEx4;

/* Extended DOM mutation table. Ex4 remains ABI-fixed for existing hosts;
 * Ex5 appends a bounded existing-element replacement adapter. `parent_id`,
 * `new_child_id` and `old_child_id` are borrowed UTF-8 element ids. The old
 * id must name a direct child of parent_id and the new id must name an
 * attached element in this document. replace_child returns >0 after a
 * replacement (including a same-node no-op), 0 when relation or hierarchy is
 * unavailable and <0 on adapter failure. The host must re-query and schedule
 * style/layout/paint after success. Text/Comment/CDATA, fragments, detached
 * nodes and mutation events remain outside this bounded callback. */
typedef int (*PBrowserScriptReplaceChildFn)(void *pw,
        const char *parent_id, const char *new_child_id,
        const char *old_child_id);
typedef struct PBrowserScriptDomMutationCallbacksEx5 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
} PBrowserScriptDomMutationCallbacksEx5;

/* Extended DOM mutation table. Ex5 remains ABI-fixed for existing hosts;
 * Ex6 appends a bounded existing-element insertion-at-index adapter while
 * preserving all five older callbacks. `parent_id` and `child_id` are
 * borrowed UTF-8 element ids; `child_index` is an unfiltered childNodes
 * position and may equal the current count to append. The operation may move
 * the element from another parent, but document structure tokens, detached
 * nodes, Text/Comment/CDATA arguments and hierarchy cycles remain outside
 * this callback. insert_child_at returns >0 after insertion, 0 when the
 * relation or index is unavailable and <0 on adapter failure. The host must
 * re-query and schedule style/layout/paint after success. */
typedef int (*PBrowserScriptInsertChildAtFn)(void *pw,
        const char *parent_id, const char *child_id,
        unsigned int child_index);
typedef struct PBrowserScriptDomMutationCallbacksEx6 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
/* Extended DOM mutation table. Ex7 remains ABI-fixed for existing hosts;
 * Ex8 appends a bounded variadic primitive-text replacement adapter while
 * preserving all seven older callbacks. `parent_id` and `old_child_id` are
 * borrowed UTF-8 element ids. `texts` points to `text_count` borrowed UTF-8
 * strings valid only for the synchronous call; the count is between one and
 * four. The callback must replace the direct old element with the complete
 * list atomically, returning >0 after success, 0 when the relation or list is
 * unavailable and <0 on adapter failure. The host must re-query and schedule
 * style/layout/paint after success. Existing-element, CharacterData,
 * DocumentFragment and mutation-event values remain outside this callback. */
typedef int (*PBrowserScriptReplaceChildWithTextListFn)(void *pw,
        const char *parent_id, const char *old_child_id,
        const char *const *texts, unsigned int text_count);
typedef struct PBrowserScriptDomMutationCallbacksEx8 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
/* Extended DOM mutation table. Ex9 remains ABI-fixed for existing hosts;
 * Ex10 appends a bounded existing-CharacterData insertion adapter while
 * preserving all nine older callbacks. `source_parent_id` and
 * `target_parent_id` are borrowed UTF-8 element ids. `source_index` and
 * `target_index` are unfiltered childNodes positions; the target index may
 * equal the current destination count to append. `node_type` is Text (3),
 * CDATA (4) or Comment (8). The callback moves the existing node without
 * cloning, returning >0 after success (including a same-parent no-op before
 * itself), 0 when a relation, type or index is unavailable and <0 on adapter
 * failure. The host must re-query and schedule style/layout/paint after
 * success. Elements, fragments, document structure and mutation events
 * remain outside this bounded callback. */
typedef int (*PBrowserScriptInsertCharacterDataChildAtFn)(void *pw,
        const char *source_parent_id, unsigned int source_index,
        unsigned int node_type, const char *target_parent_id,
        unsigned int target_index);
typedef struct PBrowserScriptDomMutationCallbacksEx10 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
} PBrowserScriptDomMutationCallbacksEx10;

/* Extended DOM mutation table. Ex10 remains ABI-fixed for existing hosts;
 * Ex11 appends a bounded existing-CharacterData replacement adapter.
 * `target_parent_id`/`source_parent_id` are borrowed UTF-8 element ids;
 * target/source indexes are unfiltered childNodes positions and the explicit
 * node types must match the indexed old/new Text, CDATA or Comment nodes.
 * The callback moves the new node into the old node's position without
 * cloning, returning >0 after replacement (including a same-node no-op), 0
 * when a relation, type or index is unavailable and <0 on adapter failure.
 * The host must re-query and schedule style/layout/paint after success.
 * Elements, fragments, document structure and mutation events remain
 * outside this bounded callback. */
typedef int (*PBrowserScriptReplaceCharacterDataChildFn)(void *pw,
...
typedef struct PBrowserScriptDomMutationCallbacksEx11 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
} PBrowserScriptDomMutationCallbacksEx11;

/* Extended DOM mutation table. Ex11 remains ABI-fixed for existing hosts;
 * Ex12 appends a bounded existing-CharacterData-to-element replacement
 * adapter. `parent_id`/`old_child_id` identify the target element and its
 * direct element child; `source_parent_id`/`source_index` identify the new
 * direct Text, CDATA or Comment child and `node_type` must match that node.
 * The callback moves the existing CharacterData node without cloning and
 * detaches the old element. It returns >0 after replacement, 0 when a
 * relation, type or hierarchy is unavailable and <0 on adapter failure.
 * The host must re-query and schedule style/layout/paint after success.
 * DocumentFragment, other node types and mutation events remain outside
 * this bounded callback. */
typedef int (*PBrowserScriptReplaceElementChildWithCharacterDataFn)(void *pw,
        const char *parent_id, const char *old_child_id,
        const char *source_parent_id, unsigned int source_index,
        unsigned int node_type);
typedef struct PBrowserScriptDomMutationCallbacksEx12 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
/* Extended DOM mutation table. Ex13 remains ABI-fixed for existing hosts;
 * Ex14 appends an atomic bounded mixed Element.replaceChildren() adapter.
 * `ELEMENT` items name already-connected direct Element children of the same
 * target; `TEXT` items carry borrowed UTF-8 strings. The item array and its
 * strings are valid only for the synchronous callback. Zero items clear the
 * target. Cross-parent nodes, CharacterData, fragments and duplicate
 * elements remain outside this adapter. The callback returns >0 after
 * success, 0 when the relation is unavailable and <0 on adapter failure. */
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_ITEM_TEXT    1u
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_ITEM_ELEMENT 2u
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_MIXED_LIST_MAX 4u
typedef struct PBrowserScriptReplaceChildrenItem {
    unsigned int kind;
    const char *element_id;
    const char *text;
} PBrowserScriptReplaceChildrenItem;
typedef int (*PBrowserScriptReplaceElementChildrenWithMixedListFn)(void *pw,
        const char *parent_id,
        const PBrowserScriptReplaceChildrenItem *items,
        unsigned int item_count);
typedef struct PBrowserScriptDomMutationCallbacksEx14 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
} PBrowserScriptDomMutationCallbacksEx14;

/* Ex15 preserves the Ex14 table and appends an atomic node-aware
 * Element.replaceChildren() adapter.  `ELEMENT` items name already-connected
 * direct Element children; `CHARACTER_DATA` items address a direct Text,
 * Comment or CDATA child by its pre-mutation child index and node type;
 * `TEXT` items carry borrowed primitive UTF-8 strings.  The item array and
 * strings are valid only for the synchronous callback.  Zero items clear the
 * target.  Cross-parent nodes, fragments, document structure tokens and
 * duplicate nodes remain rejected.  The callback returns >0 after success,
 * 0 when the relation is unavailable and <0 on adapter failure. */
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_NODE_ITEM_TEXT 1u
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_NODE_ITEM_ELEMENT 2u
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_NODE_ITEM_CHARACTER_DATA 3u
#define PBROWSER_SCRIPT_REPLACE_CHILDREN_NODE_LIST_MAX 4u
typedef struct PBrowserScriptReplaceChildrenNodeItem {
    unsigned int kind;
    unsigned int node_type;
    const char *element_id;
    unsigned int child_index;
    const char *text;
} PBrowserScriptReplaceChildrenNodeItem;
typedef int (*PBrowserScriptReplaceElementChildrenWithNodeListFn)(void *pw,
        const char *parent_id,
        const PBrowserScriptReplaceChildrenNodeItem *items,
        unsigned int item_count);
typedef struct PBrowserScriptDomMutationCallbacksEx15 {
    unsigned long size;
    void *pw;
    PBrowserScriptRemoveChildFn remove_child;
    PBrowserScriptRemoveTextChildFn remove_text_child;
...
} PBrowserScriptDomMutationCallbacksEx15;
 ...
 * The script-side FormData object shares the same fixed 64-entry budget for
 * append() and for set() of a new name. An operation that would exceed that
 * budget throws a QuotaExceededError and leaves the previous pairs unchanged;
 * replacing an existing name remains allowed at the limit. After a successful
 * Browser-side construction, the script layer dispatches
 ...
 * generated by the native callback. The independent script-side
 * URLSearchParams object uses PBROWSER_SCRIPT_URL_SEARCH_PARAMS_MAX_PAIRS for
 * append(), set() of a new name and pair-sequence construction; over-limit
 * operations throw the same DOMException name without partial mutation, while
 * replacement and append after deletion remain allowed. */
