...
`positron_script.h` 为准（当前 `PSCRIPT_MAX_NATIVE_FUNCTIONS` 为 18，浏览器组合层的
validation/report-validity/custom-validity typed callbacks 也计入这个受控槽位）。模块 provider 的源代码和释放回调由宿主拥有，DLL 只在
同步调用约定内使用。
